...
class Walker_Nav_Menu_Edit extends Walker_Nav_Menu {
	 ...
	 * @since 1.0.0
	 *
	 * @global int $_bd_nav_menu_max_depth
	 *
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		global $_bd_nav_menu_max_depth;
		$_bd_nav_menu_max_depth = $depth > $_bd_nav_menu_max_depth ? $depth : $_bd_nav_menu_max_depth;
 ...
 */
function _bd_ajax_menu_quick_search( $request = array() ) {
	$args = array();
	...
	if ( 'get-post-item' == $type ) {
		if ( post_type_exists( $object_type ) ) {
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', array( get_post( $object_id ) ) ), 0, (object) $args );
			...
			if ( isset( $request['ID'] ) ) {
				$object_id = (int) $request['ID'];
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', array( get_term( $object_id, $object_type ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
		...
		if ( 'posttype' == $matches[1] && get_post_type_object( $matches[2] ) ) {
			...
			while ( have_posts() ) {
				...
				if ( 'markup' == $response_format ) {
					...
					echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', array( get_post( $var_by_ref ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo bd_json_encode(
						array(
							'ID' => get_the_ID(),
			...
			if ( empty( $terms ) || is_bd_error( $terms ) )
				return;
			foreach( (array) $terms as $term ) {
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', array( $term ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo bd_json_encode(
						array(
							'ID' => $term->term_id,
 ...
 * @since 1.0.0
 **/
function bd_nav_menu_setup() {
	// Register meta boxes
	bd_nav_menu_post_type_meta_boxes();
	add_meta_box( 'add-custom-links', __( 'Custom Links' ), 'bd_nav_menu_item_link_meta_box', 'nav-menus', 'side', 'default' );
	bd_nav_menu_taxonomy_meta_boxes();

	// Register advanced menu items (columns)
	add_filter( 'manage_nav-menus_columns', 'bd_nav_menu_manage_columns' );
 ...
 **/
function bd_initial_nav_menu_meta_boxes() {
	global $bd_meta_boxes;

	if ( get_user_option( 'metaboxhidden_nav-menus' ) !== false || ! is_array($bd_meta_boxes) )
		return;

	$initial_meta_boxes = array( 'add-page', 'add-post', 'add-custom-links', 'add-category' );
	$hidden_meta_boxes = array();

	foreach ( array_keys($bd_meta_boxes['nav-menus']) as $context ) {
		foreach ( array_keys($bd_meta_boxes['nav-menus'][$context]) as $priority ) {
			foreach ( $bd_meta_boxes['nav-menus'][$context][$priority] as $box ) {
 ...
 * @since 1.0.0
 */
function bd_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( $post_type ) {
			...
			add_meta_box( "add-{$id}", $post_type->labels->name, 'bd_nav_menu_item_post_type_meta_box', 'nav-menus', 'side', $priority, $post_type );
		}
	}
}
 ...
 * @since 1.0.0
 */
function bd_nav_menu_taxonomy_meta_boxes() {
	...
	foreach ( $taxonomies as $tax ) {
		...
		if ( $tax ) {
			$id = $tax->name;
			add_meta_box( "add-{$id}", $tax->labels->name, 'bd_nav_menu_item_taxonomy_meta_box', 'nav-menus', 'side', 'default', $tax );
		}
	}
}
...
*/
function bd_nav_menu_disabled_check( $nav_menu_selected_id ) {
 ...
 */
function bd_nav_menu_item_link_meta_box() {
	...
	<div class="customlinkdiv" id="customlinkdiv">
		...
		<p class="button-controls">
			<span class="add-to-menu">
				<input type="submit"<?php bd_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e('Add to Menu'); ?>" name="add-custom-menu-item" id="submit-customlinkdiv" />
 ...
 */
function bd_nav_menu_item_post_type_meta_box( $object, $post_type ) {
	global $_nav_menu_placeholder, $nav_menu_selected_id;
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		...
		?>">
			<ul id="<?php echo $post_type_name; ?>checklist-most-recent" class="categorychecklist form-no-clear">
				 ...
				 * @param array  $args        An array of BD_Query arguments.
				...
				echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $most_recent), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>" id="tabs-panel-posttype-<?php echo $post_type_name; ?>-search">
			...
			<ul id="<?php echo $post_type_name; ?>-search-checklist" data-bd-lists="list:<?php echo $post_type_name?>" class="categorychecklist form-no-clear">
			<?php if ( ! empty( $search_results ) && ! is_bd_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $search_results), 0, (object) $args );
				?>
			<?php elseif ( is_bd_error( $search_results ) ) : ?>
		...
		?>">
			...
			<?php endif; ?>
			<ul id="<?php echo $post_type_name; ?>checklist" data-bd-lists="list:<?php echo $post_type_name?>" class="categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				...
				$checkbox_items = walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $posts), 0, (object) $args );
		...
		<p class="button-controls">
			...
			<span class="add-to-menu">
				<input type="submit"<?php bd_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>" name="add-post-type-menu-item" id="<?php echo esc_attr( 'submit-posttype-' . $post_type_name ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
 ...
 */
function bd_nav_menu_item_taxonomy_meta_box( $object, $taxonomy ) {
	...
	if ( ! $terms || is_bd_error($terms) ) {
		echo '<p>' . __( 'No items.' ) . '</p>';
		return;
	}

	$num_pages = ceil( bd_count_terms( $taxonomy_name , array_merge( $args, array('number' => '', 'offset' => '') ) ) / $per_page );
	...
	<div id="taxonomy-<?php echo $taxonomy_name; ?>" class="taxonomydiv">
		...
		?>">
			<ul id="<?php echo $taxonomy_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				...
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $popular_terms), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>">
			...
			<?php endif; ?>
			<ul id="<?php echo $taxonomy_name; ?>checklist" data-bd-lists="list:<?php echo $taxonomy_name?>" class="categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $terms), 0, (object) $args );
				?>
			</ul>
			<?php if ( ! empty( $page_links ) ) : ?>
				<div class="add-menu-item-pagelinks">
		...
		?>" id="tabs-panel-search-taxonomy-<?php echo $taxonomy_name; ?>">
			...
			<ul id="<?php echo $taxonomy_name; ?>-search-checklist" data-bd-lists="list:<?php echo $taxonomy_name?>" class="categorychecklist form-no-clear">
			<?php if ( ! empty( $search_results ) && ! is_bd_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $search_results), 0, (object) $args );
		...
		<p class="button-controls">
			...
			<span class="add-to-menu">
				<input type="submit"<?php bd_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>" name="add-taxonomy-menu-item" id="<?php echo esc_attr( 'submit-taxonomy-' . $taxonomy_name ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
 ...
 */
function bd_save_nav_menu_items( $menu_id = 0, $menu_data = array() ) {
	$menu_id = (int) $menu_id;
	$items_saved = array();
	...
	if ( 0 == $menu_id || is_nav_menu( $menu_id ) ) {
		...
		foreach( (array) $menu_data as $_possible_db_id => $_item_object_data ) {
			...
			$items_saved[] = bd_update_nav_menu_item( $menu_id, $_actual_db_id, $args );
 ...
 */
function _bd_nav_menu_meta_box_object( $object = null ) {
 ...
 * @return string|BD_Error $output The menu formatted to edit or error object on failure.
 */
function bd_get_nav_menu_to_edit( $menu_id = 0 ) {
	$menu = bd_get_nav_menu_object( $menu_id );

	// If the menu exists, get its items.
	if ( is_nav_menu( $menu ) ) {
		$menu_items = bd_get_nav_menu_items( $menu->term_id, array('post_status' => 'any') );
		 ...
		 */
		$walker_class_name = apply_filters( 'bd_edit_nav_menu_walker', 'Walker_Nav_Menu_Edit', $menu_id );

		if ( class_exists( $walker_class_name ) )
			$walker = new $walker_class_name;
		else
			return new BD_Error( 'menu_walker_not_exist', sprintf( __('The Walker class named <strong>%s</strong> does not exist.'), $walker_class_name ) );
		...
		$result .= walk_nav_menu_tree( array_map('bd_setup_nav_menu_item', $menu_items), 0, (object) array('walker' => $walker ) );
		$result .= ' </ul> ';
		return $result;
	} elseif ( is_bd_error( $menu ) ) {
		return $menu;
	}
 ...
 * @return string|BD_Error $output The menu formatted to edit or error object on failure.
 */
function bd_nav_menu_manage_columns() {
	return array(
 ...
 * @global bddb $bddb
 */
function _bd_delete_orphaned_draft_menu_items() {
	global $bddb;
	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	// Delete orphaned draft menu items.
	$menu_items_to_delete = $bddb->get_col($bddb->prepare("SELECT ID FROM $bddb->posts AS p LEFT JOIN $bddb->postmeta AS m ON p.ID = m.post_id WHERE post_type = 'nav_menu_item' AND post_status = 'draft' AND meta_key = '_menu_item_orphaned' AND meta_value < '%d'", $delete_timestamp ) );

	foreach( (array) $menu_items_to_delete as $menu_item_id )
		bd_delete_post( $menu_item_id, true );
}
 ...
 */
function bd_nav_menu_update_menu_items ( $nav_menu_selected_id, $nav_menu_selected_title ) {
	$unsorted_menu_items = bd_get_nav_menu_items( $nav_menu_selected_id, array( 'orderby' => 'ID', 'output' => ARRAY_A, 'output_key' => 'ID', 'post_status' => 'draft,publish' ) );
	$messages = array();
	$menu_items = array();
	...
	if ( ! empty( $_POST['menu-item-db-id'] ) ) {
		foreach( (array) $_POST['menu-item-db-id'] as $_key => $k ) {
			...
			$menu_item_db_id = bd_update_nav_menu_item( $nav_menu_selected_id, ( $_POST['menu-item-db-id'][$_key] != $_key ? 0 : $_key ), $args );

			if ( is_bd_error( $menu_item_db_id ) ) {
	...
	$nav_menu_option['auto_add'] = array_intersect( $nav_menu_option['auto_add'], bd_get_nav_menus( array( 'fields' => 'ids' ) ) );
	update_option( 'nav_menu_options', $nav_menu_option );

	bd_defer_term_counting( false );

	/** This action is documented in bd-includes/nav-menu.php */
	do_action( 'bd_update_nav_menu', $nav_menu_selected_id );
