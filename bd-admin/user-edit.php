...
bd_reset_vars( array( 'action', 'user_id', 'bd_http_referer' ) );
...
$profile_help = '<p>' . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using Blasdoise.') . '</p>' .
	'<p>' . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your Blasdoise administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.') . '</p>' .
...
$bd_http_referer = remove_query_arg(array('update', 'delete_count'), $bd_http_referer );
...
if ( is_multisite()
	...
	bd_die( __( 'You do not have permission to edit this user.' ) );
}
...
if ( is_multisite() && IS_PROFILE_PAGE && isset( $_GET[ 'newuseremail' ] ) && $current_user->ID ) {
	...
	if ( $new_email[ 'hash' ] == $_GET[ 'newuseremail' ] ) {
		...
		if ( $bddb->get_var( $bddb->prepare( "SELECT user_login FROM {$bddb->signups} WHERE user_login = %s", $current_user->user_login ) ) )
			$bddb->query( $bddb->prepare( "UPDATE {$bddb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		bd_update_user( $user );
		delete_option( $current_user->ID . '_new_email' );
		bd_redirect( add_query_arg( array('updated' => 'true'), self_admin_url( 'profile.php' ) ) );
		die();
	}
...
if ( is_multisite() ) {
	...
	if ( $user->user_login && isset( $_POST[ 'email' ] ) && is_email( $_POST[ 'email' ] ) && $bddb->get_var( $bddb->prepare( "SELECT user_login FROM {$bddb->signups} WHERE user_login = %s", $user->user_login ) ) ) {
		$bddb->query( $bddb->prepare( "UPDATE {$bddb->signups} SET user_email = %s WHERE user_login = %s", $_POST[ 'email' ], $user_login ) );
	}
}
...
if ( !is_bd_error( $errors ) ) {
	...
	if ( $bd_http_referer )
		$redirect = add_query_arg('bd_http_referer', urlencode($bd_http_referer), $redirect);
...
$sessions = BD_Session_Tokens::get_instance( $profileuser->ID );
...
<div id="message" class="updated notice is-dismissible">
	...
	<?php endif; ?>
	<?php if ( $bd_http_referer && !IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( $bd_http_referer ); ?>"><?php _e('&larr; Back to Users'); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
...
?>>
<?php bd_nonce_field('update-user_' . $user_id) ?>
<?php if ( $bd_http_referer ) : ?>
	<input type="hidden" name="bd_http_referer" value="<?php echo esc_url($bd_http_referer); ?>" />
<?php endif; ?>
<p>
...
<?php endif; ?>
<?php if ( count($_bd_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>
 ...
 * @since 1.0.0
 *
 * @param BD_User $profileuser The current BD_User object.
 */
...
<?php
	foreach ( bd_get_user_contact_methods( $profileuser ) as $name => $desc ) {
?>
...
<tr id="password" class="user-pass1-wrap">
	...
	<td>
		...
		<button type="button" class="button button-secondary bd-generate-pw hide-if-no-js"><?php _e( 'Generate Password' ); ?></button>
		<div class="bd-pwd hide-if-js">
			<span class="password-input-wrapper">
				<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="off" data-pw="<?php echo esc_attr( bd_generate_password( 24 ) ); ?>" aria-describedby="pass-strength-result" />
			</span>
