 ...
 * @since 1.0.0
 */
class BD_Widget_Pages extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		$out = bd_list_pages( apply_filters( 'widget_pages_args', array(
			'title_li'    => '',
			'echo'        => 0,
			'sort_column' => $sortby,
			'exclude'     => $exclude
	...
	public function form( $instance ) {
		//Defaults
		$instance = bd_parse_args( (array) $instance, array( 'sortby' => 'post_title', 'title' => '', 'exclude' => '') );
 ...
 * @since 1.0.0
 */
class BD_Widget_Links extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		bd_list_bookmarks( apply_filters( 'widget_links_args', array(
	...
	public function form( $instance ) {
		...
		//Defaults
		$instance = bd_parse_args( (array) $instance, array( 'images' => true, 'name' => true, 'description' => false, 'rating' => false, 'category' => false, 'orderby' => 'name', 'limit' => -1 ) );
 ...
 * @since 1.0.0
 */
class BD_Widget_Search extends BD_Widget {
	 ...
	 */
	public function widget( $args, $instance ) {
		/** This filter is documented in bd-includes/default-widgets.php */
	 ...
	 */
	public function form( $instance ) {
		$instance = bd_parse_args( (array) $instance, array( 'title' => '') );
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = bd_parse_args((array) $new_instance, array( 'title' => ''));
		$instance['title'] = strip_tags($new_instance['title']);
		return $instance;
	}
 ...
 * @since 1.0.0
 */
class BD_Widget_Archives extends BD_Widget {
...
?>
		...
		<select id="<?php echo esc_attr( $dropdown_id ); ?>" name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'>
			...
			<?php bd_get_archives( $dropdown_args ); ?>
...
<?php
		 ...
		 */
		bd_get_archives( apply_filters( 'widget_archives_args', array(
			'type'            => 'monthly',
			'show_post_count' => $c
		) ) );
?>
...
<?php
	...
	public function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
		$new_instance = bd_parse_args( (array) $new_instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
		$instance['title'] = strip_tags($new_instance['title']);
	 ...
	 */
	public function form( $instance ) {
		$instance = bd_parse_args( (array) $instance, array( 'title' => '', 'count' => 0, 'dropdown' => '') );
 ...
 * @since 1.0.0
 */
class BD_Widget_Meta extends BD_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'widget_meta', 'description' => __( "Login, RSS, &amp; Blasdoise links.") );
...
<?php
			/**
			 * Filter the "Powered by Blasdoise" text in the Meta widget.
			 *
			 * @since 1.0.0
			 *
			 * @param string $title_text Default title text for the Blasdoise link.
			 */
			echo apply_filters( 'widget_meta_poweredby', sprintf( '<li><a href="%s" title="%s">%s</a></li>',
				esc_url( __( 'http://blasdoise.com/' ) ),
				esc_attr__( 'Powered by Blasdoise, state-of-the-art personal publishing platform.' ),
				_x( 'Blasdoise', 'meta widget link text' )
			) );
 ...
 * @since 1.0.0
 */
class BD_Widget_Calendar extends BD_Widget {
 ...
 * @since 1.0.0
 */
class BD_Widget_Text extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 * @param BD_Widget $instance    BD_Widget instance.
		 */
		...
		if ( ! empty( $title ) ) {
			...
			<div class="textwidget"><?php echo !empty( $instance['filter'] ) ? bdautop( $text ) : $text; ?></div>
		<?php
		echo $args['after_widget'];
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( current_user_can('unfiltered_html') )
			...
			$instance['text'] = stripslashes( bd_filter_post_kses( addslashes($new_instance['text']) ) ); // bd_filter_post_kses() expects slashed
	 ...
	 */
	public function form( $instance ) {
		$instance = bd_parse_args( (array) $instance, array( 'title' => '', 'text' => '' ) );
		$title = strip_tags($instance['title']);
 ...
 * @since 1.0.0
 */
class BD_Widget_Categories extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $d ) {
			 ...
			 */
			bd_dropdown_categories( apply_filters( 'widget_categories_dropdown_args', $cat_args ) );
?>
...
<?php
		 ...
		 */
		bd_list_categories( apply_filters( 'widget_categories_args', $cat_args ) );
?>
		</ul>
<?php
		}
 ...
 * @since 1.0.0
 */
class BD_Widget_Recent_Posts extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = bd_cache_get( 'widget_recent_posts', 'widget' );
		}
		 ...
		 * @since 1.0.0
		 *
		 * @see BD_Query::get_posts()
		 *
		 * @param array $args An array of arguments used to retrieve the recent posts.
		 */
		$r = new BD_Query( apply_filters( 'widget_posts_args', array(
...
<?php
		...
		if ( ! $this->is_preview() ) {
			...
			bd_cache_set( 'widget_recent_posts', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		$alloptions = bd_cache_get( 'alloptions', 'options' );
	...
	public function flush_widget_cache() {
		bd_cache_delete('widget_recent_posts', 'widget');
	}
 ...
 * @since 1.0.0
 */
class BD_Widget_Recent_Comments extends BD_Widget {
	...
	public function __construct() {
		...
		if ( is_active_widget(false, false, $this->id_base) )
			add_action( 'bd_header', array($this, 'recent_comments_style') );
...
<?php
	 ...
	 */
	public function flush_widget_cache() {
		bd_cache_delete('widget_recent_comments', 'widget');
	}
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = bd_cache_get('widget_recent_comments', 'widget');
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		 ...
		 * @since 1.0.0
		 *
		 * @see BD_Comment_Query::query() for information on accepted arguments.
		 *
		...
		if ( is_array( $comments ) && $comments ) {
			...
			$post_ids = array_unique( bd_list_pluck( $comments, 'comment_post_ID' ) );
		...
		if ( ! $this->is_preview() ) {
			...
			bd_cache_set( 'widget_recent_comments', $cache, 'widget' );
		}
	}
 ...
 * @since 1.0.0
 */
class BD_Widget_RSS extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		...
		bd_widget_rss_output( $rss, $instance );
		echo $args['after_widget'];
	...
	public function update( $new_instance, $old_instance ) {
		...
		return bd_widget_rss_process( $new_instance, $testurl );
	}
 ...
 */
function bd_widget_rss_output( $rss, $args = array() ) {
	if ( is_string( $rss ) ) {
		$rss = fetch_feed($rss);
	...
	foreach ( $rss->get_items( 0, $items ) as $item ) {
		...
		$desc = esc_attr( bd_trim_words( $desc, 55, ' [&hellip;]' ) );
 ...
 */
function bd_widget_rss_form( $args, $inputs = null ) {
	$default_inputs = array( 'url' => true, 'title' => true, 'items' => true, 'show_summary' => true, 'show_author' => true, 'show_date' => true );
	$inputs = bd_parse_args( $inputs, $default_inputs );
 ...
 * @return array
 */
function bd_widget_rss_process( $widget_rss, $check_feed = true ) {
 ...
 * @since 1.0.0
 */
class BD_Widget_Tag_Cloud extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		 ...
		 */
		bd_tag_cloud( apply_filters( 'widget_tag_cloud_args', array(
 ...
 * @since 1.0.0
 */
 class BD_Nav_Menu_Widget extends BD_Widget {
	...
	public function widget( $args, $instance ) {
		// Get menu
		$nav_menu = ! empty( $instance['nav_menu'] ) ? bd_get_nav_menu_object( $instance['nav_menu'] ) : false;
		 ...
		 *     An array of arguments passed to bd_nav_menu() to retrieve a custom menu.
		 *
		 ...
		 */
		bd_nav_menu( apply_filters( 'widget_nav_menu_args', $nav_menu_args, $nav_menu, $args ) );
	...
	public function form( $instance ) {
		...
		<p class="nav-menu-widget-no-menus-message" <?php if ( ! empty( $menus ) ) { echo ' style="display:none" '; } ?>>
			<?php
			if ( isset( $GLOBALS['bd_customize'] ) && $GLOBALS['bd_customize'] instanceof BD_Customize_Manager ) {
				$url = 'javascript: bd.customize.panel( "nav_menus" ).focus();';
			} else {
...
/**
 * Register all of the default Blasdoise widgets on startup.
 *
 * Calls 'widgets_init' action after all of the Blasdoise widgets have been
 * registered.
 *
 * @since 1.0.0
 */
function bd_widgets_init() {
	...
	register_widget('BD_Widget_Pages');

	register_widget('BD_Widget_Calendar');

	register_widget('BD_Widget_Archives');

	if ( get_option( 'link_manager_enabled' ) )
		register_widget('BD_Widget_Links');

	register_widget('BD_Widget_Meta');

	register_widget('BD_Widget_Search');

	register_widget('BD_Widget_Text');

	register_widget('BD_Widget_Categories');

	register_widget('BD_Widget_Recent_Posts');

	register_widget('BD_Widget_Recent_Comments');

	register_widget('BD_Widget_RSS');

	register_widget('BD_Widget_Tag_Cloud');

	register_widget('BD_Nav_Menu_Widget');

	/**
	 * Fires after all default Blasdoise widgets have been registered.
	 *
	 * @since 1.0.0
	 */
