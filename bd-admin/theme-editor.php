...
if ( is_multisite() && ! is_network_admin() ) {
	bd_redirect( network_admin_url( 'theme-editor.php' ) );
	exit();
}

if ( !current_user_can('edit_themes') )
	bd_die('<p>'.__('You do not have sufficient permissions to edit templates for this site.').'</p>');
...
'content'	=>
	...
	<p>' . sprintf( __('Upgrading to a newer version of the same theme will override changes made here. To avoid this, consider creating a child theme instead.') ) . '</p>' .
...
bd_reset_vars( array( 'action', 'error', 'file', 'theme' ) );
...
$theme = bd_get_theme( $stylesheet );

if ( ! $theme->exists() )
	bd_die( __( 'The requested theme does not exist.' ) );

if ( $theme->errors() && 'theme_no_stylesheet' == $theme->errors()->get_error_code() )
	bd_die( __( 'The requested theme does not exist.' ) . ' ' . $theme->errors()->get_error_message() );
...
<?php
foreach ( bd_get_themes( array( 'errors' => null ) ) as $a_stylesheet => $a_theme ) {
...
<?php if ( $error ) :
	...
	<?php bd_nonce_field( 'edit-theme_' . $file . $stylesheet ); ?>
