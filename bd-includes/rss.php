 ...
 * @deprecated 1.0.0 Use SimplePie instead.
 */

/**
 * Deprecated. Use SimplePie (class-simplepie.php) instead.
 */
_deprecated_file( basename( __FILE__ ), '1.0', BDINC . '/class-simplepie.php' );
...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'Blasdoise/' . $GLOBALS['bd_version']);
...
/**
 * Retrieve URL headers and content using BD HTTP Request API.
 *
 * @since 1.0.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = bd_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_bd_error($resp) ) {
	...
	// Also note, BD_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( bd_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = bd_remote_retrieve_response_code( $resp );
	$response->response_code = bd_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = bd_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'Blasdoise/' . $GLOBALS['bd_version'];
...
class RSSCache {
	...
	function __construct( $base = '', $age = '' ) {
		$this->BASE_CACHE = BD_CONTENT_DIR . '/cache';
		if ( $base ) {
 ...
 */
function bd_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with bd_rss()
 * function.
 *
 * @since 1.0.0
 * @package External
