...
( function( tinymce ) {
// Set the minimum value for the modals z-index higher than #bdadminbar (100000)
tinymce.ui.FloatPanel.zIndex = 100100;

tinymce.PluginManager.add( 'blasdoise', function( editor ) {
	var bdAdvButton, style,
		DOM = tinymce.DOM,
		each = tinymce.each,
		__ = editor.editorManager.i18n.translate,
		$ = window.jQuery,
		bd = window.bd,
		hasWpautop = ( bd && bd.editor && bd.editor.autop && editor.getParam( 'bdautop', true ) );
	...
	function toggleToolbars( state ) {
		...
		if ( pixels && ! initial ) {
			...
			if ( state === 'hide' ) {
				...
				bdAdvButton && bdAdvButton.active( false );
			} else {
				setUserSetting('hidetb', '1');
				bdAdvButton && bdAdvButton.active( true );
			}
		}

		editor.fire( 'bd-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'bd_adv', {
		tooltip: 'Toolbar Toggle',
		cmd: 'BD_Adv',
		onPostRender: function() {
			bdAdvButton = this;
			bdAdvButton.active( getUserSetting( 'hidetb' ) === '1' ? true : false );
		}
	});

	// Hide the toolbars after loading
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'blasdoise_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		}
	});

	editor.addCommand( 'BD_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.bdActiveEditor = editor.id;
    });
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-bd-more="more" data-bd-more-text="' + moretext + '" ' +
						'class="bd-more-tag mce-bd-more" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-bd-more="nextpage" class="bd-more-tag mce-bd-nextpage" ' +
						'title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />' );
			}

			if ( event.load && event.format !== 'raw' && hasWpautop ) {
				event.content = bd.editor.autop( event.content );
			}
	...
	editor.on( 'PostProcess', function( e ) {
		if ( e.get ) {
			e.content = e.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-bd-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-bd-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					image = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-bd-more="nextpage"' ) !== -1 ) {
					image = '<!--nextpage-->';
				}
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-bd-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands
	editor.addCommand( 'BD_More', function( tag ) {
		var parent, html, title,
			classname = 'bd-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode();

		tag = tag || 'more';
		classname += ' mce-bd-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" title="' + title + '" class="' + classname + '" ' +
			'data-bd-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'BD_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'BD_Page', function() {
		editor.execCommand( 'BD_More', 'nextpage' );
	});

	editor.addCommand( 'BD_Help', function() {
		...
		if ( editor.plugins.bdtextpattern ) {
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-bd-help' );
	...
	editor.addCommand( 'BD_Medialib', function() {
		if ( bd && bd.media && bd.media.editor ) {
			bd.media.editor.open( editor.id );
		}
	});

	// Register buttons
	editor.addButton( 'bd_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'BD_More', 'more' );
		}
	});

	editor.addButton( 'bd_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'BD_More', 'nextpage' );
		}
	...
	editor.addButton( 'bd_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'BD_Help'
	});

	editor.addButton( 'bd_code', {
		tooltip: 'Code',
		cmd: 'BD_Code',
		stateSelector: 'code'
	});
		...
		if ( tinymce.Env.webkit && ( e.command === 'InsertUnorderedList' || e.command === 'InsertOrderedList' ) ) {
			if ( ! style ) {
				style = editor.dom.create( 'style', {'type': 'text/css'},
					'#tinymce,#tinymce span,#tinymce li,#tinymce li>span,#tinymce p,#tinymce p>span{font:medium sans-serif;color:#000000;line-height:normal;}');
			}
	...
	editor.on( 'init', function() {
		...
		if ( editor.getParam( 'bd_paste_filters', true ) ) {
	...
	editor.on( 'SaveContent', function( event ) {
		...
		if ( hasWpautop ) {
			event.content = bd.editor.removep( event.content );
		}
	});
	...
	editor.on( 'preInit', function() {
		...
		editor.addShortcut( 'meta+s', '', function() {
			if ( bd && bd.autosave ) {
				bd.autosave.server.triggerSave();
			}
		} );
	} );
	...
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			bdAdminbar = document.getElementById( 'bdadminbar' ),
			...
			if ( editor.id === 'content' ) {
				bdStatusbar = document.getElementById( 'post-status-info' );
			}
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					bdAdminbarBottom = bdAdminbar ? bdAdminbar.getBoundingClientRect().bottom : 0,
					...
					bdStatusbarTop = bdStatusbar ? windowHeight - bdStatusbar.getBoundingClientRect().top : 0,
					blockedTop = Math.max( 0, bdAdminbarBottom, mceToolbarBottom, iframeRect.top ),
					blockedBottom = Math.max( 0, mceStatusbarTop, bdStatusbarTop, windowHeight - iframeRect.bottom ),
		...
		editor.bd = editor.bd || {};
		editor.bd._createToolbar = create;
	}, true );
