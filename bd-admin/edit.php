...
if ( ! $typenow )
	bd_die( __( 'Invalid post type' ) );

if ( 'attachment' === $typenow ) {
	if ( bd_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
$bd_list_table = _get_list_table('BD_Posts_List_Table');
$pagenum = $bd_list_table->get_pagenum();

// Back-compat for viewing comments of an entry
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		bd_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array('trashed', 'untrashed', 'deleted', 'locked', 'ids'), bd_get_referer() );
	if ( ! $sendback )
	...
	if ( 'delete_all' == $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			$post_ids = $bddb->get_col( $bddb->prepare( "SELECT ID FROM $bddb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		}
		$doaction = 'delete';
		...
		case 'delete':
			...
			foreach( (array) $post_ids as $post_id ) {
				...
				if ( $post_del->post_type == 'attachment' ) {
					if ( ! bd_delete_attachment($post_id) )
						bd_die( __('Error in deleting.') );
				} else {
					if ( !bd_delete_post($post_id) )
						bd_die( __('Error in deleting.') );
				}
				$deleted++;
			}
	...
	bd_redirect($sendback);
	exit();
} elseif ( ! empty($_REQUEST['_bd_http_referer']) ) {
...
bd_enqueue_script('inline-edit-post');
bd_enqueue_script('heartbeat');
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = '<a href="' . esc_url( bd_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", "bulk-posts" ) ) . '">' . __('Undo') . '</a>';
	}
}
...
<?php $bd_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
