...
$( document ).on( 'heartbeat-tick.bd-check-locked-posts', function( e, data ) {
	var locked = data['bd-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('bd-locked') ) {
				...
				row.addClass('bd-locked');
			}
		} else if ( row.hasClass('bd-locked') ) {
			// Make room for the CSS animation
			row.removeClass('bd-locked').delay(1000).find('.locked-info span').empty();
		}
	});
}).on( 'heartbeat-send.bd-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['bd-check-locked-posts'] = check;
	}
}).ready( function() {
	// Set the heartbeat interval to 15 sec.
	if ( typeof bd !== 'undefined' && bd.heartbeat ) {
