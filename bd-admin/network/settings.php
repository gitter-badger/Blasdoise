...
if ( $_POST ) {
	...
	$options = array(
		...
		'illegal_names', 'limited_email_domains', 'banned_email_domains', 'BDLANG', 'admin_email',
	);

	// Handle translation install.
	if ( ! empty( $_POST['BDLANG'] ) && bd_can_install_language_pack() ) {  // @todo: Skip if already installed
	...
	foreach ( $options as $option_name ) {
		if ( ! isset($_POST[$option_name]) )
			continue;
		$value = bd_unslash( $_POST[$option_name] );
	 ...
	 * @since 1.0.0
	 */
	do_action( 'update_bdmu_options' );

	bd_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php bd_nonce_field( 'siteoptions' ); ?>
			...
			<tr>
				...
				<td>
					...
					<?php if ( is_subdomain_install() ) {
						echo '<p class="description">' . __( 'If registration is disabled, please set <code>NOBLOGREDIRECT</code> in <code>bd-config.php</code> to a URL you will redirect visitors to if they visit a non-existent site.' ) . '</p>';
					} ?>
					</fieldset>
				</td>
			</tr>
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ) ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table">
				<tr>
					<th><label for="BDLANG"><?php _e( 'Default Language' ); ?></label></th>
					<td>
						<?php
						$lang = get_site_option( 'BDLANG' );
						...
						bd_dropdown_languages( array(
							...
							'show_available_translations' => bd_can_install_language_pack(),
						) );
						?>
					</td>
				</tr>
