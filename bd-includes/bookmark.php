...
function get_bookmark($bookmark, $output = OBJECT, $filter = 'raw') {
	...
	if ( empty($bookmark) ) {
		...
		bd_cache_add($bookmark->link_id, $bookmark, 'bookmark');
		$_bookmark = $bookmark;
	} else {
		if ( isset($GLOBALS['link']) && ($GLOBALS['link']->link_id == $bookmark) ) {
			$_bookmark = & $GLOBALS['link'];
		} elseif ( ! $_bookmark = bd_cache_get($bookmark, 'bookmark') ) {
			$_bookmark = $bddb->get_row($bddb->prepare("SELECT * FROM $bddb->links WHERE link_id = %d LIMIT 1", $bookmark));
			if ( $_bookmark ) {
				$_bookmark->link_category = array_unique( bd_get_object_terms( $_bookmark->link_id, 'link_category', array( 'fields' => 'ids' ) ) );
				bd_cache_add( $_bookmark->link_id, $_bookmark, 'bookmark' );
			}
		}
	}
 ...
 * @return string|BD_Error
 */
function get_bookmark_field( $field, $bookmark, $context = 'display' ) {
	$bookmark = (int) $bookmark;
	$bookmark = get_bookmark( $bookmark );

	if ( is_bd_error($bookmark) )
		return $bookmark;
...
function get_bookmarks( $args = '' ) {
	...
	$key = md5( serialize( $r ) );
	if ( $cache = bd_cache_get( 'get_bookmarks', 'bookmark' ) ) {
	...
	if ( ! empty( $r['category_name'] ) ) {
		if ( $r['category'] = get_term_by('name', $r['category_name'], 'link_category') ) {
			...
			$cache[ $key ] = array();
			bd_cache_set( 'get_bookmarks', $cache, 'bookmark' );
			/** This filter is documented in bd-includes/bookmark.php */
	...
	$search = '';
	if ( ! empty( $r['search'] ) ) {
		$like = '%' . $bddb->esc_like( $r['search'] ) . '%';
		$search = $bddb->prepare(" AND ( (link_url LIKE %s) OR (link_name LIKE %s) OR (link_description LIKE %s) ) ", $like, $like, $like );
	}
	...
	if ( ! empty( $category_query ) ) {
		...
		$join = " INNER JOIN $bddb->term_relationships AS tr ON ($bddb->links.link_id = tr.object_id) INNER JOIN $bddb->term_taxonomy as tt ON tt.term_taxonomy_id = tr.term_taxonomy_id";
	}
	...
	$query = "SELECT * $length $recently_updated_test $get_updated FROM $bddb->links $join WHERE 1=1 $visible $category_query";
...
function clean_bookmark_cache( $bookmark_id ) {
	bd_cache_delete( $bookmark_id, 'bookmark' );
	bd_cache_delete( 'get_bookmarks', 'bookmark' );
