/* global _bdCustomizeWidgetsSettings */
(function( bd, $ ){
	...
	// Link settings
	api.Widgets.data = _bdCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;
	delete api.Widgets.data.l10n;

	/**
	 * bd.customize.Widgets.WidgetModel
	 *
	...
	/**
	 * bd.customize.Widgets.WidgetCollection
	 *
	...
	/**
	 * bd.customize.Widgets.SidebarModel
	 *
	...
	/**
	 * bd.customize.Widgets.SidebarCollection
	 *
	...
	/**
	 * bd.customize.Widgets.AvailableWidgetsPanelView
	 *
	 ...
	 */
	api.Widgets.AvailableWidgetsPanelView = bd.Backbone.View.extend({
	...
	/**
	 * bd.customize.Widgets.WidgetControl
	 *
	 * Customizer control for widgets.
	 * Note that 'widget_form' must match the BD_Widget_Form_Customize_Control::$type
	 *
	 * @constructor
	...
	api.Widgets.WidgetControl = api.Control.extend({
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.bd-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						bd.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						bd.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // bd_widget_control() outputs the link as "Delete"
		...
		/**
		 * @return {bd.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			params.bd_customize = 'on';
			params.nonce = api.Widgets.data.nonce;
			params.theme = api.settings.theme.stylesheet;
			params.customized = bd.customize.previewer.query().customized;
			...
			jqxhr = $.post( bd.ajax.settings.url, data );
		 ...
		 *
		 * @deprecated 1.0.0 Use this.expand() instead.
		 */
		expandForm: function() {
			this.expand();
		},
		 ...
		 *
		 * @deprecated 1.0.0 Use this.collapse() instead.
		 */
	...
	/**
	 * bd.customize.Widgets.SidebarSection
	 *
	...
	/**
	 * bd.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the BD_Widget_Area_Customize_Control::$type
	 *
	 * @since 1.0.0
	 *
	 * @constructor
	...
	api.Widgets.SidebarControl = api.Control.extend({
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[bd_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[bd_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[bd_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		 ...
		 *
		 * @since 1.0.0
		 * @return {bd.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo this can use widgetIdToSettingId(), then pass into bd.customize.control( x ).getSidebarWidgetsControl()
