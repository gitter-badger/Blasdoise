...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && ! isset( $_POST['html-upload'] ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
		bd_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}
 ...
 */
function bdmu_delete_blog( $blog_id, $drop = false ) {
	global $bddb;
	...
	// If ms_files_rewriting is enabled and upload_path is empty, bd_upload_dir is not reliable.
	...
	if ( $drop ) {
		 ...
		 */
		$drop_tables = apply_filters( 'bdmu_drop_tables', $tables, $blog_id );

		foreach ( (array) $drop_tables as $table ) {
			$bddb->query( "DROP TABLE IF EXISTS `$table`" );
		}

		$bddb->delete( $bddb->blogs, array( 'blog_id' => $blog_id ) );
		 ...
		 * @since 1.0.0
		 *
		 * @param string $uploads['basedir'] Uploads path without subdirectory. @see bd_upload_dir()
		 * @param int    $blog_id            The blog ID.
		 */
		$dir = apply_filters( 'bdmu_delete_blog_upload_dir', $uploads['basedir'], $blog_id );
 ...
 * @since 1.0.0
 *
 * @todo Merge with bd_delete_user() ?
 *
 * @global bddb $bddb
 *
...
function bdmu_delete_user( $id ) {
	...
	if ( ! empty( $blogs ) ) {
		foreach ( $blogs as $blog ) {
			...
			$post_ids = $bddb->get_col( $bddb->prepare( "SELECT ID FROM $bddb->posts WHERE post_author = %d", $id ) );
			foreach ( (array) $post_ids as $post_id ) {
				bd_delete_post( $post_id );
			}

			// Clean links
			$link_ids = $bddb->get_col( $bddb->prepare( "SELECT link_id FROM $bddb->links WHERE link_owner = %d", $id ) );
	...
	$meta = $bddb->get_col( $bddb->prepare( "SELECT umeta_id FROM $bddb->usermeta WHERE user_id = %d", $id ) );
	foreach ( $meta as $mid )
		delete_metadata_by_mid( 'user', $mid );

	$bddb->delete( $bddb->users, array( 'ID' => $id ) );

	clean_user_cache( $user );

	/** This action is documented in bd-admin/includes/user.php */
...
###SITEURL###' );
	...
	bd_mail( $value, sprintf( __( '[%s] New Admin Email Address' ), bd_specialchars_decode( get_option( 'blogname' ) ) ), $content );
}
 ...
 * @since 1.0.0
 *
 * @global object $errors BD_Error object.
 * @global object $bddb   Blasdoise database object.
 */
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		...
		if ( $bddb->get_var( $bddb->prepare( "SELECT user_email FROM {$bddb->users} WHERE user_email=%s", $_POST['email'] ) ) ) {
...
###SITEURL###' );
		...
		bd_mail( $_POST['email'], sprintf( __( '[%s] New Email Address' ), bd_specialchars_decode( get_option( 'blogname' ) ) ), $content );
 ...
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the bd_users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	$bddb->update( $bddb->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );
...
function _access_denied_splash() {
	...
	if ( bd_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) )
		return;

	$blog_name = get_bloginfo( 'name' );

	if ( empty( $blogs ) )
		bd_die( sprintf( __( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ), 403 );
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( $flag === false ) // Blasdoise english
 ...
 * @since 1.0.0
 *
 * @global int $bd_db_version The version number of the database.
 *
...
function site_admin_notice() {
	...
	if ( !is_super_admin() )
		return false;
	if ( get_site_option( 'bdmu_upgrade_site' ) != $bd_db_version )
	...
	<?php if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ) ) ) : ?>
		<tr>
			<th scope="row" colspan="2" class="th-full">
				...
				$sign_up_url = apply_filters( 'bd_signup_location', network_site_url( 'bd-signup.php' ) );
				?>
