...
/** Blasdoise Administration Widgets API */
 ...
 * @return string
 */
function bd_widgets_access_body_class($classes) {
	return "$classes widgets_access ";
}

if ( 'on' == $widgets_access ) {
	add_filter( 'admin_body_class', 'bd_widgets_access_body_class' );
} else {
	bd_enqueue_script('admin-widgets');
...
if ( ! current_theme_supports( 'widgets' ) ) {
	bd_die( __( 'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change.' ) );
}
...
if ( empty( $sidebars_widgets ) )
	$sidebars_widgets = bd_get_widget_defaults();

foreach ( $sidebars_widgets as $sidebar_id => $widgets ) {
	if ( 'bd_inactive_widgets' == $sidebar_id )
		continue;

	if ( !isset( $bd_registered_sidebars[ $sidebar_id ] ) ) {
...
if ( isset($_POST['savewidget']) || isset($_POST['removewidget']) ) {
	...
	if ( isset($_POST['removewidget']) && $_POST['removewidget'] ) {
		...
		if ( !in_array($widget_id, $sidebar, true) ) {
			bd_redirect( admin_url('widgets.php?error=0') );
			exit;
		}
	...
	bd_redirect( admin_url('widgets.php?message=0') );
	exit;
}
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
	...
	if ( isset($_GET['addnew']) ) {
		...
		$keys = array_keys( $bd_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset($_GET['base']) && isset($_GET['num']) ) { // multi-widget
			// Copy minimal info from an existing instance of this widget to a new instance
			foreach ( $bd_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					$control_callback = $control['callback'];
					$multi_number = (int) $_GET['num'];
					$control['params'][0]['number'] = -1;
					$widget_id = $control['id'] = $control['id_base'] . '-' . $multi_number;
					$bd_registered_widget_controls[$control['id']] = $control;
					break;
				}
			}
		}
	...
	if ( isset($bd_registered_widget_controls[$widget_id]) && !isset($control) ) {
		$control = $bd_registered_widget_controls[$widget_id];
		$control_callback = $control['callback'];
	} elseif ( !isset($bd_registered_widget_controls[$widget_id]) && isset($bd_registered_widgets[$widget_id]) ) {
		$name = esc_html( strip_tags($bd_registered_widgets[$widget_id]['name']) );
	}
	...
	if ( !isset($sidebar) )
		$sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'bd_inactive_widgets';
...
<?php
	foreach ( $bd_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr($sbname) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'bd_inactive_widgets' == $sbname || 'orphaned_widgets' == substr( $sbname, 0, 16 ) ) {
			echo '&nbsp;';
		} else {
...
<?php	bd_nonce_field("save-delete-widget-$widget_id"); ?>
	<br class="clear" />
	</div>
	</form>
	</div>
...
foreach ( $bd_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( false !== strpos( $registered_sidebar['class'], 'inactive-sidebar' ) || 'orphaned_widgets' == substr( $sidebar, 0, 16 ) ) {
		...
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php bd_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				<div class="clear"></div>
			</div>
		</div>
		<?php
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	?>
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		<?php bd_list_widget_controls( $sidebar, $registered_sidebar['name'] ); // Show the control forms for each of the widgets in this sidebar ?>
	</div>
	<?php
...
<form method="post">
<?php bd_nonce_field( 'save-sidebar-widgets', '_bdnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
