/* global _bdCustomizeNavMenusSettings, bdNavMenu, console */
( function( api, bd, $ ) {
	'use strict';

	/**
	 * Set up bdNavMenu for drag and drop.
	 */
	bdNavMenu.originalInit = bdNavMenu.init;
	bdNavMenu.options.menuItemDepthPerLevel = 20;
	bdNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	bdNavMenu.options.targetTolerance       = 10;
	bdNavMenu.init = function() {
	...
	if ( 'undefined' !== typeof _bdCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _bdCustomizeNavMenusSettings );
	}
	...
	/**
	 * bd.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's BD_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @constructor
	...
	/**
	 * bd.customize.Menus.AvailableItemCollection
	 *
	...
	/**
	 * bd.customize.Menus.AvailableMenuItemsPanelView
	 *
	 ...
	 */
	api.Menus.AvailableMenuItemsPanelView = bd.Backbone.View.extend({
		...
		doSearch: function( page ) {
			var self = this, params,
				...
				itemTemplate = bd.template( 'available-menu-item' );
			...
			if ( page < 0 ) {
				...
				bd.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = bd.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					bd.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					bd.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( type, object ) {
			...
			request = bd.ajax.post( 'load-available-menu-items-customizer', params );
	...
	/**
	 * bd.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the BD_Customize_Menu_Panel::$type.
	 *
	 * @constructor
	 * @augments bd.customize.Panel
	 */
	...
	/**
	 * bd.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the BD_Customize_Menu_Section::$type.
	 *
	 * @constructor
	 * @augments bd.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend({
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				bdNavMenu.menuList = section.container.find( '.accordion-section-content:first' );
				bdNavMenu.targetList = bdNavMenu.menuList;

				// Add attributes needed by bdNavMenu
				$( '#menu-to-edit' ).removeAttr( 'id' );
				bdNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				if ( 'resolved' !== section.deferred.initSortables.state() ) {
					bdNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
					section.deferred.initSortables.resolve( bdNavMenu.menuList ); // Now MenuControl can extend the sortable.

					// @todo Note that bd.customize.reflowPaneContents() is debounced, so this immediate change will show a slight flicker while priorities get updated.
	...
	/**
	 * bd.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 * Note that 'new_menu' must match the BD_Customize_New_Menu_Section::$type.
	 *
	 * @constructor
	 * @augments bd.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend({
		...
		onChangeExpanded: function( expanded ) {
			var section = this,
				...
				customizer = section.container.closest( '.bd-full-overlay-sidebar-content' );
			if ( expanded ) {
	...
	/**
	 * bd.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the BD_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @constructor
	 * @augments bd.customize.Control
	 */
	...
	/**
	 * bd.customize.Menus.MenuItemControl
	 *
	 * Customizer control for menu items.
	 * Note that 'menu_item' must match the BD_Customize_Menu_Item_Control::$type.
	 *
	 * @constructor
	 * @augments bd.customize.Control
	 */
	api.Menus.MenuItemControl = api.Control.extend({
		...
		_setupReorderUI: function() {
			...
			template = bd.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					bd.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {bd.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			bd.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			bd.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			bd.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			bd.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * bd.customize.Menus.MenuNameControl
	 *
	...
	/**
	 * bd.customize.Menus.MenuAutoAddControl
	 *
	...
	/**
	 * bd.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the BD_Menu_Customize_Control::$type
	 *
	 * @constructor
	 * @augments bd.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend({
		 ...
		 *
		 * Notice that the UI aspects here are handled by bdNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that bdNavMenu will be setting this .menu-item-data-parent-id input's value.
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						bd.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				bd.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				bd.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {bd.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @returns {bd.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			bd.a11y.speak( api.Menus.data.l10n.itemAdded );
	...
	/**
	 * bd.customize.Menus.NewMenuControl
	 *
	 * Customizer control for creating new menus and handling deletion of existing menus.
	 * Note that 'new_menu' must match the BD_Customize_New_Menu_Control::$type.
	 *
	 * @constructor
	 * @augments bd.customize.Control
	 */
	api.Menus.NewMenuControl = api.Control.extend({
		...
		submit: function() {
			...
			bd.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * Extends bd.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends bd.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends bd.customize.sectionConstructor with section constructor for menu.
	 */
	...
	api.Menus.applySavedData = function( data ) {
		...
		_( data.nav_menu_updates ).each(function( update ) {
			...
			if ( 'inserted' === update.status ) {
				...
				bd.customize.control.each(function( control ){
					if ( /^nav_menu_locations\[/.test( control.id ) ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in BD_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @param menuId
	 * @return {bd.customize.controlConstructor.menus[]}
	 */
	...
	function displayNavMenuName( name ) {
		name = name || '';
		name = $( '<div>' ).text( name ).html(); // Emulate esc_html() which is used in bd-admin/nav-menus.php.
		name = $.trim( name );
		return name || api.Menus.data.l10n.unnamed;
	}

})( bd.customize, bd, jQuery );
