 ...
 * @access private
 */
class BD_Media_List_Table extends BD_List_Table {
	...
	public function prepare_items() {
		global $bd_query, $post_mime_types, $avail_post_mime_types, $mode;

		list( $post_mime_types, $avail_post_mime_types ) = bd_edit_attachments_query( $_REQUEST );
		...
		$this->set_pagination_args( array(
			'total_items' => $bd_query->found_posts,
			'total_pages' => $bd_query->max_num_pages,
			'per_page' => $bd_query->query_vars['posts_per_page'],
		) );
	}
	...
	protected function get_views() {
		global $bddb, $post_mime_types, $avail_post_mime_types;

		$type_links = array();
		$_num_posts = (array) bd_count_attachments();
		$_total_posts = array_sum($_num_posts) - $_num_posts['trash'];
		$total_orphans = $bddb->get_var( "SELECT COUNT( * ) FROM $bddb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent < 1" );
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			$selected = '';
			if ( !empty( $_GET['attachment-filter'] ) && strpos( $_GET['attachment-filter'], 'post_mime_type:' ) === 0 && bd_match_mime_types( $mime_type, str_replace( 'post_mime_type:', '', $_GET['attachment-filter'] ) ) )
...
<?php
		if ( ! is_singular() ) {
			...
			/** This action is documented in bd-admin/includes/class-bd-posts-list-table.php */
...
<?php
		...
		/** This filter is documented in bd-admin/inclues/class-bd-list-table.php */
		$views = apply_filters( "views_{$this->screen->id}", array() );

		// Back compat view links.
		if ( ! empty( $views ) ) {
...
?>
	...
	public function get_columns() {
		...
		$taxonomies = bd_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	public function column_title( $post ) {
		...
		$thumb = bd_get_attachment_image( $post->ID, array( 60, 60 ), true, array( 'alt' => '' ) );
		$link_start = $link_end = '';
		...
		<p class="filename"><span class="screen-reader-text"><?php _e( 'File name:' ); ?> </span><?php echo bd_basename( $post->guid ); ?></p>
		<?php
	}
...
?>
			...
			if ( $user_can_edit ):
				$detach_url = add_query_arg( array(
					...
					'media[]' => $post->ID,
					'_bdnonce' => bd_create_nonce( 'bulk-' . $this->_args['plural'] )
				), 'upload.php' ); ?>
	...
	private function _get_row_actions( $post, $att_title ) {
		...
		if ( $this->detached ) {
			...
			if ( current_user_can( 'delete_post', $post->ID ) )
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = "<a class='submitdelete' href='" . bd_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-post_' . $post->ID ) . "'>" . __( 'Trash' ) . "</a>";
				} else {
					$delete_ays = !MEDIA_TRASH ? " onclick='return showNotice.warn();'" : '';
					$actions['delete'] = "<a class='submitdelete'$delete_ays href='" . bd_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-post_' . $post->ID ) . "'>" . __( 'Delete Permanently' ) . "</a>";
				}
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash )
					$actions['untrash'] = "<a class='submitdelete' href='" . bd_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-post_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
		 ...
		 * @param BD_Post $post     BD_Post object for the current attachment.
		 * @param bool    $detached Whether the list table contains media not attached
		 *                          to any posts. Default true.
		 */
