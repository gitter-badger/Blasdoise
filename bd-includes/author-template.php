 ...
 *
 * These functions must be used within the Blasdoise Loop.
 *
 * @package Blasdoise
 * @subpackage Template
 */
...
function the_author( $deprecated = '', $deprecated_echo = true ) {
	...
	if ( $deprecated_echo !== true )
		_deprecated_argument( __FUNCTION__, '1.0', __('Use <code>get_the_author()</code> instead if you do not want the value echoed.') );
	if ( $deprecated_echo )
		echo get_the_author();
...
function get_author_posts_url($author_id, $author_nicename = '') {
	...
	$link = $bd_rewrite->get_author_permastruct();
...
function bd_list_authors( $args = '' ) {
	...
	$query_args = bd_array_slice_assoc( $args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	$query_args['fields'] = 'ids';
	$authors = get_users( $query_args );

	$author_count = array();
	foreach ( (array) $bddb->get_results( "SELECT DISTINCT post_author, COUNT(ID) AS count FROM $bddb->posts WHERE " . get_private_posts_cap_sql( 'post' ) . " GROUP BY post_author" ) as $row ) {
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = get_transient( 'is_multi_author' ) ) ) {
		$rows = (array) $bddb->get_col("SELECT DISTINCT post_author FROM $bddb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
