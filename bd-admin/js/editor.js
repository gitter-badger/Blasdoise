...
( function( $ ) {
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'bd-switch-editor' ) ) {
						id = target.attr( 'data-bd-editor-id' );
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<bd-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<bd-line-break>/g, '\n' );
			}

			// and the <br> tags in captions
			if ( preserve_br ) {
				html = html.replace( /<bd-temp-br([^>]*)>/g, '<br$1>' );
			}

			return html;
		}

		// Similar to `bdautop()` in formatting.php
		function autop( text ) {
			...
			// put back the line breaks in pre|script
			if ( preserve_linebreaks ) {
				text = text.replace( /<bd-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				text = text.replace( /<bd-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		window.bd.editor.autop = bdautop;
		window.bd.editor.removep = pre_bdautop;

		exports = {
			go: switchEditor,
			bdautop: bdautop,
			pre_bdautop: pre_bdautop,
			_bd_Autop: autop,
			_bd_Nop: removep
		};
