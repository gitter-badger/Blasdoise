...
(function($) {
	...
	function generatePassword() {
		if ( typeof zxcvbn !== 'function' ) {
			...
			$pass1.trigger( 'pwupdate' ).trigger( 'bd-check-valid-field' );
	...
	function bindToggleButton() {
		$toggleButton = $pass1Row.find('.bd-hide-pw');
		$toggleButton.show().on( 'click', function () {
			if ( 1 === parseInt( $toggleButton.data( 'toggle' ), 10 ) ) {
				...
				$toggleButton
					...
					.end()
					.find( '.basicons' )
						.removeClass('basicons-visibility')
						.addClass('basicons-hidden');
				...
				$toggleButton
					...
					.end()
					.find( '.basicons' )
						.removeClass('basicons-hidden')
						.addClass('basicons-visibility');
	...
	function bindPasswordForm() {
		...
		$passwordWrapper = $pass1Row.find('.bd-pwd').hide();

		bindToggleButton();

		$generateButton = $pass1Row.find( 'button.bd-generate-pw' ).show();
		...
		$cancelButton = $pass1Row.find( 'button.bd-cancel-pw' );
	...
	function check_pass_strength() {
		...
		strength = bd.passwordStrength.meter( pass1, bd.passwordStrength.userInputBlacklist(), pass1 );
	...
	$(document).ready( function() {
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// repaint icons
				if ( typeof bd !== 'undefined' && bd.svgPainter ) {
					try {
						colors = $.parseJSON( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						bd.svgPainter.setColors( colors );
						bd.svgPainter.paint();
					}
				}
