...
if ( typeof(jQuery) != 'undefined' ) {
	if ( typeof(jQuery.fn.hoverIntent) == 'undefined' ) {
		/* jshint ignore:start */
		// hoverIntent v1.8.1 - Copy of bd-includes/js/hoverIntent.min.js
	...
	jQuery(document).ready(function($){
		var adminbar = $('#bdadminbar'), refresh, touchOpen, touchClose, disableHoverIntent = false;
		...
		touchOpen = function(unbind) {
			adminbar.find('li.menupop').on('click.bd-mobile-hover', function(e) {
				var el = $(this);

				if ( el.parent().is('#bd-admin-bar-root-default') && !el.hasClass('hover') ) {
					e.preventDefault();
				...
				if ( unbind ) {
					$('li.menupop').off('click.bd-mobile-hover');
		...
		touchClose = function() {
			...
			$(document.body).on( mobileEvent+'.bd-mobile-hover', function(e) {
				if ( !$(e.target).closest('#bdadminbar').length )
					adminbar.find('li.menupop.hover').removeClass('hover');
			});
		};
		...
		$('#bd-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#bd-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});

		$('#bdadminbar li.menupop > .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 13 )
				return;
		...
		$('#bdadminbar .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 27 )
				return;
		...
		adminbar.click( function(e) {
			if ( e.target.id != 'bdadminbar' && e.target.id != 'bd-admin-bar-top-secondary' ) {
				return;
			}
		...
		if ( 'sessionStorage' in window ) {
			$('#bd-admin-bar-logout a').click( function() {
				try {
					for ( var key in sessionStorage ) {
						if ( key.indexOf('bd-autosave-') != -1 )
		...
		if ( navigator.userAgent && document.body.className.indexOf( 'no-font-face' ) === -1 &&
			/Android (1.0|1.1|1.5|1.6|2.0|2.1)|Nokia|Opera Mini|w(eb)?OSBrowser|webOS|UCWEB|Windows Phone OS 7|XBLBD7|ZuneBD7|MSIE 7/.test( navigator.userAgent ) ) {
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'bd-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			// Ensure that the #bdadminbar was the target of the click.
			if ( t.id != 'bdadminbar' && t.id != 'bd-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			aB = d.getElementById('bdadminbar');
			...
			if ( d.body && aB ) {
				...
				addEvent( document.getElementById('bd-admin-bar-logout'), 'click', function() {
					if ( 'sessionStorage' in window ) {
						try {
							for ( var key in sessionStorage ) {
								if ( key.indexOf('bd-autosave-') != -1 )
