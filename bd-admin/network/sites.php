...
$bd_list_table = _get_list_table( 'BD_MS_Sites_List_Table' );
...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'confirm' === $_GET['action'] ) {
		...
		if ( ! array_key_exists( $site_action, $manage_actions ) ) {
			bd_die( __( 'The requested action is not valid.' ) );
		}
		...
		if ( $current_site->blog_id == $id ) {
			bd_die( __( 'You are not allowed to change the current site.' ) );
		}
		...
		?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr( $site_action ); ?>" method="post">
					...
					<input type="hidden" name="_bd_http_referer" value="<?php echo esc_attr( bd_get_referer() ); ?>" />
					<?php bd_nonce_field( $site_action . '_' . $id, '_bdnonce', false ); ?>
	...
	switch ( $_GET['action'] ) {
		...
		case 'deleteblog':
			if ( ! current_user_can( 'delete_sites' ) )
				bd_die( __( 'You do not have permission to access this page.' ), '', array( 'response' => 403 ) );

			$updated_action = 'not_deleted';
			if ( $id != '0' && $id != $current_site->blog_id && current_user_can( 'delete_site', $id ) ) {
				bdmu_delete_blog( $id, true );
						...
						switch ( $doaction ) {
							case 'delete':
								if ( ! current_user_can( 'delete_site', $val ) )
									bd_die( __( 'You are not allowed to delete the site.' ) );

								$updated_action = 'all_delete';
								bdmu_delete_blog( $val, true );
							break;
	...
	if ( ! empty( $updated_action ) ) {
		bd_safe_redirect( add_query_arg( array( 'updated' => $updated_action ), bd_get_referer() ) );
		exit();
	}
}
...
<?php $bd_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>
