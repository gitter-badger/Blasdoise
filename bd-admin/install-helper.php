 ...
 * plugin installation. This file is not included by Blasdoise and it is
 ...
 *
 *     check_column( 'bd_links', 'link_description', 'mediumtext' );
 *     if ( check_column( $bddb->comments, 'comment_author', 'tinytext' ) ) {
 *         echo "ok\n";
 *     }
 *
 *     $error_count = 0;
 *     $tablename = $bddb->links;
 *     // Check the column.
 *     if ( ! check_column($bddb->links, 'link_description', 'varchar( 255 )' ) ) {
 *         $ddl = "ALTER TABLE $bddb->links MODIFY COLUMN link_description varchar(255) NOT NULL DEFAULT '' ";
 *         $q = $bddb->query( $ddl );
 *     }
 *
 *     if ( check_column( $bddb->links, 'link_description', 'varchar( 255 )' ) ) {
...
function maybe_create_table($table_name, $create_ddl) {
	global $bddb;
	foreach ($bddb->get_col("SHOW TABLES",0) as $table ) {
		if ($table == $table_name) {
			return true;
		}
	}
...
function check_column($table_name, $col_name, $col_type, $is_null = null, $key = null, $default = null, $extra = null) {
	global $bddb;
	$diffs = 0;
	$results = $bddb->get_results("DESC $table_name");
