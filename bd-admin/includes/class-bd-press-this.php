...
class BD_Press_This {
	...
	public function side_load_images( $post_id, $content = '' ) {
		...
		if ( preg_match_all( '/<img [^>]+>/', $content, $matches ) && current_user_can( 'upload_files' ) ) {
			foreach ( (array) $matches[0] as $image ) {
				...
				// Don't try to sideload a file without a file extension, leads to BD upload error.
	...
	public function save_post() {
		if ( empty( $_POST['post_ID'] ) || ! $post_id = (int) $_POST['post_ID'] ) {
			bd_send_json_error( array( 'errorMessage' => __( 'Missing post ID.' ) ) );
		}

		if ( empty( $_POST['_bdnonce'] ) || ! bd_verify_nonce( $_POST['_bdnonce'], 'update-post_' . $post_id ) ||
			! current_user_can( 'edit_post', $post_id ) ) {

			bd_send_json_error( array( 'errorMessage' => __( 'Invalid post.' ) ) );
		}
		...
		$updated = bd_update_post( $post, true );

		if ( is_bd_error( $updated ) ) {
			bd_send_json_error( array( 'errorMessage' => $updated->get_error_message() ) );
		} else {
			...
			if ( $redirect ) {
				bd_send_json_success( array( 'redirect' => $redirect, 'force' => $forceRedirect ) );
			} else {
				bd_send_json_success( array( 'postSaved' => true ) );
			}
		}
	}
	...
	public function add_category() {
		if ( false === bd_verify_nonce( $_POST['new_cat_nonce'], 'add-category' ) ) {
			bd_send_json_error();
		}
		...
		foreach ( $names as $cat_name ) {
			...
			if ( term_exists( $cat_name, $taxonomy->name, $parent ) ) {
				if ( count( $names ) === 1 ) {
					bd_send_json_error( array( 'errorMessage' => __( 'This category already exists.' ) ) );
				} else {
					continue;
				}
			}
		...
		if ( empty( $added ) ) {
			bd_send_json_error( array( 'errorMessage' => __( 'This category cannot be added. Please change the name and try again.' ) ) );
		}

		foreach ( $added as $new_cat_id ) {
			$new_cat = get_category( $new_cat_id );

			if ( is_bd_error( $new_cat ) ) {
				bd_send_json_error( array( 'errorMessage' => __( 'Error while adding the category. Please try again later.' ) ) );
			}
	...
	public function fetch_source_html( $url ) {
		...
		if ( empty( $url ) ) {
			return new BD_Error( 'invalid-url', __( 'A valid URL was not provided.' ) );
		}

		$remote_url = bd_safe_remote_get( $url, array(
			'timeout' => 30,
			// Use an explicit user-agent for Press This
			'user-agent' => 'Press This (Blasdoise/' . $bd_version . '); ' . get_bloginfo( 'url' )
		) );

		if ( is_bd_error( $remote_url ) ) {
			return $remote_url;
		}
		...
		$source_content = bd_remote_retrieve_body( $remote_url );
		$source_content = bd_kses( $source_content, $useful_html_elements );
	 ...
	 *
	 * Excluded URLs include share-this type buttons, loaders, spinners, spacers, BD interface images,
	 * tiny buttons or thumbs, mathtag.com or quantserve.com images, or the BD stats gif.
	 *
	 * @ignore
	 * @since 1.0.0
	 *
	...
	private function _limit_img( $src ) {
		...
		if ( preg_match( '/\/ad[sx]{1}?\//', $src ) ) {
			...
			return '';
		} else if ( preg_match( '/\/bd-includes\//', $src ) ) {
			// Classic BD interface images
			return '';
	...
	public function merge_or_fetch_data() {
		...
		foreach ( array( 'u', 's', 't', 'v' ) as $key ) {
			if ( ! empty( $_POST[ $key ] ) ) {
				$value = bd_unslash( $_POST[ $key ] );
			} else if ( ! empty( $_GET[ $key ] ) ) {
				$value = bd_unslash( $_GET[ $key ] );
			} else {
				continue;
			}
		...
		if ( apply_filters( 'enable_press_this_media_discovery', true ) ) {
			...
			if ( empty( $_POST ) && ! empty( $data['u'] ) ) {
				...
				foreach ( array( '_images', '_embeds' ) as $type ) {
					...
					foreach ( $items as $key => $value ) {
						if ( $type === '_images' ) {
							$value = $this->_limit_img( bd_unslash( $value ) );
						} else {
							$value = $this->_limit_embed( bd_unslash( $value ) );
						}
				...
				foreach ( array( '_meta', '_links' ) as $type ) {
					...
					foreach ( $items as $key => $value ) {
						...
						if ( $type === '_meta' ) {
							$value = $this->_limit_string( bd_unslash( $value ) );
							...
							if ( in_array( $key, array( 'canonical', 'shortlink', 'icon' ), true ) ) {
								$data[ $type ][ $key ] = $this->_limit_url( bd_unslash( $value ) );
							}
						}
					}
				}
			...
			if ( ! empty( $_REQUEST['i'] ) && ( $img_src = $this->_limit_img( bd_unslash( $_REQUEST['i'] ) ) ) ) {
				if ( empty( $data['_images'] ) ) {
	...
	public function add_editor_style( $styles ) {
		...
		if ( 'off' !== _x( 'on', 'Raleway font: on or off' ) ) {
			...
			$query_args = array(
				'family' => urlencode( 'Raleway:400italic,700italic,400,600,700' ),
				'subset' => urlencode( $subsets ),
			);

			$raleway_font_url = ',' . add_query_arg( $query_args, 'https://fonts.googleapis.com/css' );
		}

		return $styles . $press_this . $raleway_font_url;
	}
	...
	public function categories_html( $post ) {
		...
		if ( current_user_can( $taxonomy->cap->edit_terms ) ) {
			...
			<button type="button" class="add-cat-toggle button-subtle" aria-expanded="false">
				<span class="basicons basicons-plus"></span><span class="screen-reader-text"><?php _e( 'Toggle add category' ); ?></span>
			</button>
		...
		<div class="categories-search-wrapper">
			...
			<label for="categories-search">
				<span class="basicons basicons-search"></span><span class="screen-reader-text"><?php _e( 'Search categories' ); ?></span>
			</label>
		</div>
		<div aria-label="<?php esc_attr_e( 'Categories' ); ?>">
			<ul class="categories-select">
				<?php bd_terms_checklist( $post->ID, array( 'taxonomy' => 'category', 'list_only' => true ) ); ?>
			</ul>
		</div>
		<?php
	}
	...
	public function tags_html( $post ) {
		...
		if ( ! $esc_tags || is_bd_error( $esc_tags ) ) {
			$esc_tags = '';
		}
...
<head>
	...
	<script>
		window.bdPressThisData   = <?php echo bd_json_encode( $site_data ); ?>;
		window.bdPressThisConfig = <?php echo bd_json_encode( $site_settings ); ?>;
	</script>
	...
	<?php
		...
		bd_enqueue_media( array( 'post' => $post_ID ) );
		bd_enqueue_style( 'press-this' );
		bd_enqueue_script( 'press-this' );
		bd_enqueue_script( 'json2' );
...
?>
<body class="bd-admin bd-core-ui <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
	...
	<form id="pressthis-form" method="post" action="post.php" autocomplete="off">
		...
		bd_nonce_field( 'update-post_' . $post_ID, '_bdnonce', false );
		bd_nonce_field( 'add-category', '_ajax_nonce-add-category', false );
	...
	<div class="wrapper">
		<div class="editor-wrapper">
			<div class="alerts" role="alert" aria-live="assertive" aria-relevant="all" aria-atomic="true">
				...
				if ( isset( $data['v'] ) && $this->version > $data['v'] ) {
					...
					<p class="alert is-notice">
						<?php printf( __( 'You should upgrade <a href="%s" target="_blank">your bookmarklet</a> to the latest version!' ), admin_url( 'options-general.php' ) ); ?>
					</p>
					<?php
				}
			...
			<div id="app-container" class="editor">
				...
				<?php
				bd_editor( $post_content, 'pressthis', array(
					...
					'tinymce'          => array(
						'resize'                => false,
						'blasdoise_adv_hidden'  => false,
						'add_unload_trigger'    => false,
						'statusbar'             => false,
						'autoresize_min_height' => 600,
						'bd_autoresize_on'      => true,
						'plugins'               => 'lists,media,paste,tabfocus,fullscreen,blasdoise,bdautoresize,bdeditimage,bdgallery,bdlink,bdtextpattern,bdview',
			...
			<div class="post-options">
				...
				<?php if ( $supports_formats ) : ?>
					<button type="button" class="button-reset post-option">
						<span class="basicons basicons-admin-post"></span>
						...
						<span class="basicons post-option-forward"></span>
					</button>
				<?php endif; ?>

				<button type="button" class="button-reset post-option">
					<span class="basicons basicons-category"></span>
					<span class="post-option-title"><?php _e( 'Categories' ); ?></span>
					<span class="basicons post-option-forward"></span>
				</button>

				<button type="button" class="button-reset post-option">
					<span class="basicons basicons-tag"></span>
			...
			<?php if ( $supports_formats ) : ?>
				<div class="setting-modal is-off-screen is-hidden">
					<button type="button" class="button-reset modal-close">
						<span class="basicons post-option-back"></span>
	...
	<div class="press-this-actions">
		<div class="pressthis-media-buttons">
			<button type="button" class="insert-media button-subtle" data-editor="pressthis">
				<span class="basicons basicons-admin-media"></span>
			...
			<div class="split-button">
				<div class="split-button-head">
					<button type="button" class="publish-button split-button-primary" aria-live="polite">
						...
						<i class="basicons basicons-arrow-down-alt2"></i>
 ...
 */
$GLOBALS['bd_press_this'] = new BD_Press_This;
