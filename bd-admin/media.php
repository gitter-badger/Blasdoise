...
case 'editattachment' :
	...
	if ( !current_user_can('edit_post', $attachment_id) )
		bd_die ( __('You are not allowed to edit this attachment.') );
...
case 'edit' :
	...
	if ( empty( $_GET['attachment_id'] ) ) {
		bd_redirect( admin_url('upload.php') );
		exit();
	}
	...
	if ( empty($att->ID) ) bd_die( __('You attempted to edit an attachment that doesn&#8217;t exist. Perhaps it was deleted?') );
	if ( 'attachment' !== $att->post_type ) bd_die( __('You attempted to edit an item that isn&#8217;t an attachment. Please go back and try again.') );
	if ( $att->post_status == 'trash' ) bd_die( __('You can&#8217;t edit this attachment because it is in the Trash. Please move it out of the Trash and try again.') );
...
<?php bd_original_referer_field(true, 'previous'); ?>
