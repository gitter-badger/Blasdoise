...
/**
 * Blasdoise XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional Blasdoise API for managing comments, pages, posts,
 * options, etc.
 *
 * As of Blasdoise 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the xmlrpc_enabled filter found in bd_xmlrpc_server::login().
 *
 * @package Blasdoise
 * @subpackage Publishing
 * @since 1.0.0
 */
class bd_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @access public
	...
	public function __construct() {
		$this->methods = array(
			// Blasdoise API
			'bd.getUsersBlogs'		=> 'this:bd_getUsersBlogs',
			'bd.newPost'			=> 'this:bd_newPost',
			'bd.editPost'			=> 'this:bd_editPost',
			'bd.deletePost'			=> 'this:bd_deletePost',
			'bd.getPost'			=> 'this:bd_getPost',
			'bd.getPosts'			=> 'this:bd_getPosts',
			'bd.newTerm'			=> 'this:bd_newTerm',
			'bd.editTerm'			=> 'this:bd_editTerm',
			'bd.deleteTerm'			=> 'this:bd_deleteTerm',
			'bd.getTerm'			=> 'this:bd_getTerm',
			'bd.getTerms'			=> 'this:bd_getTerms',
			'bd.getTaxonomy'		=> 'this:bd_getTaxonomy',
			'bd.getTaxonomies'		=> 'this:bd_getTaxonomies',
			'bd.getUser'			=> 'this:bd_getUser',
			'bd.getUsers'			=> 'this:bd_getUsers',
			'bd.getProfile'			=> 'this:bd_getProfile',
			'bd.editProfile'		=> 'this:bd_editProfile',
			'bd.getPage'			=> 'this:bd_getPage',
			'bd.getPages'			=> 'this:bd_getPages',
			'bd.newPage'			=> 'this:bd_newPage',
			'bd.deletePage'			=> 'this:bd_deletePage',
			'bd.editPage'			=> 'this:bd_editPage',
			'bd.getPageList'		=> 'this:bd_getPageList',
			'bd.getAuthors'			=> 'this:bd_getAuthors',
			'bd.getCategories'		=> 'this:mw_getCategories',		// Alias
			'bd.getTags'			=> 'this:bd_getTags',
			'bd.newCategory'		=> 'this:bd_newCategory',
			'bd.deleteCategory'		=> 'this:bd_deleteCategory',
			'bd.suggestCategories'	=> 'this:bd_suggestCategories',
			'bd.uploadFile'			=> 'this:mw_newMediaObject',	// Alias
			'bd.deleteFile'			=> 'this:bd_deletePost',		// Alias
			'bd.getCommentCount'	=> 'this:bd_getCommentCount',
			'bd.getPostStatusList'	=> 'this:bd_getPostStatusList',
			'bd.getPageStatusList'	=> 'this:bd_getPageStatusList',
			'bd.getPageTemplates'	=> 'this:bd_getPageTemplates',
			'bd.getOptions'			=> 'this:bd_getOptions',
			'bd.setOptions'			=> 'this:bd_setOptions',
			'bd.getComment'			=> 'this:bd_getComment',
			'bd.getComments'		=> 'this:bd_getComments',
			'bd.deleteComment'		=> 'this:bd_deleteComment',
			'bd.editComment'		=> 'this:bd_editComment',
			'bd.newComment'			=> 'this:bd_newComment',
			'bd.getCommentStatusList' => 'this:bd_getCommentStatusList',
			'bd.getMediaItem'		=> 'this:bd_getMediaItem',
			'bd.getMediaLibrary'	=> 'this:bd_getMediaLibrary',
			'bd.getPostFormats'     => 'this:bd_getPostFormats',
			'bd.getPostType'		=> 'this:bd_getPostType',
			'bd.getPostTypes'		=> 'this:bd_getPostTypes',
			'bd.getRevisions'		=> 'this:bd_getRevisions',
			'bd.restoreRevision'	=> 'this:bd_restoreRevision',
	 ...
	 * @return BD_User|bool BD_User object if authentication passed, false otherwise
	 */
	public function login( $username, $password ) {
		...
		if (is_bd_error($user)) {
			 ...
			 * @param BD_User $user  BD_User object.
			 */
			$this->error = apply_filters( 'xmlrpc_login_error', $this->error, $user );
			return false;
		}

		bd_set_current_user( $user->ID );
		return $user;
	}
	 ...
	 * @since 1.0.0
	 * @deprecated 1.0.0
	 * @deprecated use bd_xmlrpc_server::login
	 * @see bd_xmlrpc_server::login
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields($post_id, $fields) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset($meta['id']) ) {
				...
				if ( isset($meta['key']) ) {
					$meta['key'] = bd_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key )
						continue;
					$meta['value'] = bd_unslash( $meta['value'] );
					if ( current_user_can( 'edit_post_meta', $post_id, $meta['key'] ) )
						update_metadata_by_mid( 'post', $meta['id'], $meta['value'] );
				} elseif ( current_user_can( 'delete_post_meta', $post_id, $pmeta->meta_key ) ) {
					delete_metadata_by_mid( 'post', $meta['id'] );
				}
			} elseif ( current_user_can( 'add_post_meta', $post_id, bd_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function bd_getUsersBlogs( $args ) {
		// If this isn't on BDMU then just use blogger_getUsersBlogs
		if ( !is_multisite() ) {
		 ...
		 * equal to the method's name, e.g., bd.getUsersBlogs, bd.newPost, etc.
		 *
		 * @since 1.0.0
		 *
		 * @param string $name The method name.
		 */
		do_action( 'xmlrpc_call', 'bd.getUsersBlogs' );
	...
	/**
	 * Convert a Blasdoise date string to an IXR_Date object.
	 *
	 * @access protected
	 *
	...
	/**
	 * Convert a Blasdoise GMT date string to an IXR_Date object.
	 *
	 * @access protected
	 *
	 * @param string $date_gmt Blasdoise GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields ) ) {
			...
			$terms = bd_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms'] = array();
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => bd_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => bd_get_attachment_metadata( $media_item->ID ),
		);
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( bd_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'bd_slug'                => $page->post_name,
			'bd_password'            => $page->post_password,
			'bd_author'              => $author->display_name,
			'bd_page_parent_id'      => $page->post_parent,
			'bd_page_parent_title'   => $parent_title,
			'bd_page_order'          => $page->menu_order,
			'bd_author_id'           => (string) $author->ID,
			'bd_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'bd_page_template'       => $page_template
		);
		 ...
		 * @param BD_Post $page  Page object.
		 */
	 ...
	 * @access protected
	 *
	 * @param BD_User $user   The unprepared user object.
	...
	protected function _prepare_user( $user, $fields ) {
		 ...
		 * @param BD_User $user   User object.
	 ...
	 *         Content struct for adding a new post. See bd_insert_post() for information on
	 ...
	 */
	public function bd_newPost( $args ) {
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.newPost' );
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = bd_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for bd_newPost() and bd_editPost(), containing shared logic.
	 *
	 * @since 1.0.0
	 * @access protected
	 *
	 * @see bd_insert_post()
	 *
	 * @param BD_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = bd_parse_args( $content_struct, $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = bd_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_bd_insert_post_data', $post_data, $content_struct );

		$post_ID = $update ? bd_update_post( $post_data, true ) : bd_insert_post( $post_data, true );
	 ...
	 */
	public function bd_editPost( $args ) {
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.editPost' );
	 ...
	 */
	public function bd_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.deletePost' );
	 ...
	 *  - 'enclosure'
	 */
	public function bd_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'bd.getPost' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPost' );
	 ...
	 * @since 1.0.0
	 *
	 * @see bd_get_recent_posts()
	 * @see bd_getPost() for more on `$fields`
	 ...
	 */
	public function bd_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'bd.getPosts' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPosts' );
		...
		$posts_list = bd_get_recent_posts( $query );
	 ...
	 */
	public function bd_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.newTerm' );
		...
		$term = bd_insert_term( $term_data['name'] , $taxonomy['name'] , $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function bd_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.editTerm' );
		...
		$term = bd_update_term( $term_id , $taxonomy['name'] , $term_data );
	 ...
	 */
	public function bd_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.deleteTerm' );
		...
		$result = bd_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function bd_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getTerm' );
	 ...
	 */
	public function bd_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getTerms' );
	 ...
	 */
	public function bd_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'bd.getTaxonomy' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getTaxonomy' );
	 ...
	 */
	public function bd_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'bd.getTaxonomies' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getTaxonomies' );
	 ...
	 *  - 'roles'
	 */
	public function bd_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'bd.getUser' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getUser' );
	 ...
	 *
	 * @uses get_users()
	 * @see bd_getUser() for more on $fields and return values
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function bd_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'bd.getUsers' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getUsers' );
	 ...
	 *     @type array  $fields (optional)
	 * }
	 * @return array|IXR_Error (@see bd_getUser)
	 */
	public function bd_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'bd.getProfile' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getProfile' );
	 ...
	 */
	public function bd_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.editProfile' );
		...
		$result = bd_update_user( $user_data );
	 ...
	 */
	public function bd_getPage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getPages( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPages' );
	 ...
	 * @since 1.0.0
	 *
	 * @see bd_xmlrpc_server::mw_newPost()
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function bd_newPage( $args ) {
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.newPage' );
	 ...
	 */
	public function bd_deletePage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.deletePage' );
		 ...
		 */
		do_action( 'xmlrpc_call_success_bd_deletePage', $page_id, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_editPage( $args ) {
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.editPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getPageList( $args ) {
		global $bddb;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPageList' );

		// Get list of pages ids and titles
		$page_list = $bddb->get_results("
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getAuthors( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getAuthors' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getKeywords' );
	 ...
	 */
	public function bd_newCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.newCategory' );
		...
		$cat_id = bd_insert_category($new_category, true);
		 ...
		 */
		do_action( 'xmlrpc_call_success_bd_newCategory', $cat_id, $args );
	 ...
	 * }
	 * @return bool|IXR_Error See {@link bd_delete_term()} for return info.
	 */
	public function bd_deleteCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.deleteCategory' );

		if ( !current_user_can('manage_categories') )
			return new IXR_Error( 401, __( 'Sorry, you do not have the right to delete a category.' ) );

		$status = bd_delete_term( $category_id, 'category' );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_bd_deleteCategory', $category_id, $args );
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_suggestCategories( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.suggestCategories' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getComment($args) {
		$this->escape($args);
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getComment' );
	 ...
	 * - 'offset' - Default is 0. See {@link BD_Query::query()} for more.
	 *
	 * @since 1.0.0
	 *
	 * @param array  $args {
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of comments. See {@link bd_xmlrpc_server::bd_getComment()} for a description of each item contents
	 */
	public function bd_getComments( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getComments' );
	 ...
	 * See {@link bd_delete_comment()} for more information on
	 * this behavior.
	 *
	 * @since 1.0.0
	 *
	 ...
	 *     @type int    $comment_ID
	 * }
	 * @return bool|IXR_Error {@link bd_delete_comment()}
	 */
	public function bd_deleteComment($args) {
		$this->escape($args);
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.deleteComment' );

		$status = bd_delete_comment( $comment_ID );
		...
		if ( $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_bd_deleteComment', $comment_ID, $args );
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function bd_editComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.editComment' );
		...
		$result = bd_update_comment($comment);
		if ( is_bd_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		 ...
		 */
		do_action( 'xmlrpc_call_success_bd_editComment', $comment_ID, $args );
	 ...
	 *     @type array      $content_struct
	 * }
	 * @return int|IXR_Error {@link bd_new_comment()}
	 */
	public function bd_newComment($args) {
		$this->escape($args);
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.newComment' );

		$comment_ID = bd_new_comment( $comment );
		 ...
		 */
		do_action( 'xmlrpc_call_success_bd_newComment', $comment_ID, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getCommentStatusList($args) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getCommentStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getCommentCount( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getCommentCount' );

		$count = bd_count_comments( $post_id );
		return array(
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPostStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPageStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[$o_name]['option'], bd_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function bd_getMediaItem( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getMediaItem' );
	 ...
	 * - 'offset' - Default is 0. See BD_Query::query() for more.
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of media items. See bd_xmlrpc_server::bd_getMediaItem() for a description of each item contents
	 */
	public function bd_getMediaLibrary($args) {
		$this->escape($args);
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getMediaLibrary' );
	 ...
	 */
	public function bd_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPostFormats' );
	 ...
	 *  - 'supports'
	 */
	public function bd_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'bd.getPostType' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPostType' );
	 ...
	 * @return array|IXR_Error
	 */
	public function bd_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'bd.getPostTypes' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getPostTypes' );
	 ...
	 */
	public function bd_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_revision_fields', array( 'post_date', 'post_date_gmt' ), 'bd.getRevisions' );
		}
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.getRevisions' );
		...
		if ( ! bd_revisions_enabled( $post ) )
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );

		$revisions = bd_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves
			if ( bd_is_post_autosave( $revision ) )
				continue;
	 ...
	 */
	public function bd_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		/** This action is documented in bd-includes/class-bd-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'bd.restoreRevision' );

		if ( ! $revision = bd_get_post_revision( $revision_id ) )
			return new IXR_Error( 404, __( 'Invalid post ID.' ) );
		...
		$post = bd_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs($args) {
		...
		$rpc->query('bd.getUsersBlogs', $args[1], $args[2]);
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode(',', bd_get_post_categories($post_ID));

		$content  = '<title>'.bd_unslash($post_data['post_title']).'</title>';
		$content .= '<category>'.$categories.'</category>';
		$content .= bd_unslash($post_data['post_content']);
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = implode(',', bd_get_post_categories($entry['ID']));

			$content  = '<title>'.bd_unslash($entry['post_title']).'</title>';
			$content .= '<category>'.$categories.'</category>';
			$content .= bd_unslash($entry['post_content']);
	...
	public function blogger_newPost( $args ) {
		...
		$post_ID = bd_insert_post($post_data);
		if ( is_bd_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	public function blogger_editPost( $args ) {
		...
		$result = bd_update_post( $postdata );
	...
	public function mw_newPost($args) {
		...
		if ( !empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				$post_type = 'page';
				if ( !empty( $content_struct['bd_page_template'] ) )
					$page_template = $content_struct['bd_page_template'];
		...
		if ( isset( $content_struct['bd_post_format'] ) ) {
			$content_struct['bd_post_format'] = sanitize_key( $content_struct['bd_post_format'] );
			if ( !array_key_exists( $content_struct['bd_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format' ) );
			}
		}

		// Let Blasdoise generate the post_name (slug) unless
		// one has been provided.
		$post_name = "";
		if ( isset($content_struct['bd_slug']) )
			$post_name = $content_struct['bd_slug'];

		// Only use a password if one was given.
		if ( isset($content_struct['bd_password']) )
			$post_password = $content_struct['bd_password'];

		// Only set a post parent if one was provided.
		if ( isset($content_struct['bd_page_parent_id']) )
			$post_parent = $content_struct['bd_page_parent_id'];

		// Only set the menu_order if it was provided.
		if ( isset($content_struct['bd_page_order']) )
			$menu_order = $content_struct['bd_page_order'];

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset( $content_struct['bd_author_id'] ) && ( $user->ID != $content_struct['bd_author_id'] ) ) {
			switch ( $post_type ) {
				case "post":
			...
			$author = get_userdata( $content_struct['bd_author_id'] );
			if ( ! $author )
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			$post_author = $content_struct['bd_author_id'];
		}
		...
		if ( isset ( $content_struct['bd_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_ID, $content_struct['bd_post_thumbnail'] ) === false )
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );

			unset( $content_struct['bd_post_thumbnail'] );
		}
		...
		// in this function
		if ( isset( $content_struct['bd_post_format'] ) )
			set_post_format( $post_ID, $content_struct['bd_post_format'] );

		$post_ID = bd_insert_post( $postdata, true );
		if ( is_bd_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	public function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $bddb->get_results( "SELECT ID, guid FROM {$bddb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( ! empty( $file->guid ) && strpos( $post_content, $file->guid ) !== false )
					$bddb->update($bddb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID) );
			}
		}
	}
	...
	public function mw_editPost( $args ) {
		...
		// Use bd.editPost to edit post types other than post and page.
		...
		// Let Blasdoise manage slug if none was provided.
		...
		$page_template = null;
		if ( ! empty( $content_struct['bd_page_template'] ) && 'page' == $post_type )
			$page_template = $content_struct['bd_page_template'];

		$post_author = $postdata['post_author'];

		// Only set the post_author if one is set.
		if ( isset( $content_struct['bd_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['bd_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		$result = bd_update_post($newpost, true);
		if ( is_bd_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		...
		if ( isset ( $content_struct['bd_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['bd_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_ID );
			} else {
				if ( set_post_thumbnail( $post_ID, $content_struct['bd_post_thumbnail'] ) === false )
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
	...
	public function mw_getPost( $args ) {
		...
		if ($postdata['post_date'] != '') {
			...
			$categories = array();
			$catids = bd_get_post_categories($post_ID);
			foreach($catids as $catid)
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = bd_get_post_tags( $post_ID );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag )
					$tagnames[] = $tag->name;
			...
			$resp = array(
				...
				'bd_more_text' => $post['more_text'],
				'mt_allow_comments' => $allow_comments,
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'bd_slug' => $postdata['post_name'],
				'bd_password' => $postdata['post_password'],
				'bd_author_id' => (string) $author->ID,
				'bd_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				'post_status' => $postdata['post_status'],
				'custom_fields' => $this->get_custom_fields($post_ID),
				'bd_post_format' => $post_format,
				'sticky' => $sticky,
			...
			$resp['bd_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = array();
			$catids = bd_get_post_categories($entry['ID']);
			foreach( $catids as $catid )
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = bd_get_post_tags( $entry['ID'] );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'bd_slug' => $entry['post_name'],
				'bd_password' => $entry['post_password'],
				'bd_author_id' => (string) $author->ID,
				'bd_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				...
				'bd_post_thumbnail' => get_post_thumbnail_id( $entry['ID'] )
			);
		}
	...
	public function mw_newMediaObject( $args ) {
		...
		if ( !empty($data['overwrite']) && ($data['overwrite'] == true) ) {
			...
			bd_delete_attachment($old_file->ID);

			// Make sure the new name is different by pre-pending the
			// previous post id.
			$filename = preg_replace('/^bdid\d+-/', '', $name);
			$name = "bdid{$old_file->ID}-{$filename}";
		}

		$upload = bd_upload_bits($name, null, $bits);
		...
		// Save the data
		$id = bd_insert_attachment( $attachment, $upload[ 'file' ], $post_id );
		bd_update_attachment_metadata( $id, bd_generate_attachment_metadata( $id, $upload['file'] ) );
		...
		/** This filter is documented in bd-admin/includes/file.php */
		return apply_filters( 'bd_handle_upload', $struct, 'upload' );
	}
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$catids = bd_get_post_categories(intval($post_ID));
	...
	public function mt_setPostCategories( $args ) {
		...
		bd_set_post_categories($post_ID, $catids);
	...
	public function mt_getTrackbackPings( $post_ID ) {
		...
		$comments = $bddb->get_results( $bddb->prepare("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $bddb->comments WHERE comment_post_ID = %d", $post_ID) );
	...
	public function mt_publishPost( $args ) {
		...
		// retain old cats
		$cats = bd_get_post_categories($post_ID);
		$postdata['post_category'] = $cats;
		$this->escape($postdata);

		return bd_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		if ( $post_ID = url_to_postid($pagelinkedto) ) {
			...
			if ( intval($urltest['fragment']) ) {
				...
				$sql = $bddb->prepare("SELECT ID FROM $bddb->posts WHERE post_title RLIKE %s", $title );
				if (! ($post_ID = $bddb->get_var($sql)) ) {
		...
		if ( $bddb->get_results( $bddb->prepare("SELECT * FROM $bddb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom) ) )
		...
		/** This filter is documented in bd-includes/class-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'Blasdoise/' . $bd_version . '; ' . get_bloginfo( 'url' ) );
		...
		$request = bd_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$linea = bd_remote_retrieve_body( $request );
		...
		foreach ( $p as $para ) {
			if ( strpos($para, $pagelinkedto) !== false ) { // it exists, but is it a link?
				...
				$excerpt = preg_replace('|\</?bdcontext\>|', '', $para);

				// prevent really long link text
				if ( strlen($context[1]) > 100 )
					$context[1] = substr($context[1], 0, 100) . '&#8230;';

				$marker = '<bdcontext>'.$context[1].'</bdcontext>';    // set up our marker
				$excerpt= str_replace($context[0], $marker, $excerpt); // swap out the link for our marker
				$excerpt = strip_tags($excerpt, '<bdcontext>');        // strip all tags but our context marker
