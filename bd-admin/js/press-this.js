...
( function( $, window ) {
	var PressThis = function() {
		var editor, $mediaList, $mediaThumbWrap,
			...
			sidebarIsOpen         = false,
			settings              = window.bdPressThisConfig || {},
			data                  = window.bdPressThisData || {},
			smallestWidth         = 128,
			hasSetFocus           = false,
			catsCache             = [],
		...
		function insertSelectedMedia( $element ) {
			...
			src = checkUrl( $element.attr( 'data-bd-src' ) || '' );
		...
		function addImg( src, displaySrc, i ) {
			...
			$element.attr( 'data-bd-src', src ).css( 'background-image', 'url(' + displaySrc + ')' )
		...
		function renderDetectedMedia() {
			...
			if ( data._embeds ) {
				$.each( data._embeds, function ( i, src ) {
					...
					$element.attr( 'data-bd-src', src ).find( 'span' ).text( __( 'suggestedEmbedAlt' ).replace( '%d', i + 1 ) );
			...
			if ( data._images ) {
				$.each( data._images, function( i, src ) {
					...
					if ( src.indexOf( 'blasdoise.com/' ) > -1 ) {
	...
	window.bd = window.bd || {};
	window.bd.pressThis = new PressThis();
