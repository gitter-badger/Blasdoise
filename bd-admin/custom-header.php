...
class Custom_Image_Header {
	...
	public function __construct($admin_header_callback, $admin_image_div_callback = '') {
		...
		add_action( 'bd_ajax_custom-header-crop',   array( $this, 'ajax_header_crop'        ) );
		add_action( 'bd_ajax_custom-header-add',    array( $this, 'ajax_header_add'         ) );
		add_action( 'bd_ajax_custom-header-remove', array( $this, 'ajax_header_remove'      ) );
	}
	...
	public function help() {
		...
		get_current_screen()->add_help_tab( array(
			...
			'content' =>
				...
				'<p>' . __( 'If your theme has more than one default header image, or you have uploaded more than one custom header image, you have the option of having Blasdoise display a randomly different image on each page of your site. Click the &#8220;Random&#8221; radio button next to the Uploaded Images or Default Images section to enable this feature.') . '</p>' .
		...
		get_current_screen()->add_help_tab( array(
			...
			'content' =>
				...
				'<p>' . __( 'In the Header Text section of this page, you can choose whether to display this text or hide it. You can also choose a color for the text by clicking the Select Color button and either typing in a legitimate HTML hex value, e.g. &#8220;#333333000&#8221; for red, or by choosing a color using the color picker.' ) . '</p>' .
	...
	public function step() {
		...
		$step = (int) $_GET['step'];
		if ( $step < 1 || 3 < $step ||
			( 2 == $step && ! bd_verify_nonce( $_REQUEST['_bdnonce-custom-header-upload'], 'custom-header-upload' ) ) ||
			( 3 == $step && ! bd_verify_nonce( $_REQUEST['_bdnonce'], 'custom-header-crop-image' ) )
		)
			return 1;
	...
	public function js_includes() {
		...
		if ( ( 1 == $step || 3 == $step ) ) {
			bd_enqueue_media();
			bd_enqueue_script( 'custom-header' );
			if ( current_theme_supports( 'custom-header', 'header-text' ) )
				bd_enqueue_script( 'bd-color-picker' );
		} elseif ( 2 == $step ) {
			bd_enqueue_script('imgareaselect');
		}
	}
	...
	public function take_action() {
		...
		if ( isset( $_POST['resetheader'] ) ) {
			check_admin_referer( 'custom-header-options', '_bdnonce-custom-header-options' );
	...
	public function process_default_headers() {
		...
		$this->default_headers = $_bd_default_headers;
...
(function($){
	...
	$(document).ready(function() {
		...
		text_color.bdColorPicker({
			change: function( event, ui ) {
				pickColor( text_color.bdColorPicker('color') );
			},
			clear: function() {
				pickColor( '' );
			}
...
<td>
	...
	?></p>
	<form enctype="multipart/form-data" id="upload-form" class="bd-upload-form" method="post" action="<?php echo esc_url( add_query_arg( 'step', 2 ) ) ?>">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php bd_nonce_field( 'custom-header-upload', '_bdnonce-custom-header-upload' ); ?>
	...
	<?php
		$modal_update_href = esc_url( add_query_arg( array(
			'page' => 'custom-header',
			'step' => 2,
			'_bdnonce-custom-header-upload' => bd_create_nonce('custom-header-upload'),
...
bd_nonce_field( 'custom-header-options', '_bdnonce-custom-header-options' ); ?>
...
<?php }
	...
	public function step_2() {
		check_admin_referer('custom-header-upload', '_bdnonce-custom-header-upload');
		...
		if ( empty( $_POST ) && isset( $_GET['file'] ) ) {
			...
			$url = bd_get_attachment_image_src( $attachment_id, 'full' );
			$url = $url[0];
		} elseif ( isset( $_POST ) ) {
		...
		if ( file_exists( $file ) ) {
			...
			$data = bd_get_attachment_metadata( $attachment_id );
			...
			&& $width == get_theme_support( 'custom-header', 'width' ) && $height == get_theme_support( 'custom-header', 'height' ) )
		{
			 ...
			 */
			do_action( 'bd_create_file_in_uploads', $file, $attachment_id ); // For replication

			return $this->finished();
		} elseif ( $width > $max_width ) {
			$oitar = $width / $max_width;
			$image = bd_crop_image($attachment_id, 0, 0, $width, $height, $max_width, $height / $oitar, false, str_replace(basename($file), 'midsize-'.basename($file), $file));
			if ( ! $image || is_bd_error( $image ) )
				bd_die( __( 'Image could not be processed. Please go back and try again.' ), __( 'Image Processing Error' ) );

			/** This filter is documented in bd-admin/custom-header.php */
			$image = apply_filters( 'bd_create_file_in_uploads', $image, $attachment_id ); // For replication
...
<form method="post" action="<?php echo esc_url(add_query_arg('step', 3)); ?>">
	...
	<?php } ?>
	<?php bd_nonce_field( 'custom-header-crop-image' ) ?>
	...
	public function step_2_manage_upload() {
		...
		$bd_filetype = bd_check_filetype_and_ext( $uploaded_file['tmp_name'], $uploaded_file['name'] );
		if ( ! bd_match_mime_types( 'image', $bd_filetype['type'] ) )
			bd_die( __( 'The uploaded file is not a valid image. Please try again.' ) );

		$file = bd_handle_upload($uploaded_file, $overrides);

		if ( isset($file['error']) )
			bd_die( $file['error'],  __( 'Image Upload Error' ) );
	...
	public function step_3() {
		...
		if ( empty( $_POST['skip-cropping'] ) )
			$cropped = bd_crop_image( $attachment_id, (int) $_POST['x1'], (int) $_POST['y1'], (int) $_POST['width'], (int) $_POST['height'], $width, $height );
		...
		if ( file_exists( $medium ) ) {
			bd_delete_file( $medium );
		}

		if ( empty( $_POST['create-new-attachment'] ) && empty( $_POST['skip-cropping'] ) ) {
			bd_delete_file( $original );
		}
	...
	public function admin_page() {
		if ( ! current_user_can('edit_theme_options') )
			bd_die(__('You do not have permission to customize headers.'));
		$step = $this->step();
		if ( 2 == $step )
			$this->step_2();
		elseif ( 3 == $step )
	...
	final public function set_header_image( $choice ) {
		if ( is_array( $choice ) || is_object( $choice ) ) {
			...
			update_post_meta( $choice['attachment_id'], '_bd_attachment_is_custom_header', get_stylesheet() );
	...
	final public function insert_attachment( $object, $cropped ) {
		 ...
		 */
		$metadata = apply_filters( 'bd_header_image_attachment_metadata', $metadata );
	...
	public function ajax_header_crop() {
		...
		if ( ! $cropped || is_bd_error( $cropped ) ) {
			bd_send_json_error( array( 'message' => __( 'Image could not be processed. Please go back and try again.' ) ) );
		}
	...
	public function ajax_header_add() {
		...
		$key = '_bd_attachment_custom_header_last_used_' . get_stylesheet();
		update_post_meta( $attachment_id, $key, time() );
		update_post_meta( $attachment_id, '_bd_attachment_is_custom_header', get_stylesheet() );
	...
	public function ajax_header_remove() {
		...
		delete_post_meta( $attachment_id, '_bd_attachment_is_custom_header', get_stylesheet() );
	...
	/**
	 *
	 * @param BD_Customize_Manager $bd_customize
	 */
	public function customize_set_last_used( $bd_customize ) {
		$data = $bd_customize->get_setting( 'header_image_data' )->post_value();
	...
	public function get_uploaded_header_images() {
		...
		$timestamp_key = '_bd_attachment_custom_header_last_used_' . get_stylesheet();
		$alt_text_key = '_bd_attachment_image_alt';
