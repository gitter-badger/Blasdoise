 ...
 * @access private
 */
class BD_MS_Themes_List_Table extends BD_List_Table {
	...
	public function prepare_items() {
		...
		$themes = array(
			/**
			 * Filter the full array of BD_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 1.0.0
			 *
			 * @param array $all An array of BD_Theme objects to display in the list table.
			 */
			'all' => apply_filters( 'all_themes', bd_get_themes() ),
			'search' => array(),
			'enabled' => array(),
			'disabled' => array(),
			'upgrade' => array(),
			'broken' => $this->is_site_themes ? array() : bd_get_themes( array( 'errors' => true ) ),
		);
		...
		BD_Theme::sort_by_name( $this->items );
	 ...
	 * @access public
	 *
	 * @param BD_Theme $theme The current BD_Theme object.
	 */
	...
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				$actions['enable'] = '<a href="' . esc_url( bd_nonce_url($url . 'action=enable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'enable-theme_' . $stylesheet ) ) . '" title="' . esc_attr__('Enable this theme') . '" class="edit">' . ( $this->is_site_themes ? __( 'Enable' ) : __( 'Network Enable' ) ) . '</a>';
			}
		} else {
			$actions['disable'] = '<a href="' . esc_url( bd_nonce_url($url . 'action=disable&amp;theme=' . $theme_key . '&amp;paged=' . $page . '&amp;s=' . $s, 'disable-theme_' . $stylesheet ) ) . '" title="' . esc_attr__('Disable this theme') . '">' . ( $this->is_site_themes ? __( 'Disable' ) : __( 'Network Disable' ) ) . '</a>';
		}
		...
		if ( ! $allowed && current_user_can( 'delete_themes' ) && ! $this->is_site_themes && $stylesheet != get_option( 'stylesheet' ) && $stylesheet != get_option( 'template' ) ) {
			$actions['delete'] = '<a href="' . esc_url( bd_nonce_url( 'themes.php?action=delete-selected&amp;checked[]=' . $theme_key . '&amp;theme_status=' . $context . '&amp;paged=' . $page . '&amp;s=' . $s, 'bulk-themes' ) ) . '" title="' . esc_attr__( 'Delete this theme' ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
		/**
	...
	public function column_default( $theme, $column_name ) {
		 ...
		 * @param string   $stylesheet  Directory name of the theme.
		 * @param BD_Theme $theme       Current BD_Theme object.
		 */
	 ...
	 * @access public
	 *
	 * @param BD_Theme $item The current BD_Theme object.
	 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes )
			remove_action( "after_theme_row_$stylesheet", 'bd_theme_update_row' );
