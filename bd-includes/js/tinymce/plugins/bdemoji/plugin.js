( function( tinymce, bd ) {
	tinymce.PluginManager.add( 'bdemoji', function( editor ) {
		var typing,
			env = tinymce.Env,
		...
		if ( ! bd || ! bd.emoji || ! bd.emoji.replaceEmoji ) {
			return;
		}
		...
		function setImgAttr( image ) {
			...
			image.setAttribute( 'data-bd-emoji', '1' );
		}
		...
		function replaceEmoji( node ) {
			...
			bd.emoji.parse( node, { imgAttr: imgAttr } );
		}
		...
		editor.on( 'setcontent', function( event ) {
			...
			if ( window.twemoji && window.twemoji.test( node.textContent || node.innerText ) ) {
				...
				// In IE all content in the editor is left selected after bd.emoji.parse()...
		...
		editor.on( 'postprocess', function( event ) {
			if ( event.content ) {
				event.content = event.content.replace( /<img[^>]+data-bd-emoji="[^>]+>/g, function( img ) {
		...
		editor.on( 'resolvename', function( event ) {
			if ( event.target.nodeName === 'IMG' && editor.dom.getAttrib( event.target, 'data-bd-emoji' ) ) {
				event.preventDefault();
			}
		} );
	} );
} )( window.tinymce, window.bd );
