...
/** Blasdoise Translation Install API */
...
/* translators: date and time format for exact current time, mainly about timezones. */
...
<script type="text/javascript">
	jQuery(document).ready(function($){
		var $siteName = $( '#bd-admin-bar-site-name' ).children( 'a' ).first(),
			homeURL = ( <?php echo bd_json_encode( get_home_url() ); ?> || '' ).replace( /^(https?:\/\/)?(www\.)?/, '' );
...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __('The Blasdoise URL and the Site URL can be the same (example.com) or different; for example, having the Blasdoise core files (example.com/blasdoise) in a subdirectory instead of the root directory.') . '</p>' .
...
<?php if ( !is_multisite() ) { ?>
<tr>
<th scope="row"><label for="siteurl"><?php _e('Blasdoise Address (URL)') ?></label></th>
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'BD_SITEURL' ) ); ?> class="regular-text code<?php if ( defined( 'BD_SITEURL' ) ) echo ' disabled' ?>" /></td>
</tr>
<tr>
<th scope="row"><label for="home"><?php _e('Site Address (URL)') ?></label></th>
<td><input name="home" type="url" id="home" aria-describedby="home-description" value="<?php form_option( 'home' ); ?>"<?php disabled( defined( 'BD_HOME' ) ); ?> class="regular-text code<?php if ( defined( 'BD_HOME' ) ) echo ' disabled' ?>" />
<p class="description" id="home-description"><?php _e( 'Enter the address here if you want your site home page to be different from your Blasdoise installation directory.' ); ?></p></td>
</tr>
<tr>
...
<td>
<select name="default_role" id="default_role"><?php bd_dropdown_roles( get_option('default_role') ); ?></select>
</td>
</tr>
<?php } else { ?>
<tr>
...
<?php echo bd_timezone_choice($tzstring); ?>
</select>
...
for ($day_index = 0; $day_index <= 6; $day_index++) :
	...
	echo "\n\t<option value='" . esc_attr($day_index) . "' $selected>" . $bd_locale->get_weekday($day_index) . '</option>';
endfor;
?>
</select></td>
</tr>
...
if ( ! is_multisite() && defined( 'BDLANG' ) && '' !== BDLANG && 'en_US' !== BDLANG && ! in_array( BDLANG, $languages ) ) {
	$languages[] = BDLANG;
}
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th width="33%" scope="row"><label for="BDLANG"><?php _e( 'Site Language' ); ?></label></th>
		<td>
			<?php
			$locale = get_locale();
			...
			bd_dropdown_languages( array(
				'name'         => 'BDLANG',
				'id'           => 'BDLANG',
				'selected'     => $locale,
				'languages'    => $languages,
				'translations' => $translations,
				'show_available_translations' => ( ! is_multisite() || is_super_admin() ) && bd_can_install_language_pack(),
			) );

			// Add note about deprecated BDLANG constant.
			if ( defined( 'BDLANG' ) && ( '' !== BDLANG ) && $locale !== BDLANG ) {
				if ( is_super_admin() ) {
					?>
					<p class="description">
						<strong><?php _e( 'Note:' ); ?></strong> <?php printf( __( 'The %s constant in your %s file is no longer needed.' ), '<code>BDLANG</code>', '<code>bd-config.php</code>' ); ?>
					</p>
					<?php
				}
				_deprecated_argument( 'define()', '1.0', sprintf( __( 'The %s constant in your %s file is no longer needed.' ), 'BDLANG', 'bd-config.php' ) );
			}
			?>
		</td>
	</tr>
