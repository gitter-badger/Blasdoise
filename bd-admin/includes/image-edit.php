...
/**
 * Loads the BD image-editing interface.
 *
 * @param int         $post_id Post ID.
 * @param bool|object $msg     Optional. Message to display for image editor updates or errors.
 *                             Default false.
 */
function bd_image_editor($post_id, $msg = false) {
	$nonce = bd_create_nonce("image_editor-$post_id");
	...
	$backup_sizes = get_post_meta( $post_id, '_bd_attachment_backup_sizes', true );
	$can_restore = false;
	...
	<div class="imgedit-group-top">
		<h3><?php _e( 'Scale Image' ); ?> <a href="#" class="basicons basicons-editor-help imgedit-help-toggle" onclick="imageEdit.toggleHelp(this);return false;"></a></h3>
...
<?php if ( $can_restore ) { ?>
	...
	<div class="imgedit-group">
	<div class="imgedit-group-top">
		<h3><a onclick="imageEdit.toggleHelp(this);return false;" href="#"><?php _e('Restore Original Image'); ?> <span class="basicons basicons-arrow-down imgedit-help-toggle"></span></a></h3>
		<div class="imgedit-help">
...
<?php } ?>
	...
	<div class="imgedit-group">
	<div class="imgedit-group-top">
		<h3><?php _e('Image Crop'); ?> <a href="#" class="basicons basicons-editor-help imgedit-help-toggle" onclick="imageEdit.toggleHelp(this);return false;"></a></h3>
	...
	<?php if ( $thumb && $sub_sizes ) {
		$thumb_img = bd_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
	?>

	<div class="imgedit-group imgedit-applyto">
	<div class="imgedit-group-top">
		<h3><?php _e('Thumbnail Settings'); ?> <a href="#" class="basicons basicons-editor-help imgedit-help-toggle" onclick="imageEdit.toggleHelp(this);return false;"></a></h3>
	...
	<div class="imgedit-panel-content">
		...
		if ( bd_image_editor_supports( array( 'mime_type' => get_post_mime_type( $post_id ), 'methods' => array( 'rotate' ) ) ) ) { ?>
...
/**
 * Streams image in BD_Image_Editor to browser.
 * Provided for backcompat reasons
 *
 * @param BD_Image_Editor $image
 * @param string $mime_type
 * @param int $post_id
 * @return bool
 */
function bd_stream_image( $image, $mime_type, $post_id ) {
	if ( $image instanceof BD_Image_Editor ) {

		/**
		 * Filter the BD_Image_Editor instance for the image to be streamed to the browser.
		 *
		 * @since 1.0.0
		 *
		 * @param BD_Image_Editor $image   BD_Image_Editor instance.
		 * @param int             $post_id Post ID.
		 */
		$image = apply_filters( 'image_editor_save_pre', $image, $post_id );

		if ( is_bd_error( $image->stream( $mime_type ) ) )
			return false;

		return true;
	} else {
		_deprecated_argument( __FUNCTION__, '1.0', __( '$image needs to be an BD_Image_Editor object' ) );

		/**
		 * Filter the GD image resource to be streamed to the browser.
		 *
		 * @since 1.0.0
		 * @deprecated 1.0.0 Use image_editor_save_pre instead.
		 *
 ...
 * @return bool
 */
function bd_save_image_file( $filename, $image, $mime_type, $post_id ) {
	if ( $image instanceof BD_Image_Editor ) {

		/** This filter is documented in bd-admin/includes/image-edit.php */
		$image = apply_filters( 'image_editor_save_pre', $image, $post_id );
		 ...
		 * @param int             $post_id   Post ID.
		 */
		$saved = apply_filters( 'bd_save_image_editor_file', null, $filename, $image, $mime_type, $post_id );
		 ...
		 *
		 * @since 1.0.0
		 * @deprecated 1.0.0 Use bd_save_image_editor_file instead.
		 *
		 * @param mixed           $override  Value to return instead of saving. Default null.
		 * @param string          $filename  Name of the file to be saved.
		 * @param BD_Image_Editor $image     BD_Image_Editor instance.
		 ...
		 * @param int             $post_id   Post ID.
		 */
		$saved = apply_filters( 'bd_save_image_file', null, $filename, $image, $mime_type, $post_id );
		...
		switch ( $mime_type ) {
			case 'image/jpeg':
				...
				/** This filter is documented in bd-includes/class-bd-image-editor.php */
...
function _rotate_image_resource($img, $angle) {
	_deprecated_function( __FUNCTION__, '1.0', __( 'Use BD_Image_Editor::rotate' ) );
...
function _flip_image_resource($img, $horz, $vert) {
	_deprecated_function( __FUNCTION__, '1.0', __( 'Use BD_Image_Editor::flip' ) );
	$w = imagesx($img);
	$h = imagesy($img);
	$dst = bd_imagecreatetruecolor($w, $h);
	if ( is_resource($dst) ) {
		$sx = $vert ? ($w - 1) : 0;
		$sy = $horz ? ($h - 1) : 0;
		$sw = $vert ? -$w : $w;
 ...
 * @since 1.0.0
 *
 * @param BD_Image_Editor $image   {@see BD_Image_Editor} instance.
 * @param array           $changes Array of change operations.
 * @return BD_Image_Editor {@see BD_Image_Editor} instance with changes applied.
 */
function image_edit_apply_changes( $image, $changes ) {
	...
	if ( $image instanceof BD_Image_Editor ) {
		...
		/**
		 * Filter the BD_Image_Editor instance before applying changes to the image.
		 *
		 * @since 1.0.0
		 *
		 * @param BD_Image_Editor $image   BD_Image_Editor instance.
 		 * @param array           $changes Array of change operations.
		 */
		$image = apply_filters( 'bd_image_editor_before_change', $image, $changes );
		 ...
		 *
		 * @since 1.0.0
		 * @deprecated 1.0.0 Use bd_image_editor_before_change instead.
		 *
...
function stream_preview_image( $post_id ) {
	...
	$img = bd_get_image_editor( _load_image_to_edit_path( $post_id ) );

    if ( is_bd_error( $img ) )
        return false;

	$changes = !empty($_REQUEST['history']) ? json_decode( bd_unslash($_REQUEST['history']) ) : null;
	if ( $changes )
	...
	if ( is_bd_error( $img->resize( $w2, $h2 ) ) )
		return false;

	return bd_stream_image( $img, $post->post_mime_type, $post_id );
}
 ...
 */
function bd_restore_image($post_id) {
	$meta = bd_get_attachment_metadata($post_id);
	$file = get_attached_file($post_id);
	$backup_sizes = $old_backup_sizes = get_post_meta( $post_id, '_bd_attachment_backup_sizes', true );
	$restored = false;
	$msg = new stdClass;
	...
	if ( isset($backup_sizes['full-orig']) && is_array($backup_sizes['full-orig']) ) {
		...
		$meta['file'] = _bd_relative_upload_path( $restored_file );
	...
	if ( ! bd_update_attachment_metadata( $post_id, $meta ) ||
		( $old_backup_sizes !== $backup_sizes && ! update_post_meta( $post_id, '_bd_attachment_backup_sizes', $backup_sizes ) ) ) {
 ...
 * @return \stdClass
 */
function bd_save_image( $post_id ) {
	...
	$img = bd_get_image_editor( _load_image_to_edit_path( $post_id, 'full' ) );
	if ( is_bd_error( $img ) ) {
	...
	if ( $scale && $fwidth > 0 && $fheight > 0 ) {
		...
		$changes = json_decode( bd_unslash($_REQUEST['history']) );
		if ( $changes )
			$img = image_edit_apply_changes($img, $changes);
	} else {
	...
	if ( !bd_save_image_file($new_path, $img, $post->post_mime_type, $post_id) ) {
	...
	if ( 'nothumb' == $target || 'all' == $target || 'full' == $target || $scaled ) {
		...
		$meta['file'] = _bd_relative_upload_path( $new_path );
	...
	if ( isset( $sizes ) ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( isset( $_bd_additional_image_sizes[ $size ] ) ) {
				$width  = intval( $_bd_additional_image_sizes[ $size ]['width'] );
				$height = intval( $_bd_additional_image_sizes[ $size ]['height'] );
				$crop   = ( $nocrop ) ? false : $_bd_additional_image_sizes[ $size ]['crop'];
			} else {
	...
	if ( $success ) {
		bd_update_attachment_metadata( $post_id, $meta );
		update_post_meta( $post_id, '_bd_attachment_backup_sizes', $backup_sizes);

		if ( $target == 'thumbnail' || $target == 'all' || $target == 'full' ) {
			// Check if it's an image edit from attachment edit screen
			if ( ! empty( $_REQUEST['context'] ) && 'edit-attachment' == $_REQUEST['context'] ) {
				$thumb_url = bd_get_attachment_image_src( $post_id, array( 900, 600 ), true );
