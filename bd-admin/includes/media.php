<?php
/**
 * Blasdoise Administration Media API.
 *
 * @package Blasdoise
...
function media_upload_tabs() {
	...
	/**
	 * Filter the available tabs in the legacy media popup.
	 *
	 * @since 1.0.0
	 *
...
function update_gallery_tab($tabs) {
	...
	if ( $post_id )
		$attachments = intval( $bddb->get_var( $bddb->prepare( "SELECT count(*) FROM $bddb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent = %d", $post_id ) ) );
...
function the_media_upload_tabs() {
	...
	if ( !empty($tabs) ) {
		...
		if ( isset($redir_tab) && array_key_exists($redir_tab, $tabs) ) {
			...
			/** This filter is documented in bd-admin/media-upload.php */
...
function get_image_send_to_editor($id, $caption, $title, $align, $url='', $rel = false, $size='medium', $alt = '') {
	...
	$rel = $rel ? ' rel="attachment bd-att-' . esc_attr($id).'"' : '';
 ...
 * @param array  $overrides Override the {@link bd_handle_upload()} behavior. Optional.
 * @return int|BD_Error ID of the attachment or a BD_Error object on failure.
 */
function media_handle_upload($file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false )) {
	...
	$file = bd_handle_upload($_FILES[$file_id], $overrides, $time);

	if ( isset($file['error']) )
		return new BD_Error( 'upload_error', $file['error'] );
	...
	if ( preg_match( '#^audio#', $type ) ) {
		$meta = bd_read_audio_metadata( $file );
	...
	} elseif ( 0 === strpos( $type, 'image/' ) && $image_meta = @bd_read_image_metadata( $file ) ) {
	...
	// Save the data
	$id = bd_insert_attachment($attachment, $file, $post_id);
	if ( !is_bd_error($id) ) {
		bd_update_attachment_metadata( $id, bd_generate_attachment_metadata( $id, $file ) );
	}
 ...
 * @return int|object The ID of the attachment or a BD_Error on failure
 */
function media_handle_sideload($file_array, $post_id, $desc = null, $post_data = array()) {
	...
	$file = bd_handle_sideload( $file_array, $overrides, $time );
	if ( isset($file['error']) )
		return new BD_Error( 'upload_error', $file['error'] );
	...
	// Use image exif/iptc data for title and caption defaults if possible.
	if ( $image_meta = @bd_read_image_metadata($file) ) {
		if ( trim( $image_meta['title'] ) && ! is_numeric( sanitize_title( $image_meta['title'] ) ) )
			$title = $image_meta['title'];
		if ( trim( $image_meta['caption'] ) )
 ...
 */
function bd_iframe($content_func /* ... */) {
	_bd_admin_html_begin();
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php _e('Uploads'); ?> &#8212; <?php _e('Blasdoise'); ?></title>
<?php
...
<?php
	...
	/**
	 * Fires when admin styles enqueued for the legacy media upload popup are printed.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Fires when admin scripts enqueued for the legacy media upload popup are printed.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * Fires when scripts enqueued for the admin header for the legacy
...
<body<?php if ( isset($GLOBALS['body_id']) ) echo ' id="' . $GLOBALS['body_id'] . '"'; ?> class="bd-core-ui no-js">
<script type="text/javascript">
...
function media_buttons($editor_id = 'content') {
	...
	bd_enqueue_media( array(
		'post' => $post
	) );

	$img = '<span class="bd-media-buttons-icon"></span> ';
	...
	/**
	 * Filter the legacy media buttons.
	 *
	 * @since 1.0.0
	 * @deprecated 1.0.0 Use 'media_buttons' action instead.
	 *
	...
	if ( $legacy_filter ) {
		// #BD22559. Close <a> if a plugin started by closing <a> to open their own <a> tag.
 ...
 * @since 1.0.0
 *
 * @return mixed void|object BD_Error on failure
 */
function media_upload_form_handler() {
	...
	if ( !empty($_POST['attachments']) ) foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
		 ...
		 * @since 1.0.0
		 *
		 * @see bd_get_attachment_metadata()
		 *
		 * @param BD_Post $post       The BD_Post object.
		 * @param array   $attachment An array of attachment metadata.
		 */
		$post = apply_filters( 'attachment_fields_to_save', $post, $attachment );

		if ( isset($attachment['image_alt']) ) {
			$image_alt = bd_unslash( $attachment['image_alt'] );
			if ( $image_alt != get_post_meta($attachment_id, '_bd_attachment_image_alt', true) ) {
				$image_alt = bd_strip_all_tags( $image_alt, true );

				// Update_meta expects slashed.
				update_post_meta( $attachment_id, '_bd_attachment_image_alt', bd_slash( $image_alt ) );
			}
		}
		...
		foreach ( get_attachment_taxonomies($post) as $t ) {
			if ( isset($attachment[$t]) )
				bd_set_object_terms($attachment_id, array_map('trim', preg_split('/,+/', $attachment[$t])), $t, false);
		}
	}
	...
	if ( isset($send_id) ) {
		$attachment = bd_unslash( $_POST['attachments'][$send_id] );

		$html = isset( $attachment['post_title'] ) ? $attachment['post_title'] : '';
		if ( !empty($attachment['url']) ) {
			$rel = '';
			if ( strpos($attachment['url'], 'attachment_id') || get_attachment_link($send_id) == $attachment['url'] )
				$rel = " rel='attachment bd-att-" . esc_attr($send_id) . "'";
 ...
 * @return null|string
 */
function bd_media_upload_handler() {
	$errors = array();
	$id = 0;
	...
	if ( !empty($_POST['insertonlybutton']) ) {
		...
		if ( isset( $_POST['media_type'] ) && 'image' != $_POST['media_type'] ) {
			$title = esc_html( bd_unslash( $_POST['title'] ) );
			if ( empty( $title ) )
			...
			$align = '';
			$alt = esc_attr( bd_unslash( $_POST['alt'] ) );
			if ( isset($_POST['align']) ) {
				$align = esc_attr( bd_unslash( $_POST['align'] ) );
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' ) {
		...
		if ( isset( $_GET['type'] ) && in_array( $_GET['type'], array( 'video', 'audio', 'file' ) ) )
			$type = $_GET['type'];
		return bd_iframe( 'media_upload_type_url_form', $type, $errors, $id );
	}

	return bd_iframe( 'media_upload_type_form', 'image', $errors, $id );
}
 ...
 * @return string|BD_Error Populated HTML img tag on success, BD_Error object otherwise.
 */
function media_sideload_image( $file, $post_id, $desc = null, $return = 'html' ) {
	if ( ! empty( $file ) ) {
		...
		if ( is_bd_error( $file_array['tmp_name'] ) ) {
	...
	if ( ! empty( $src ) ) {
		...
		return new BD_Error( 'image_sideload_failed' );
	}
}
...
function media_upload_gallery() {
	...
	bd_enqueue_script('admin-gallery');
	return bd_iframe( 'media_upload_gallery_form', $errors );
}
...
function media_upload_library() {
	...
	return bd_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields($post, $url_type = '') {
	...
	$file = bd_get_attachment_url($post->ID);
 ...
 * @since 1.0.0
 *
 * @param BD_Post $edit_post Attachment BD_Post object.
 * @return string HTML markup for the textarea element.
 */
function bd_caption_input_textarea($edit_post) {
 ...
 * @param BD_Post $post        The BD_Post attachment object.
 ...
 * @since 1.0.0
 *
 * @param array $post       The BD_Post attachment object converted to an array.
 * @param array $attachment An array of attachment metadata.
...
function image_attachment_fields_to_save( $post, $attachment ) {
	if ( substr( $post['post_mime_type'], 0, 5 ) == 'image' ) {
		if ( strlen( trim( $post['post_title'] ) ) == 0 ) {
			...
			$post['post_title'] = preg_replace( '/\.\w+$/', '', bd_basename( $attachment_url ) );
...
function get_attachment_fields_to_edit($post, $errors = null) {
	if ( is_int($post) )
		$post = get_post($post);
	if ( is_array($post) )
		$post = new BD_Post( (object) $post );

	$image_url = bd_get_attachment_url($post->ID);
	...
	$form_fields = array(
		...
		'post_excerpt' => array(
			'label'      => __('Caption'),
			'input'      => 'html',
			'html'       => bd_caption_input_textarea($edit_post)
		),
		'post_content' => array(
		...
		'image_url'	=> array(
			...
			'value'      => bd_get_attachment_url($post->ID),
	...
	if ( substr($post->post_mime_type, 0, 5) == 'image' ) {
		$alt = get_post_meta($post->ID, '_bd_attachment_image_alt', true);
		if ( empty($alt) )
			$alt = '';
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array($GLOBALS['bd_the_query']->posts) )
			foreach ( $GLOBALS['bd_the_query']->posts as $attachment )
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( ( $attachment_id = intval( $attachment_id ) ) && $thumb_url = bd_get_attachment_image_src( $attachment_id, 'thumbnail', true ) )
	...
	$filename = esc_html( bd_basename( $post->guid ) );
	$title = esc_attr( $post->post_title );

	$post_mime_types = get_post_mime_types();
	$keys = array_keys( bd_match_mime_types( array_keys( $post_mime_types ), $post->post_mime_type ) );
	$type = reset( $keys );
	...
	$display_title = $r['show_title'] ? "<div class='filename new'><span class='title'>" . bd_html_excerpt( $display_title, 60, '&hellip;' ) . "</span></div>" : '';
	...
	$media_dims = '';
	$meta = bd_get_attachment_metadata( $post->ID );
	...
	$image_edit_button = '';
	if ( bd_attachment_is_image( $post->ID ) && bd_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
		$nonce = bd_create_nonce( "image_editor-$post->ID" );
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( !EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . bd_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete-permanently'>" . __( 'Delete Permanently' ) . '</a>';
		} elseif ( !MEDIA_TRASH ) {
			$delete = "<a href='#' class='del-link' onclick=\"document.getElementById('del_attachment_$attachment_id').style.display='block';return false;\">" . __( 'Delete' ) . "</a>
			 ...
			 <a href='" . bd_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . __( 'Continue' ) . "</a>
			 <a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __( 'Cancel' ) . "</a>
			 </div>";
		} else {
			$delete = "<a href='" . bd_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Move to Trash' ) . "</a>
			<a href='" . bd_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-post_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . __( 'Undo' ) . "</a>";
		}
	} else {
		$delete = '';
	}
	...
	if ( 'image' == $type && $calling_post_id && current_theme_supports( 'post-thumbnails', get_post_type( $calling_post_id ) )
		...
		$ajax_nonce = bd_create_nonce( "set_post_thumbnail-$calling_post_id" );
		$thumbnail = "<a class='bd-post-thumbnail' id='bd-post-thumbnail-" . $attachment_id . "' href='#' onclick='BDSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html( $calling_post_type_object->labels->use_featured_image ) . "</a>";
	}
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . sprintf( __('The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.'), 'http://blasdoise.com/apps/' ) . '</p>';
		return;
	}
...
<div id="media-upload-error"><?php
	...
	if (isset($errors['upload_error']) && is_bd_error($errors['upload_error']))
...
/**
 * Fires just before the legacy upload interface is loaded.
 *
 * @since 1.0.0
 */
...
$post_params = array(
	"post_id" => $post_id,
	"_bdnonce" => bd_create_nonce('media-form'),
	"type" => $_type,
	"tab" => $_tab,
	"short" => "1",
);
...
bdUploaderInit = <?php echo bd_json_encode( $plupload_init ); ?>;
</script>
 ...
 */
echo apply_filters( 'type_url_form_media', bd_media_insert_url_form( $type ) );
?>
</div>
</div>
</form>
...
<p class="ml-submit">
<input type="button" class="button" style="display:none;" onMouseDown="bdgallery.update();" name="insert-gallery" id="insert-gallery" value="<?php esc_attr_e( 'Insert gallery' ); ?>" />
<input type="button" class="button" style="display:none;" onMouseDown="bdgallery.update();" name="update-gallery" id="update-gallery" value="<?php esc_attr_e( 'Update gallery settings' ); ?>" />
</p>
</div>
</form>
<?php
...
function media_upload_library_form($errors) {
	global $bddb, $bd_query, $bd_locale, $type, $tab, $post_mime_types;
	...
	list($post_mime_types, $avail_post_mime_types) = bd_edit_attachments_query( $q );
...
<?php
$type_links = array();
$_num_posts = (array) bd_count_attachments();
$matches = bd_match_mime_types(array_keys($post_mime_types), array_keys($_num_posts));
...
if ( empty($_GET['post_mime_type']) && !empty($num_posts[$type]) ) {
	...
	list($post_mime_types, $avail_post_mime_types) = bd_edit_attachments_query();
}
...
foreach ( $post_mime_types as $mime_type => $label ) {
	...
	if ( !bd_match_mime_types($mime_type, $avail_post_mime_types) )
		continue;

	if ( isset($_GET['post_mime_type']) && bd_match_mime_types($mime_type, $_GET['post_mime_type']) )
		$class = ' class="current"';
...
$page_links = paginate_links( array(
	...
	'prev_text' => __('&laquo;'),
	'next_text' => __('&raquo;'),
	'total' => ceil($bd_query->found_posts / 10),
	'current' => $q['paged'],
));
...
$arc_query = "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $bddb->posts WHERE post_type = 'attachment' ORDER BY post_date DESC";

$arc_result = $bddb->get_results( $arc_query );
...
foreach ($arc_result as $arc_row) {
	...
	echo esc_html( $bd_locale->get_month($arc_row->mmonth) . " $arc_row->yyear" );
	echo "</option>\n";
}
?>
</select>
 ...
 */
function bd_media_insert_url_form( $default_view = 'image' ) {
	/** This filter is documented in bd-admin/includes/media.php */
	if ( ! apply_filters( 'disable_captions', '' ) ) {
		$caption = '
		<tr class="image-only">
...
';
	...
	<table class="describe ' . $table_class . '"><tbody>
		...
		<tr class="image-only">
			<td></td>
			<td>
				<input type="button" class="button" id="go_button" style="color:#bbbbbb;" onclick="addExtImage.insert()" value="' . esc_attr__('Insert into Post') . '" />
			</td>
		</tr>
		<tr class="not-image">
			<td></td>
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
	   <?php _e('You are using the browser&#8217;s built-in file uploader. The Blasdoise uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.'); ?>
	</p>
	<?php
}
...
function edit_form_image_editor( $post ) {
	...
	if ( $open )
		require_once ABSPATH . 'bd-admin/includes/image-edit.php';

	$thumb_url = false;
	if ( $attachment_id = intval( $post->ID ) )
		$thumb_url = bd_get_attachment_image_src( $attachment_id, array( 900, 450 ), true );

	$alt_text = get_post_meta( $post->ID, '_bd_attachment_image_alt', true );

	$att_url = bd_get_attachment_url( $post->ID ); ?>
	<div class="bd_attachment_holder">
	<?php
	if ( bd_attachment_is_image( $post->ID ) ) :
		$image_edit_button = '';
		if ( bd_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
			$nonce = bd_create_nonce( "image_editor-$post->ID" );
			$image_edit_button = "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" . esc_attr__( 'Edit Image' ) . "' /> <span class='spinner'></span>";
		}
	?>
		...
		<div<?php if ( $open ) echo ' style="display:none"'; ?> class="bd_attachment_image" id="media-head-<?php echo $attachment_id; ?>">
		...
		<div<?php if ( ! $open ) echo ' style="display:none"'; ?> class="image-editor" id="image-editor-<?php echo $attachment_id; ?>">
			<?php if ( $open ) bd_image_editor( $attachment_id ); ?>
		</div>
	<?php
	elseif ( $attachment_id && bd_attachment_is( 'audio', $post ) ):

		bd_maybe_generate_attachment_metadata( $post );

		echo bd_audio_shortcode( array( 'src' => $att_url ) );

	elseif ( $attachment_id && bd_attachment_is( 'video', $post ) ):
		...
		if ( ! empty( $thumb_id ) ) {
			$attr['poster'] = bd_get_attachment_url( $thumb_id );
		}

		echo bd_video_shortcode( $attr );

	endif; ?>
	</div>
	<div class="bd_attachment_details edit-form-section">
		<p>
	...
	<?php if ( 'image' === substr( $post->post_mime_type, 0, 5 ) ) : ?>
		<p>
			...
			<input type="text" class="widefat" name="_bd_attachment_image_alt" id="attachment_alt" value="<?php echo esc_attr( $alt_text ); ?>" />
		</p>
	<?php endif; ?>
	...
	<?php bd_editor( $post->post_content, 'attachment_content', $editor_args ); ?>
 ...
 */
function bd_add_id3_tag_data( &$metadata, $data ) {
 ...
 */
function bd_read_video_metadata( $file ) {
	if ( ! file_exists( $file ) )
		return false;
	...
	if ( ! class_exists( 'getID3' ) )
		require( ABSPATH . BDINC . '/id3/getid3.php' );
	$id3 = new getID3();
	...
	bd_add_id3_tag_data( $metadata, $data );
 ...
 * @return array|bool Returns array of metadata, if found.
 */
function bd_read_audio_metadata( $file ) {
	if ( ! file_exists( $file ) )
		return false;
	$metadata = array();
 ...
 *                          Default 'attach'.
 */
function bd_media_attach_action( $parent_id, $action = 'attach' ) {
	global $bddb;
	...
	if ( ! empty( $ids ) ) {
		...
		if ( 'attach' === $action ) {
			$result = $bddb->query( $bddb->prepare( "UPDATE $bddb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID IN ( $ids_string )", $parent_id ) );
		} else {
			$result = $bddb->query( "UPDATE $bddb->posts SET post_parent = 0 WHERE post_type = 'attachment' AND ID IN ( $ids_string )" );
		}
