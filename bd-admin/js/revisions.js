...
(function($) {
	var revisions;
	/**
	 * Expose the module in window.bd.revisions.
	 */
	revisions = bd.revisions = { model: {}, view: {}, controller: {} };

	// Link post revisions data served from the back-end.
	revisions.settings = window._bdRevisionsSettings || {};
	...
	revisions.model.Diffs = Backbone.Collection.extend({
		...
		load: function( comparisons ) {
			bd.revisions.log( 'load', comparisons );
			...
			return this.fetch({ data: { compare: comparisons }, remove: false }).done( function() {
				bd.revisions.log( 'load:complete', comparisons );
			});
		},
		...
		sync: function( method, model, options ) {
			if ( 'read' === method ) {
				...
				var deferred = bd.ajax.send( options ),
	...
	/**
	 * bd.revisions.model.FrameState
	 *
	 * The frame state.
	 *
	 * @see bd.revisions.view.Frame
	 *
	 ...
	 */
	revisions.view.Frame = bd.Backbone.View.extend({
		className: 'revisions',
		template: bd.template('revisions-frame'),
		...
		render: function() {
			bd.Backbone.View.prototype.render.apply( this, arguments );

			$('html').css( 'overflow-y', 'scroll' );
			$('#bdbody-content .wrap').append( this.el );
	...
	revisions.view.Controls = bd.Backbone.View.extend({
		...
		ready: function() {
			...
			this.window = $(window);
			this.window.on( 'scroll.bd.revisions', {controls: this}, function(e) {
				...
				if ( scrolled >= controls.top ) {
					if ( ! frame.$el.hasClass('pinned') ) {
						...
						controls.window.on('resize.bd.revisions.pinning click.bd.revisions.pinning', {controls: controls}, function(e) {
							e.data.controls.setWidth();
						});
					}
					frame.$el.addClass('pinned');
				} else if ( frame.$el.hasClass('pinned') ) {
					controls.window.off('.bd.revisions.pinning');
	...
	// The tickmarks view
	revisions.view.Tickmarks = bd.Backbone.View.extend({
	...
	// The metabox view
	revisions.view.Metabox = bd.Backbone.View.extend({
	...
	revisions.view.Meta = bd.Backbone.View.extend({
		template: bd.template('revisions-meta'),
	...
	// The checkbox view.
	revisions.view.Checkbox = bd.Backbone.View.extend({
		className: 'revisions-checkbox',
		template: bd.template('revisions-checkbox'),
	...
	revisions.view.Tooltip = bd.Backbone.View.extend({
	...
	revisions.view.Buttons = bd.Backbone.View.extend({
		className: 'revisions-buttons',
		template: bd.template('revisions-buttons'),
	...
	// The slider view.
	revisions.view.Slider = bd.Backbone.View.extend({
		className: 'bd-slider',
		direction: isRtl ? 'right' : 'left',
		...
		start: function( event, ui ) {
			...
			$( window ).on( 'mousemove.bd.revisions', { view: this }, function( e ) {
				var handles,
					view              = e.data.view,
		...
		stop: function() {
			$( window ).off('mousemove.bd.revisions');
	...
	revisions.view.Diff = bd.Backbone.View.extend({
		className: 'revisions-diff',
		template:  bd.template('revisions-diff'),
	...
	revisions.init = function() {
		...
		state = new revisions.model.FrameState({
			initialDiffState: {
				// bd_localize_script doesn't stringifies ints, so cast them.
				to: parseInt( revisions.settings.to, 10 ),
				from: parseInt( revisions.settings.from, 10 ),
				// bd_localize_script does not allow for top-level booleans so do a comparator here.
