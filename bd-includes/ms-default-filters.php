...
// Users
add_filter( 'bdmu_validate_user_signup', 'signup_nonce_check' );
add_action( 'init', 'maybe_add_existing_user_to_blog' );
add_action( 'bdmu_new_user', 'newuser_notify_siteadmin' );
add_action( 'bdmu_activate_user', 'add_new_user_to_blog', 10, 3 );
add_filter( 'sanitize_user', 'strtolower' );

// Blogs
add_filter( 'bdmu_validate_blog_signup', 'signup_nonce_check' );
add_action( 'bdmu_new_blog', 'bdmu_log_new_registrations', 10, 2 );
add_action( 'bdmu_new_blog', 'newblog_notify_siteadmin', 10, 2 );
...
// Counts
add_action( 'admin_init', 'bd_schedule_update_network_counts');
add_action( 'update_network_counts', 'bd_update_network_counts');
foreach ( array( 'user_register', 'deleted_user', 'bdmu_new_user', 'make_spam_user', 'make_ham_user' ) as $action )
	add_action( $action, 'bd_maybe_update_network_user_counts' );
foreach ( array( 'make_spam_blog', 'make_ham_blog', 'archive_blog', 'unarchive_blog', 'make_delete_blog', 'make_undelete_blog' ) as $action )
	add_action( $action, 'bd_maybe_update_network_site_counts' );
unset( $action );

// Files
add_filter( 'bd_upload_bits', 'upload_is_file_too_big' );
...
// BD_HOME and BD_SITEURL should not have any effect in MS
remove_filter( 'option_siteurl', '_config_bd_siteurl' );
remove_filter( 'option_home',    '_config_bd_home'    );
