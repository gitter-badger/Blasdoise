 ...
 * @return string|BD_Error A list of category parents on success, BD_Error on failure.
 */
...
function get_the_category( $id = false ) {
	...
	if ( ! $categories || is_bd_error( $categories ) )
		$categories = array();
 ...
 * @return string|BD_Error Category name on success, BD_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents='', $post_id = false ) {
	...
	$rel = ( is_object( $bd_rewrite ) && $bd_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of Blasdoise, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the Blasdoise Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 1.0.0
 *
 * @param int|string|array $category Category ID, name or slug, or array of said.
 * @param int|object $post Optional. Post to check instead of the current post. (since 1.0.0)
 ...
 */
function bd_dropdown_categories( $args = '' ) {
	$defaults = array(
	 ...
	 */
	$output = apply_filters( 'bd_dropdown_cats', $output, $r );
 ...
 */
function bd_list_categories( $args = '' ) {
	$defaults = array(
	 ...
	 */
	$html = apply_filters( 'bd_list_categories', $output, $args );
...
function bd_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_bd_error( $tags ) )
		return;
	...
	$return = bd_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args
	 ...
	 */
	$return = apply_filters( 'bd_tag_cloud', $return, $args );
 ...
 */
function bd_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_bd_object_name_sort_cb' );
			} else {
				uasort( $tags, '_bd_object_count_sort_cb' );
			}
	 ...
	 */
	$tags_data = apply_filters( 'bd_generate_tag_cloud_data', $tags_data );
	...
	if ( $args['filter'] ) {
		 ...
		 * to the $filter argument in bd_generate_tag_cloud().
		 *
		 * @since 1.0.0
		 *
		 * @see bd_generate_tag_cloud()
		 *
		 * @param array|string $return String containing the generated HTML tag cloud output
		 *                             or an array of tag links if the 'format' argument
		 *                             equals 'array'.
		 * @param array        $tags   An array of terms used in the tag cloud.
		 * @param array        $args   An array of bd_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'bd_generate_tag_cloud', $return, $tags, $args );
	}
 ...
 * @return int
 */
function _bd_object_name_sort_cb( $a, $b ) {
 ...
 * @return bool
 */
function _bd_object_count_sort_cb( $a, $b ) {
...
class Walker_Category extends Walker {
	 ...
	 * @param array  $args     An array of arguments. @see bd_list_categories()
	...
	public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( 'list' == $args['style'] ) {
			 ...
			 * @param array  $args        An array of bd_list_categories() arguments.
			 */
	 ...
	 * @param array  $args   An array of arguments. Only uses 'list' for whether should append to output. @see bd_list_categories()
	 */
...
class Walker_CategoryDropdown extends Walker {
	 ...
	 *                         See {@see bd_dropdown_categories()}.
	 */
	public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
 ...
 * @return array|false|BD_Error Array of tag objects on success, false on failure.
 */
 ...
 * @return string|false|BD_Error A list of tags on success, false if there are no terms, BD_Error on failure.
 */
...
function term_description( $term = 0, $taxonomy = 'post_tag' ) {
	...
	return is_bd_error( $description ) ? '' : $description;
}
 ...
 * @return array|false|BD_Error Array of term objects on success, false if there are no terms
 *                              or the post does not exist, BD_Error on failure.
 */
function get_the_terms( $post, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = bd_get_object_terms( $post->ID, $taxonomy );
		bd_cache_add($post->ID, $terms, $taxonomy . '_relationships');
	}
	 ...
	 * @since 1.0.0
	 *
	 * @param array|BD_Error $terms    List of attached terms, or BD_Error on failure.
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return string|false|BD_Error A list of terms on success, false if there are no terms, BD_Error on failure.
 */
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return false|void False on Blasdoise error.
 */
function the_terms( $id, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_bd_error( $term_list ) )
		return false;
 ...
 *
 * Prior to v2.7 of Blasdoise, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the Blasdoise Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 1.0.0
