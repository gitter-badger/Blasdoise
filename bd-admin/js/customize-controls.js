/* global _bdCustomizeHeader, _bdCustomizeBackground, _bdMediaViewsL10n, MediaElementPlayer */
(function( exports, $ ){
	var Container, focus, api = bd.customize;

	/**
	 * @class
	 * @augments bd.customize.Value
	 * @augments bd.customize.Class
	 *
	 * @param options
	 ...
	 * @since 1.0.0
	 *
	 * @param {bd.customize.Class} instance
	...
	focus = function ( params ) {
		...
		focus = function () {
			...
			// Note that we can't use :focusable due to a jQuery UI issue.
	 ...
	 * @since 1.0.0
	 *
	 * @param {(bd.customize.Panel|bd.customize.Section|bd.customize.Control)} a
	 * @param {(bd.customize.Panel|bd.customize.Section|bd.customize.Control)} b
	 * @returns {Number}
	 */
	...
	Container = api.Class.extend({
		 ...
		 * @param {string=default} [options.params.type] - The type of the panel. See bd.customize.panelConstructor.
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = bd.template( container.templateSelector );
			} else {
				template = bd.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend({
		 ...
		 * @param {string=default} [options.params.type] - The type of the section. See bd.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.container.closest( '.bd-full-overlay-sidebar-content' ),
				content = section.container.find( '.accordion-section-content' ),
				overlay = section.container.closest( '.bd-full-overlay' ),
	 ...
	 * @augments bd.customize.Container
	 */
	api.ThemesSection = api.Section.extend({
		 ...
		 */
		initialize: function () {
			this.$customizeSidebar = $( '.bd-full-overlay-sidebar-content:first' );
		...
		ready: function () {
			...
			section.template = bd.template( 'customize-themes-details-view' );
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var position, scroll,
				...
				overlay = section.closest( '.bd-full-overlay' ),
				container = section.closest( '.bd-full-overlay-sidebar-content' ),
	...
	api.Panel = Container.extend({
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = bd.template( panel.templateSelector + '-content' );
			} else {
				template = bd.template( 'customize-panel-default-content' );
			}
	 ...
	 *
	 * @see PHP class BD_Customize_Control.
	 *
	 * @class
	 * @augments bd.customize.Class
	 *
	...
	api.Control = api.Class.extend({
		...
		/**
		 * @deprecated 1.0.0 Use this.onChangeActive() instead.
		 */
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + control.templateSelector ).length ) {
				template = bd.template( control.templateSelector );
	...
	api.ColorControl = api.Control.extend({
		ready: function() {
			...
			picker.val( control.setting() ).bdColorPicker({
				change: function() {
					control.setting.set( picker.bdColorPicker('color') );
				},
				clear: function() {
			...
			this.setting.bind( function ( value ) {
				picker.val( value );
				picker.bdColorPicker( 'color', value );
			});
		}
	});
	...
	api.MediaControl = api.Control.extend({
		...
		cleanupPlayer: function () {
			this.player && bd.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = bd.media({
				...
				states: [
					new bd.media.controller.Library({
						...
						library:   bd.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._bdmejsSettings || {};
	 ...
	 *
	 * @class
	 * @augments bd.customize.MediaControl
	 * @augments bd.customize.Control
	 * @augments bd.customize.Class
	 */
	 ...
	 *
	 * @class
	 * @augments bd.customize.UploadControl
	 * @augments bd.customize.MediaControl
	 * @augments bd.customize.Control
	 * @augments bd.customize.Class
	 */
	...
	api.BackgroundControl = api.UploadControl.extend({
		...
		select: function() {
			...
			bd.ajax.post( 'custom-background-add', {
				nonce: _bdCustomizeBackground.nonces.add,
				bd_customize: 'on',
	...
	api.CroppedImageControl = api.MediaControl.extend({
		...
		initFrame: function() {
			...
			this.frame = bd.media({
				...
				states: [
					...
					new bd.media.controller.CustomizeImageCropper({
		 ...
		 * @param {bd.media.controller.Cropper} controller
	 ...
	 *
	 * @class
	 * @augments bd.customize.CroppedImageControl
	 * @augments bd.customize.MediaControl
	 * @augments bd.customize.Control
	 * @augments bd.customize.Class
	 */
		...
		initFrame: function() {
			...
			this.frame = bd.media({
				...
				states: [
					...
					new bd.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	api.HeaderControl = api.Control.extend({
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _bdCustomizeHeader.uploads, function( imageObj ) {
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_bdCustomizeHeader.data.width, 10),
				yInit = parseInt(_bdCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_bdCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_bdCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = bd.media({
				...
				states: [
					new bd.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _bdCustomizeHeader.data.width,
						suggestedHeight: _bdCustomizeHeader.data.height
					}),
					new bd.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new bd.customize.HeaderTool.ImageModel from provided
	...
	api.ThemeControl = api.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click keydown touchend', '.theme', function( event ) {
				...
				$( '.bd-full-overlay' ).addClass( 'customize-loading' );
	...
	/**
	 * @class
	 * @augments bd.customize.Messenger
	 * @augments bd.customize.Class
	 * @mixes bd.customize.Events
	 */
	...
	api.Previewer = api.Messenger.extend({
		...
		initialize: function( params, options ) {
			...
			this.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Check for URLs that include "/bd-admin/" or end in "/bd-admin".
				// Strip hashes and query strings before testing.
				if ( /\/bd-admin(\/|$)/.test( to.replace( /[#?].*$/, '' ) ) )
					return null;
		...
		refreshNonces: function() {
			...
			request = bd.ajax.post( 'customize_refresh_nonces', {
				bd_customize: 'on',
				theme: api.settings.theme.stylesheet
			});
	...
	$( function() {
		api.settings = window._bdCustomizeSettings;
		api.l10n = window._bdCustomizeControlsL10n;
		...
		var parent, topFocus,
			body = $( document.body ),
			overlay = body.children( '.bd-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			signature:   'BD_CUSTOMIZER_SIGNATURE'
		}, {
			...
			save: function() {
				...
				submit = function () {
					...
					request = bd.ajax.post( 'customize_save', query );
