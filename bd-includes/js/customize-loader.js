/* global _bdCustomizeLoaderSettings, confirm */
window.bd = window.bd || {};
...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo bd_customize_url(); ?>">Open Customizer</a>
	 *
	 * @augments bd.customize.Events
	 */
	Loader = $.extend( {}, api.Events, {
		...
		initialize: function() {
			...
			// the Customizer.
			$('#bdbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'bd_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'bd_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _bdCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.bd.customize.Loader
	api.Loader = Loader;
})( bd, jQuery );
