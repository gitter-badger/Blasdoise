<?php
/**
 * Blasdoise Administration Revisions API
 *
 * @package Blasdoise
 ...
 *                    Or, false on failure.
 */
function bd_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	...
	foreach ( _bd_post_revision_fields() as $field => $name ) {
		/**
		 ...
		 * @param BD_Post $compare_from         The revision post object to compare to or from.
		 * @param string  null                  The context of whether the current revision is the old
		 *                                      or the new one. Values are 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_bd_post_revision_field_$field", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in bd-admin/includes/revision.php */
		$content_to = apply_filters( "_bd_post_revision_field_$field", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 *
		 * Filter the options passed to {@see bd_text_diff()} when viewing a post revision.
		 *
		 * @since 1.0.0
		 *
		 * @param array   $args {
		 *     Associative array of options to pass to {@see bd_text_diff()}.
		 *
		 *     @type bool $show_split_view True for split view (two columns), false for
		 *                                 un-split view (single column). Default true.
		 * }
		 * @param string  $field        The current revision field.
		 * @param BD_Post $compare_from The revision post to compare from.
		 * @param BD_Post $compare_to   The revision post to compare to.
		 */
		$args = apply_filters( 'revision_text_diff_options', $args, $field, $compare_from, $compare_to );

		$diff = bd_text_diff( $content_from, $content_to, $args );
	 ...
	 * @param BD_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'bd_get_revision_ui_diff', $return, $compare_from, $compare_to );
 ...
 */
function bd_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post = get_post( $post );
	$authors = array();
	$now_gmt = time();

	$revisions = bd_get_post_revisions( $post->ID, array( 'order' => 'ASC', 'check_enabled' => false ) );
	...
	cache_users( bd_list_pluck( $revisions, 'post_author' ) );
	...
	foreach ( $revisions as $revision ) {
		...
		if ( $can_restore ) {
			$restore_link = str_replace( '&amp;', '&', bd_nonce_url(
				add_query_arg(
		...
		$autosave = (bool) bd_is_post_autosave( $revision );
	...
	$diffs = array( array(
		...
		'fields' => bd_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
	));

	return array(
		'postId'           => $post->ID,
		'nonce'            => bd_create_nonce( 'revisions-ajax-nonce' ),
 ...
 * @since 1.0.0
 *
 * @global BD_Post $post The global `$post` object.
 */
function bd_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( bd_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>
