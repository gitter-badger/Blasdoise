 ...
 * @see bd_ajax_nopriv_heartbeat(), bd_ajax_heartbeat()
 *
...
( function( $, window, undefined ) {
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.bd-heartbeat', function() {
			...
			$(window).on( 'unload.bd-heartbeat', function() {
		...
		function userIsActive() {
			...
			$document.off( '.bd-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.bd-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.bd-heartbeat-active keyup.bd-heartbeat-active touchend.bd-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.bd-heartbeat-active keyup.bd-heartbeat-active touchend.bd-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 * Use bd.heartbeat.isQueued('handle') to see if any data is already queued for that handle.
		 *
	...
	window.bd = window.bd || {};
	window.bd.heartbeat = new Heartbeat();
