/* global _bdThemeSettings, confirm */
window.bd = window.bd || {};
...
// Setup app structure
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: bd.template });

themes.Model = Backbone.Model.extend({
	// Adds attributes to the default data coming through the themes api
...
themes.view.Appearance = bd.Backbone.View.extend({

	el: '#bdbody-content .wrap .theme-browser',
	...
	searchContainer: $( '#bdbody h1:first' ),
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="bd-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
	},
...
themes.Collection = Backbone.Collection.extend({
	...
	apiCall: function( request, paginated ) {
		return bd.ajax.send( 'query-themes', {
			data: {
			// Request data
				request: _.extend({
					per_page: 100,
...
themes.view.Theme = bd.Backbone.View.extend({
...
themes.view.Details = bd.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.bd-themes', function( event ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'bd-full-overlay expanded',
...
themes.view.Themes = bd.Backbone.View.extend({
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			bd.a11y.speak( l10n.noThemesFound );
		} else {
			bd.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = bd.Backbone.View.extend({

	tagName: 'input',
	className: 'bd-filter-search',
	id: 'bd-filter-search-input',
	searching: false,
...
themes.Router = Backbone.Router.extend({
	...
	search: function( query ) {
		$( '.bd-filter-search' ).val( query );
	},

	themes: function() {
		$( '.bd-filter-search' ).val( '' );
	},
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.bd-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	// in new location
	searchContainer: $( '.bd-filter .search-form' ),
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `bd.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.bd-filter-search' ).focus().trigger( 'keyup' );
		});
...
$( document ).ready(function() {
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _bdThemeSettings.settings.confirmDelete );
	});
});
...
jQuery(document).ready( function($) {
	tb_position = function() {
		...
		if ( $('#bdadminbar').length ) {
			adminbar_height = parseInt( $('#bdadminbar').css('height'), 10 );
		}
