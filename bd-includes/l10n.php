 ...
 *
 * If the locale is not set already, then the BDLANG constant is used if it is
 ...
 */
function get_locale() {
	global $locale, $bd_local_package;

	if ( isset( $locale ) ) {
		/**
		 * Filter Blasdoise install's locale ID.
		 *
		 * @since 1.0.0
		 *
	...
	if ( isset( $bd_local_package ) ) {
		$locale = $bd_local_package;
	}

	// BDLANG was defined in bd-config.
	if ( defined( 'BDLANG' ) ) {
		$locale = BDLANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( defined( 'BD_INSTALLING' ) || ( false === $ms_locale = get_option( 'BDLANG' ) ) ) {
			$ms_locale = get_site_option( 'BDLANG' );
		}
	...
	/** This filter is documented in bd-includes/l10n.php */
	return apply_filters( 'locale', $locale );
}
 ...
 *
 * Loads the .mo file in BD_LANG_DIR constant path from Blasdoise root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', BD_LANG_DIR . "/$locale.mo" );

	if ( ( is_multisite() || ( defined( 'BD_INSTALLING_NETWORK' ) && BD_INSTALLING_NETWORK ) ) && ! file_exists(  BD_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', BD_LANG_DIR . "/ms-$locale.mo" );
		return $return;
	}

	if ( is_admin() || defined( 'BD_INSTALLING' ) || ( defined( 'BD_REPAIRING' ) && BD_REPAIRING ) ) {
		load_textdomain( 'default', BD_LANG_DIR . "/admin-$locale.mo" );
	}

	if ( is_network_admin() || ( defined( 'BD_INSTALLING_NETWORK' ) && BD_INSTALLING_NETWORK ) )
		load_textdomain( 'default', BD_LANG_DIR . "/admin-network-$locale.mo" );
 ...
 * @param string $plugin_rel_path Optional. Relative path to BD_PLUGIN_DIR where the .mo file resides.
 *                                Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	if ( false !== $plugin_rel_path	) {
		$path = BD_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
	...
	$mofile = BD_LANG_DIR . '/plugins/' . $mofile;
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Relative to BDMU_PLUGIN_DIR directory in which the .mo file resides.
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = trailingslashit( BDMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' ) );
...
function load_theme_textdomain( $domain, $path = false ) {
	...
	// Otherwise, load from the languages directory
	$mofile = BD_LANG_DIR . "/themes/{$domain}-{$locale}.mo";
	return load_textdomain( $domain, $mofile );
}
 ...
 *
 * The default directory is BD_LANG_DIR.
 *
 * @since 1.0.0
 *
...
function get_available_languages( $dir = null ) {
	...
	$lang_files = glob( ( is_null( $dir) ? BD_LANG_DIR : $dir ) . '/*.mo' );
	if ( $lang_files ) {
 ...
 *
 * Looks in the bd-content/languages directory for translations of
 * plugins or themes.
 *
 * @since 1.0.0
 *
 ...
 */
function bd_get_installed_translations( $type ) {
	if ( $type !== 'themes' && $type !== 'plugins' && $type !== 'core' )
		return array();

	$dir = 'core' === $type ? '' : "/$type";

	if ( ! is_dir( BD_LANG_DIR ) )
		return array();

	if ( $dir && ! is_dir( BD_LANG_DIR . $dir ) )
		return array();

	$files = scandir( BD_LANG_DIR . $dir );
	if ( ! $files )
		return array();
	...
	foreach ( $files as $file ) {
		...
		$language_data[ $textdomain ][ $language ] = bd_get_pomo_file_data( BD_LANG_DIR . "$dir/$file" );
	}
 ...
 */
function bd_get_pomo_file_data( $po_file ) {
 ...
 */
function bd_dropdown_languages( $args = array() ) {
