...
if ( is_multisite() && ! is_network_admin() ) {
	bd_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit();
}

if ( !current_user_can('edit_plugins') )
	bd_die( __('You do not have sufficient permissions to edit plugins for this site.') );

$title = __("Edit Plugins");
$parent_file = 'plugins.php';

bd_reset_vars( array( 'action', 'error', 'file', 'plugin' ) );
...
case 'update':
	...
	$newcontent = bd_unslash( $_POST['newcontent'] );
	if ( is_writeable($real_file) ) {
		...
		if ( is_plugin_active($file) || isset($_POST['phperror']) ) {
			...
			bd_redirect(add_query_arg('_bdnonce', bd_create_nonce('edit-plugin-test_' . $file), "plugin-editor.php?file=$file&liveupdate=1&scrollto=$scrollto&networkwide=" . $network_wide));
			exit;
		}
		bd_redirect( self_admin_url("plugin-editor.php?file=$file&a=te&scrollto=$scrollto") );
	} else {
		bd_redirect( self_admin_url("plugin-editor.php?file=$file&scrollto=$scrollto") );
	}
	exit;
...
default:
	...
	if ( ! is_file($real_file) ) {
		bd_die(sprintf('<p>%s</p>', __('No such file exists! Double check the name and try again.')));
	} else {
		...
		if ( preg_match('/\.([^.]+)$/', $real_file, $matches) ) {
			...
			if ( !in_array( $ext, $editable_extensions) )
				bd_die(sprintf('<p>%s</p>', __('Files of this type are not editable.')));
		}
	}
	...
	update_recently_edited(BD_PLUGIN_DIR . '/' . $file);

	$content = file_get_contents( $real_file );

	if ( '.php' == substr( $real_file, strrpos( $real_file, '.' ) ) ) {
		$functions = bd_doc_link_parse( $content );
...
<?php elseif (isset($_GET['phperror'])) : ?>
 <div id="message" class="updated"><p><?php _e('This plugin has been deactivated because your changes resulted in a <strong>fatal error</strong>.') ?></p>
	<?php
		if ( bd_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $file) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php bloginfo('bdurl'); ?>/bd-admin/plugins.php?action=error_scrape&amp;plugin=<?php echo esc_attr($file); ?>&amp;_bdnonce=<?php echo esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php } ?>
</div>
<?php endif; ?>
<div class="wrap">
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php bd_nonce_field('edit-plugin_' . $file) ?>
