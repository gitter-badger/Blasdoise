...
/** Sets up the Blasdoise Environment. */
...
if ( is_array( get_site_option( 'illegal_names' )) && isset( $_GET[ 'new' ] ) && in_array( $_GET[ 'new' ], get_site_option( 'illegal_names' ) ) ) {
	bd_redirect( network_home_url() );
	die();
}

/**
 * Prints signup_header via bd_header
 *
 * @since 1.0.0
 */
...
add_action( 'bd_header', 'do_signup_header' );

if ( !is_multisite() ) {
	bd_redirect( site_url('bd-login.php?action=register') );
	die();
}

if ( !is_main_site() ) {
	bd_redirect( network_site_url( 'bd-signup.php' ) );
	die();
}

// Fix for page title
$bd_query->is_404 = false;
 ...
 * @since 1.0.0
 */
function bdmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'bd_header', 'bdmu_signup_stylesheet' );
get_header();
...
function validate_blog_form() {
	...
	return bdmu_validate_blog_signup($_POST['blogname'], $_POST['blog_title'], $user);
}
...
function validate_user_form() {
	return bdmu_validate_user_signup($_POST['user_name'], $_POST['user_email']);
}
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	<form id="setupform" method="post" action="bd-signup.php">
...
function validate_another_blog_signup() {
	global $bddb, $blogname, $blog_title, $errors, $domain, $path;
	$current_user = bd_get_current_user();
	if ( ! is_user_logged_in() ) {
		die();
	}
	 ...
	 *
	 * @since 1.0.0
	 * @deprecated 1.0.0 Use the 'add_signup_meta' filter instead.
	 *
	...
	bdmu_create_blog( $domain, $path, $blog_title, $current_user->ID, $meta, $bddb->siteid );
...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array() ) {
	...
	<p>
		<?php printf( __( '<a href="http://%1$s">http://%2$s</a> is your new site. <a href="%3$s">Log in</a> as &#8220;%4$s&#8221; using your existing password.' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "bd-login.php", $user_name ) ?>
	</p>
	<?php
	/**
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	...
	<form id="setupform" method="post" action="bd-signup.php" novalidate="novalidate">
		<input type="hidden" name="stage" value="validate-user-signup" />
		<?php
		/** This action is documented in bd-signup.php */
...
function validate_user_signup() {
	...
	/** This filter is documented in bd-signup.php */
	bdmu_signup_user( $user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function validate_blog_signup() {
	...
	$user_result = bdmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
	...
	$result = bdmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'] );
	$domain = $result['domain'];
	$path = $result['path'];
	$blogname = $result['blogname'];
	$blog_title = $result['blog_title'];
	...
	bdmu_signup_blog($domain, $path, $blog_title, $user_name, $user_email, $meta);
 ...
 */
$active_signup = apply_filters( 'bdmu_active_signup', $active_signup );
...
if ( $active_signup == 'none' ) {
	...
	$login_url = site_url( 'bd-login.php?redirect_to=' . urlencode( network_site_url( 'bd-signup.php' ) ) );
...
<?php
/**
 * Fires after the sign-up forms, before bd_footer.
 *
 * @since 1.0.0
 */
