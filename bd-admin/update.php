...
if ( isset($_GET['action']) ) {
	...
	if ( 'update-selected' == $action ) {
		if ( ! current_user_can( 'update_plugins' ) )
			bd_die( __( 'You do not have sufficient permissions to update plugins for this site.' ) );
		...
		if ( ! isset($_GET['failure']) && ! isset($_GET['success']) ) {
			bd_redirect( admin_url('update.php?action=activate-plugin&failure=true&plugin=' . urlencode( $plugin ) . '&_bdnonce=' . $_GET['_bdnonce']) );
			activate_plugin( $plugin, '', ! empty( $_GET['networkwide'] ), true );
			bd_redirect( admin_url('update.php?action=activate-plugin&success=true&plugin=' . urlencode( $plugin ) . '&_bdnonce=' . $_GET['_bdnonce']) );
			die();
		}
		...
		if ( ! current_user_can('update_themes') )
			bd_die(__('You do not have sufficient permissions to update themes for this site.'));
		...
		include_once( ABSPATH . 'bd-admin/includes/class-bd-upgrader.php' ); //for themes_api..
		 ...
		 * provided in the request for bd-admin/update.php. Can be used to
