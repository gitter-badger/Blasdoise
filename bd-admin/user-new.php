...
%%s' ), get_bloginfo( 'name' ), home_url(), bd_specialchars_decode( translate_user_role( $role['name'] ) ) );
	}
	add_filter( 'bdmu_signup_user_notification_email', 'admin_created_user_email' );
}
...
if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
	...
	$user_details = null;
	$user_email = bd_unslash( $_REQUEST['email'] );
	if ( false !== strpos( $user_email, '@' ) ) {
		...
		if ( is_super_admin() ) {
			...
			bd_redirect( add_query_arg( array('update' => 'enter_email'), 'user-new.php' ) );
			die();
		}
	}

	if ( !$user_details ) {
		bd_redirect( add_query_arg( array('update' => 'does_not_exist'), 'user-new.php' ) );
		die();
	}
...
%4$s' );
			bd_mail( $new_user_email, sprintf( __( '[%s] Joining confirmation' ), bd_specialchars_decode( get_option( 'blogname' ) ) ), sprintf( $message, get_option( 'blogname' ), home_url(), bd_specialchars_decode( translate_user_role( $role['name'] ) ), home_url( "/newbloguser/$newuser_key/" ) ) );
			$redirect = add_query_arg( array('update' => 'add'), 'user-new.php' );
		}
	}
	bd_redirect( $redirect );
	die();
} elseif ( isset($_REQUEST['action']) && 'createuser' == $_REQUEST['action'] ) {
	check_admin_referer( 'create-user', '_bdnonce_create-user' );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = bd_unslash( $_REQUEST['email'] );
		$user_details = bdmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		if ( is_bd_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
			 ...
			 */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( bd_unslash( $_REQUEST['user_login'] ), true ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				add_filter( 'bdmu_signup_user_notification', '__return_false' ); // Disable confirmation email
				add_filter( 'bdmu_welcome_user_notification', '__return_false' ); // Disable welcome email
			}
			bdmu_signup_user( $new_user_login, $new_user_email, array( 'add_to_blog' => $bddb->blogid, 'new_role' => $_REQUEST['role'] ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && is_super_admin() ) {
				$key = $bddb->get_var( $bddb->prepare( "SELECT activation_key FROM {$bddb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				bdmu_activate_signup( $key );
				$redirect = add_query_arg( array('update' => 'addnoconfirmation'), 'user-new.php' );
			} else {
...
bd_enqueue_script('bd-ajax-response');
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! bd_is_large_network( 'users' )
	...
	bd_enqueue_script( 'user-suggest' );
}
...
<?php if ( isset($errors) && is_bd_error( $errors ) ) : ?>
	<div class="error">
		<ul>
		<?php
...
<?php if ( isset($add_user_errors) && is_bd_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<table class="form-table">
	<tr class="form-field form-required">
		...
		<td><input name="email" type="<?php echo $type; ?>" id="adduser-email" class="bd-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e('Role'); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php bd_dropdown_roles( get_option('default_role') ); ?>
			</select>
		</td>
	</tr>
...
<form method="post" name="createuser" id="createuser" class="validate" novalidate="novalidate"<?php
	/** This action is documented in bd-admin/user-new.php */
	do_action( 'user_new_form_tag' );
?>>
<input name="action" type="hidden" value="createuser" />
<?php bd_nonce_field( 'create-user', '_bdnonce_create-user' ); ?>
<?php
// Load up the passed data, else set to a default.
$creating = isset( $_POST['createuser'] );

$new_user_login = $creating && isset( $_POST['user_login'] ) ? bd_unslash( $_POST['user_login'] ) : '';
$new_user_firstname = $creating && isset( $_POST['first_name'] ) ? bd_unslash( $_POST['first_name'] ) : '';
$new_user_lastname = $creating && isset( $_POST['last_name'] ) ? bd_unslash( $_POST['last_name'] ) : '';
$new_user_email = $creating && isset( $_POST['email'] ) ? bd_unslash( $_POST['email'] ) : '';
$new_user_uri = $creating && isset( $_POST['url'] ) ? bd_unslash( $_POST['url'] ) : '';
$new_user_role = $creating && isset( $_POST['role'] ) ? bd_unslash( $_POST['role'] ) : '';
$new_user_send_password = $creating && isset( $_POST['send_password'] ) ? bd_unslash( $_POST['send_password'] ) : true;
$new_user_ignore_pass = $creating && isset( $_POST['noconfirmation'] ) ? bd_unslash( $_POST['noconfirmation'] ) : '';
...
if ( apply_filters( 'show_password_fields', true ) ) : ?>
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button button-secondary bd-generate-pw hide-if-no-js"><?php _e( 'Show password' ); ?></button>
			<div class="bd-pwd hide-if-js">
				<?php $initial_password = bd_generate_password( 24 ); ?>
				...
				<button type="button" class="button button-secondary bd-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
					<span class="basicons basicons-hidden"></span>
					<span class="text"><?php _e( 'Hide' ); ?></span>
				</button>
				<button type="button" class="button button-secondary bd-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change' ); ?>">
...
<?php } // !is_multisite ?>
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			bd_dropdown_roles($new_user_role);
			?>
			</select>
		</td>
	</tr>
