...
(function($){
	...
	$.widget('bd.pointer', {
		options: {
			pointerClass: 'bd-pointer',
			pointerWidth: 320,
			content: function() {
				return $(this).text();
			},
			buttons: function( event, t ) {
				var close  = ( bdPointerL10n ) ? bdPointerL10n.dismiss : 'Dismiss',
		...
		_create: function() {
			...
			this.content = $('<div class="bd-pointer-content"></div>');
			this.arrow   = $('<div class="bd-pointer-arrow"><div class="bd-pointer-arrow-inner"></div></div>');
		...
		_update: function( event, content ) {
			...
			if ( buttons ) {
				buttons.wrap('<div class="bd-pointer-buttons" />').parent().appendTo( this.content );
			}
		...
		repoint: function() {
			...
			this.pointer[0].className = this.pointer[0].className.replace( /bd-pointer-[^\s'"]*/, '' );

			// Add arrow class.
			this.pointer.addClass( 'bd-pointer-' + edge );
		},
