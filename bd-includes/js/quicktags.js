/* global adminpage, bdActiveEditor, quicktagsL10n, bdLink, prompt */
/*
 * Quicktags
 *
 * This is the HTML editor in Blasdoise. It can be attached to any textarea and will
...
(function(){
	...
	qt = QTags = function(settings) {
		...
		wrap = document.getElementById( 'bd-' + id + '-wrap' );
	 ...
	 * use add_action( 'admin_print_footer_scripts', 'output_my_js', 100 ) or add_action( 'bd_footer', 'output_my_js', 100 )
	 *
	...
	qt.insertContent = function(content) {
		var sel, startPos, endPos, scrollTop, text, canvas = document.getElementById(bdActiveEditor);
	...
	qt.Button.prototype.html = function(idPrefix) {
		var active, on, bd,
			...
			dfw = ( bd = window.bd ) && bd.editor && bd.editor.dfw;
	...
	qt.DFWButton.prototype.callback = function() {
		...
		if ( ! ( bd = window.bd ) || ! bd.editor || ! bd.editor.dfw ) {
			return;
		}

		window.bd.editor.dfw.toggle();
	};
