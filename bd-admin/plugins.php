...
if ( ! current_user_can('activate_plugins') )
	bd_die( __( 'You do not have sufficient permissions to manage plugins for this site.' ) );

$bd_list_table = _get_list_table('BD_Plugins_List_Table');
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can('activate_plugins') )
				bd_die(__('You do not have sufficient permissions to activate plugins for this site.'));

			if ( is_multisite() && ! is_network_admin() && is_network_only_plugin( $plugin ) ) {
				bd_redirect( self_admin_url("plugins.php?plugin_status=$status&paged=$page&s=$s") );
				exit;
			}
			...
			if ( is_bd_error( $result ) ) {
				if ( 'unexpected_output' == $result->get_error_code() ) {
					...
					bd_redirect(add_query_arg('_error_nonce', bd_create_nonce('plugin-activation-error_' . $plugin), $redirect));
					exit;
				} else {
					bd_die($result);
				}
			...
			if ( isset($_GET['from']) && 'import' == $_GET['from'] ) {
				bd_redirect( self_admin_url("plugins.php?import=" . str_replace('-importer', '', dirname($plugin))) ); // overrides the ?error=true one above and redirects to the Imports page, stripping the -importer suffix
			} else {
				bd_redirect( self_admin_url("plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s") ); // overrides the ?error=true one above
			}
			exit;
		...
		case 'activate-selected':
			...
			bd_redirect( self_admin_url("plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
		...
		case 'error_scrape':
			...
			function plugin_sandbox_scrape( $plugin ) {
				...
				include( BD_PLUGIN_DIR . '/' . $plugin );
			}
			plugin_sandbox_scrape( $plugin );
			/** This action is documented in bd-admin/includes/plugin.php */
			do_action( "activate_{$plugin}" );
			exit;

		case 'deactivate':
			if ( ! current_user_can('activate_plugins') )
				bd_die(__('You do not have sufficient permissions to deactivate plugins for this site.'));
			...
			if ( headers_sent() )
				...
				bd_redirect( self_admin_url("plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
		...
		case 'deactivate-selected':
			...
			bd_redirect( self_admin_url("plugins.php?deactivate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;

		case 'delete-selected':
			if ( ! current_user_can('delete_plugins') ) {
				bd_die(__('You do not have sufficient permissions to delete plugins for this site.'));
			}
			...
			if ( empty( $plugins ) ) {
				bd_redirect( self_admin_url( "plugins.php?error=true&main=true&plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}

			include(ABSPATH . 'bd-admin/update.php');

			$parent_file = 'plugins.php';

			if ( ! isset($_REQUEST['verify-delete']) ) {
				bd_enqueue_script('jquery');
				require_once(ABSPATH . 'bd-admin/admin-header.php');
				?>
			<div class="wrap">
				<?php
					...
					foreach ( (array) $plugins as $plugin ) {
						...
						if ( '.' == $plugin_slug ) {
							$files_to_delete[] = BD_PLUGIN_DIR . '/' . $plugin;
							if ( $data = get_plugin_data( BD_PLUGIN_DIR . '/' . $plugin ) ) {
							...
							$files = list_files( BD_PLUGIN_DIR . '/' . $plugin_slug );
							if ( $files ) {
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array) $files_to_delete as $file ) {
							echo '<li>' . esc_html( str_replace( BD_PLUGIN_DIR, '', $file ) ) . '</li>';
						}
					?>
					</ul>
				</div>
			...
			bd_redirect( self_admin_url("plugins.php?deleted=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
...
'title'		=> __('Overview'),
'content'	=>
	'<p>' . __('Plugins extend and expand the functionality of Blasdoise. Once a plugin is installed, you may activate it or deactivate it here.') . '</p>' .
	'<p>' . __('You can find additional plugins for your site by using the Plugin Installer functionality or by browsing the Blasdoise Plugin Directory directly and installing new plugins manually. To manually install a plugin you generally just need to upload the plugin file into your <code>/bd-content/plugins</code> directory. Once a plugin has been installed, you can activate it here.') . '</p>'
) );
get_current_screen()->add_help_tab( array(
'id'		=> 'compatibility-problems',
'title'		=> __('Troubleshooting'),
'content'	=>
	'<p>' . __('Most of the time, plugins play nicely with the core of Blasdoise and with other plugins. Sometimes, though, a plugin&#8217;s code will get in the way of another plugin, causing compatibility issues. If your site starts doing strange things, this may be the problem. Try deactivating all your plugins and re-activating them in various combinations until you isolate which one(s) caused the issue.') . '</p>' .
	'<p>' . sprintf( __('If something goes wrong with a plugin and you can&#8217;t use Blasdoise, delete or rename that file in the <code>%s</code> directory and it will be automatically deactivated.'), BD_PLUGIN_DIR) . '</p>'
) );
...
<?php if ( isset($_GET['error']) ) :
	...
	<?php
		if ( !isset( $_GET['main'] ) && !isset($_GET['charsout']) && bd_verify_nonce($_GET['_error_nonce'], 'plugin-activation-error_' . $plugin) ) { ?>
	<iframe style="border:0" width="100%" height="70px" src="<?php echo 'plugins.php?action=error_scrape&amp;plugin=' . esc_attr($plugin) . '&amp;_bdnonce=' . esc_attr($_GET['_error_nonce']); ?>"></iframe>
	<?php
		}
	?>
	</div>
...
<?php $bd_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>
