...
function confirm_delete_users( $users ) {
	...
	<?php
	bd_nonce_field( 'ms-users-delete' );
	...
	<?php foreach ( ( $allusers = (array) $_POST['allusers'] ) as $user_id ) {
		if ( $user_id != '' && $user_id != '0' ) {
			...
			if ( ! current_user_can( 'delete_user', $delete_user->ID ) ) {
				bd_die( sprintf( __( 'Warning! User %s cannot be deleted.' ), $delete_user->user_login ) );
			}

			if ( in_array( $delete_user->user_login, $site_admins ) ) {
				bd_die( sprintf( __( 'Warning! User cannot be deleted. The user %s is a network administrator.' ), '<em>' . $delete_user->user_login . '</em>' ) );
			}
			?>
			<tr>
	...
	<?php
	/** This action is documented in bd-admin/users.php */
...
if ( isset( $_GET['action'] ) ) {
	/** This action is documented in bd-admin/network/edit.php */
	...
	switch ( $_GET['action'] ) {
		case 'deleteuser':
			...
			if ( $id != '0' && $id != '1' ) {
				...
				bd_redirect( network_admin_url( 'users.php' ) );
			}
			exit();
		...
		case 'allusers':
			...
			if ( ( isset( $_POST['action']) || isset($_POST['action2'] ) ) && isset( $_POST['allusers'] ) ) {
						...
						switch ( $doaction ) {
							...
							case 'spam':
								...
								if ( is_super_admin( $user->ID ) )
									bd_die( sprintf( __( 'Warning! User cannot be modified. The user %s is a network administrator.' ), esc_html( $user->user_login ) ) );
				...
				bd_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $userfunction ), bd_get_referer() ) );
			} else {
		...
		case 'dodelete':
			...
			bd_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $deletefunction ), network_admin_url( 'users.php' ) ) );
			exit();
	}
}

$bd_list_table = _get_list_table('BD_MS_Users_List_Table');
...
<div class="wrap">
	...
	<form method="get" class="search-form">
		<?php $bd_list_table->search_box( __( 'Search Users' ), 'all-user' ); ?>
	</form>
...
<?php require_once( ABSPATH . 'bd-admin/admin-footer.php' ); ?>
