...
bdWidgets = {
	...
	init : function() {
		...
		$('#widgets-right .sidebar-name').click( function() {
			...
			$document.triggerHandler( 'bd-pin-menu' );
		});
		...
		$(document.body).bind('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				bdWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				bdWidgets.save( target.closest('div.widget'), 1, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				bdWidgets.close( widget );
				e.preventDefault();
			}
		});
		...
		$('#widget-list').children('.widget').draggable({
			...
			start: function( event, ui ) {
				...
				if ( chooser.length ) {
					...
					$( '#bdbody-content' ).append( chooser.hide() );
		...
		sidebars.droppable( {
			...
			over: function( event ) {
				...
				if ( bdWidgets.hoveredSidebar && ! $wrap.is( bdWidgets.hoveredSidebar ) ) {
					// Close the previous Sidebar as the Widget has been dragged onto another Sidebar.
					bdWidgets.closeSidebar( event );
				}

				if ( $wrap.hasClass( 'closed' ) ) {
					bdWidgets.hoveredSidebar = $wrap;
					$wrap.removeClass( 'closed' );
				}
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				bdWidgets.hoveredSidebar = null;

				if ( $widget.hasClass('deleting') ) {
					bdWidgets.save( $widget, 1, 0, 1 ); // delete widget
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					bdWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_bdnonce_widgets').val(),
			sidebars: []
		};
...
$document.ready( function(){ bdWidgets.init(); } );
