...
class Custom_Background {
	...
	public function __construct($admin_header_callback = '', $admin_image_div_callback = '') {
		...
		add_action( 'bd_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 1.0.0.
		add_action( 'bd_ajax_set-background-image', array( $this, 'bd_set_background_image' ) );
	}
	...
	public function admin_load() {
		get_current_screen()->add_help_tab( array(
			...
			'content' =>
				...
				'<p>' . __( 'You can also choose a background color by clicking the Select Color button and either typing in a legitimate HTML hex value, e.g. &#8220;#333333000&#8221; for red, or by choosing a color using the color picker.' ) . '</p>' .
		...
		bd_enqueue_media();
		bd_enqueue_script('custom-background');
	...
	public function take_action() {
		...
		if ( isset($_POST['reset-background']) ) {
			check_admin_referer('custom-background-reset', '_bdnonce-custom-background-reset');
		...
		if ( isset($_POST['remove-background']) ) {
			...
			check_admin_referer('custom-background-remove', '_bdnonce-custom-background-remove');
			set_theme_mod('background_image', '');
			set_theme_mod('background_image_thumb', '');
			$this->updated = true;
			bd_safe_redirect( $_POST['_bd_http_referer'] );
			return;
		}
...
<td>
<form method="post">
<?php bd_nonce_field('custom-background-remove', '_bdnonce-custom-background-remove'); ?>
...
<td>
<form method="post">
<?php bd_nonce_field('custom-background-reset', '_bdnonce-custom-background-reset'); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="bd-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php bd_nonce_field( 'custom-background-upload', '_bdnonce-custom-background-upload' ); ?>
...
<?php bd_nonce_field('custom-background'); ?>
...
<?php
	...
	public function handle_upload() {
		...
		check_admin_referer('custom-background-upload', '_bdnonce-custom-background-upload');
		...
		// Save the data
		$id = bd_insert_attachment($object, $file);

		// Add the meta-data
		bd_update_attachment_metadata( $id, bd_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_bd_attachment_is_custom_background', get_option('stylesheet' ) );

		set_theme_mod('background_image', esc_url_raw($url));

		$thumbnail = bd_get_attachment_image_src( $id, 'thumbnail' );
		set_theme_mod('background_image_thumb', esc_url_raw( $thumbnail[0] ) );

		/** This action is documented in bd-admin/custom-header.php */
		do_action( 'bd_create_file_in_uploads', $file, $id ); // For replication
		$this->updated = true;
	}
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_bd_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 1.0.0
	 */
	public function bd_set_background_image() {
		...
		update_post_meta( $attachment_id, '_bd_attachment_is_custom_background', get_option('stylesheet' ) );
		$url = bd_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = bd_get_attachment_image_src( $attachment_id, 'thumbnail' );
