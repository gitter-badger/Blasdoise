...
/**
 * $post_type is set when the BD_Terms_List_Table instance is created
 *
 * @global string $post_type
 */
global $post_type;

$bd_list_table = _get_list_table('BD_Terms_List_Table');
...
case 'add-tag':
	...
	check_admin_referer( 'add-tag', '_bdnonce_add-tag' );

	if ( !current_user_can( $tax->cap->edit_terms ) )
		bd_die( __( 'Cheatin&#8217; uh?' ), 403 );

	$ret = bd_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
	$location = 'edit-tags.php?taxonomy=' . $taxonomy;
	if ( 'post' != $post_type )
		$location .= '&post_type=' . $post_type;

	if ( $referer = bd_get_original_referer() ) {
		if ( false !== strpos( $referer, 'edit-tags.php' ) )
			$location = $referer;
	}

	if ( $ret && !is_bd_error( $ret ) )
...
case 'delete':
	...
	bd_delete_term( $tag_ID, $taxonomy );
...
case 'edit':
	...
	include( ABSPATH . 'bd-admin/edit-tag-form.php' );
...
case 'editedtag':
	...
	$ret = bd_update_term( $tag_ID, $taxonomy, $_POST );
...
if ( ! $location && ! empty( $_REQUEST['_bd_http_referer'] ) ) {
	$location = remove_query_arg( array('_bd_http_referer', '_bdnonce'), bd_unslash($_SERVER['REQUEST_URI']) );
}
...
bd_enqueue_script('admin-tags');
if ( current_user_can($tax->cap->edit_terms) )
	bd_enqueue_script('inline-edit-tax');
...
<?php $bd_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>
...
if ( !is_null( $tax->labels->popular_items ) ) {
	if ( current_user_can( $tax->cap->edit_terms ) )
		$tag_cloud = bd_tag_cloud( array( 'taxonomy' => $taxonomy, 'post_type' => $post_type, 'echo' => false, 'link' => 'edit' ) );
	else
		$tag_cloud = bd_tag_cloud( array( 'taxonomy' => $taxonomy, 'echo' => false ) );
...
if ( current_user_can($tax->cap->edit_terms) ) {
	if ( 'category' == $taxonomy ) {
		 ...
		 *
		 * @since 1.0.0
		 * @deprecated 1.0.0 Use {$taxonomy}_pre_add_form instead.
		 *
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />
<?php bd_nonce_field('add-tag', '_bdnonce_add-tag'); ?>
...
<div class="form-field term-parent-wrap">
	...
	bd_dropdown_categories( $dropdown_args );
	?>
...
if ( 'category' == $taxonomy ) {
	 ...
	 *
	 * @since 1.0.0
	 * @deprecated 1.0.0 Use {$taxonomy}_add_form instead.
	 *
	 * @param object $arg Optional arguments cast to an object.
	 */
