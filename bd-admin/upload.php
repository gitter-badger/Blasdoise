...
if ( !current_user_can('upload_files') )
	bd_die( __( 'You do not have permission to upload files.' ) );
...
if ( 'grid' === $mode ) {
	bd_enqueue_media();
	bd_enqueue_script( 'media-grid' );
	bd_enqueue_script( 'media' );

	$q = $_GET;
	// let JS handle this
	unset( $q['s'] );
	$vars = bd_edit_attachments_query_vars( $q );
	...
	bd_localize_script( 'media-grid', '_bdMediaGridSettings', array(
	...
	?>
	<div class="wrap" id="bd-media-grid" data-search="<?php _admin_search_query() ?>">
		<h1>
		<?php
		echo esc_html( $title );
		if ( current_user_can( 'upload_files' ) ) { ?>
...
$bd_list_table = _get_list_table('BD_Media_List_Table');
...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction ) {
		$post_ids = $bddb->get_col( "SELECT ID FROM $bddb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			bd_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			bd_media_attach_action( $_REQUEST['found_post_id'] );
			break;
		...
		case 'trash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id ) )
					bd_die( __( 'You are not allowed to move this post to the trash.' ) );
		...
		case 'untrash':
			...
			foreach ( (array) $post_ids as $post_id ) {
				if ( !current_user_can( 'delete_post', $post_id ) )
					bd_die( __( 'You are not allowed to move this post out of the trash.' ) );
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id_del ) {
				if ( !current_user_can( 'delete_post', $post_id_del ) )
					bd_die( __( 'You are not allowed to delete this post.' ) );

				if ( !bd_delete_attachment( $post_id_del ) )
...
if ( ! empty( $_GET['trashed'] ) && $trashed = absint( $_GET['trashed'] ) ) {
	...
	$message .= ' <a href="' . esc_url( bd_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __('Undo') . '</a>';
...
$messages[4] = __( 'Media attachment moved to the trash.' ) . ' <a href="' . esc_url( bd_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __( 'Undo' ) . '</a>';
