...
if ( current_user_can( 'switch_themes' ) && isset($_GET['action'] ) ) {
	if ( 'activate' == $_GET['action'] ) {
		...
		$theme = bd_get_theme( $_GET['stylesheet'] );
		if ( ! $theme->exists() || ! $theme->is_allowed() )
			bd_die( __( 'Cheatin&#8217; uh?' ), 403 );
		switch_theme( $theme->get_stylesheet() );
		bd_redirect( admin_url('themes.php?activated=true') );
		exit;
		...
		if ( $active->get( 'Template' ) == $_GET['stylesheet'] ) {
			bd_redirect( admin_url( 'themes.php?delete-active-child=true' ) );
		} else {
			delete_theme( $_GET['stylesheet'] );
			bd_redirect( admin_url( 'themes.php?deleted=true' ) );
		}
		exit;
	}
}
...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview  = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your Blasdoise installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf( __('If you would like to see more themes to choose from, click on the &#8220;Add New&#8221; button and you will be able to browse or search for additional themes from the Blasdoise Theme Directory. Themes in the Blasdoise Theme Directory are designed and developed by third parties, and are compatible with the license Blasdoise uses. Oh, and they&#8217;re free!') ) . '</p>';
	}
...
if ( current_user_can( 'switch_themes' ) ) {
	...
	$themes = bd_prepare_themes_for_js( array( bd_get_theme() ) );
}
bd_reset_vars( array( 'theme', 'search' ) );
	...
	if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
		foreach ( (array) $submenu['themes.php'] as $item) {
			...
			if ( !empty($submenu[$item[2]]) ) {
				...
				if ( file_exists(BD_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}") || !empty($menu_hook))
				...
				if ( file_exists( ABSPATH . "bd-admin/$menu_file" ) ) {
...
if ( ! is_multisite() && current_user_can('edit_themes') && $broken_themes = bd_get_themes( array( 'errors' => true ) ) ) {
?>
	...
	<?php foreach ( $broken_themes as $broken_theme ) : ?>
		<tr>
			...
			if ( $can_delete ) {
				...
				$delete_url = bd_nonce_url( $delete_url, 'delete-theme_' . $stylesheet );
				?>
...
<script id="tmpl-theme-single" type="text/template">
	...
	<div class="theme-wrap">
		<div class="theme-header">
			<button class="left basicons basicons-no"><span class="screen-reader-text"><?php _e( 'Show previous theme' ); ?></span></button>
			<button class="right basicons basicons-no"><span class="screen-reader-text"><?php _e( 'Show next theme' ); ?></span></button>
			<button class="close basicons basicons-no"><span class="screen-reader-text"><?php _e( 'Close details dialog' ); ?></span></button>
		</div>
