 ...
 *
 * These functions are meant to live inside of the Blasdoise loop.
 *
 * @package Blasdoise
 * @subpackage Template
 */
 ...
 * @param int|BD_Post  $post_id    Post ID or BD_Post object. Default current post.
...
function get_comment_class( $class = '', $comment_id = null, $post_id = null ) {
	 ...
	 * @param int|BD_Post $post_id    The post ID or BD_Post object.
	 */
...
function get_comment_link( $comment = null, $args = array() ) {
	global $bd_rewrite, $in_comment_loop;
	...
	if ( $args['per_page'] ) {
		...
		if ( $bd_rewrite->using_permalinks() )
			$link = user_trailingslashit( trailingslashit( get_permalink( $comment->comment_post_ID ) ) . $bd_rewrite->comments_pagination_base . '-' . $args['page'], 'comment' );
		else
...
function get_comments_link( $post_id = 0 ) {
	 ...
	 * @param int|BD_Post $post_id       Post ID or BD_Post object.
	 */
...
function get_trackback_url() {
	if ( '' != get_option('permalink_structure') )
		...
		$tb_url = get_option('siteurl') . '/bd-trackback.php?p=' . get_the_ID();
...
function trackback_url( $deprecated_echo = true ) {
	if ( $deprecated_echo !== true )
		_deprecated_argument( __FUNCTION__, '1.0', __('Use <code>get_trackback_url()</code> instead if you do not want the value echoed.') );
	if ( $deprecated_echo )
...
function trackback_rdf( $deprecated = '' ) {
	...
	the_permalink();
	echo '"'."\n";
	echo '    dc:title="'.str_replace('--', '&#x2d;&#x2d;', bdtexturize(strip_tags(get_the_title()))).'"'."\n";
 ...
 * @since 1.0.0
 */
function bd_comment_form_unfiltered_html_nonce() {
	$post = get_post();
	$post_id = $post ? $post->ID : 0;

	if ( current_user_can( 'unfiltered_html' ) ) {
		bd_nonce_field( 'unfiltered-html-comment_' . $post_id, '_bd_unfiltered_html_comment_disabled', false );
		echo "<script>(function(){if(window===window.parent){document.getElementById('_bd_unfiltered_html_comment_disabled').name='_bd_unfiltered_html_comment';}})();</script>\n";
	}
}
 ...
 *
 * Uses the Blasdoise database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the Blasdoise process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $bd_query, $withcomments, $post, $bddb, $id, $comment, $user_login, $user_ID, $user_identity, $overridden_cpage;
	 ...
	 */
	$commenter = bd_get_current_commenter();
	 ...
	 * @param int   $post_ID  Post ID.
	 */
	$bd_query->comments = apply_filters( 'comments_array', $comments, $post->ID );
	$comments = &$bd_query->comments;
	$bd_query->comment_count = count($bd_query->comments);

	if ( $separate_comments ) {
		$bd_query->comments_by_type = separate_comments($comments);
		$comments_by_type = &$bd_query->comments_by_type;
	}
	...
	elseif ( file_exists( TEMPLATEPATH . $file ) )
		...
		require( ABSPATH . BDINC . '/theme-compat/comments.php');
}
 ...
 *
 * @global string $bdcommentspopupfile  The URL to use for the popup window.
 * @global int    $bdcommentsjavascript Whether to use JavaScript. Set when function is called.
 *
 * @since 1.0.0
 *
...
function comments_popup_script( $width = 400, $height = 400, $file = '' ) {
	global $bdcommentspopupfile, $bdcommentsjavascript;

	if (empty ($file)) {
		$bdcommentspopupfile = '';  // Use the index.
	} else {
		$bdcommentspopupfile = $file;
	}

	$bdcommentsjavascript = 1;
	$javascript = "<script type='text/javascript'>\nfunction bdopen (macagna) {\n    window.open(macagna, '_blank', 'width=$width,height=$height,scrollbars=yes,status=yes');\n}\n</script>\n";
	echo $javascript;
}
...
function comments_popup_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	echo '<a href="';
	if ( $bdcommentsjavascript ) {
		if ( empty( $bdcommentspopupfile ) )
			$home = home_url();
		else
			$home = get_option('siteurl');
		echo $home . '/' . $bdcommentspopupfile . '?comments_popup=' . $id;
		echo '" onclick="bdopen(this.href); return false"';
 ...
 * @param int|BD_Post $post    Post ID or BD_Post object the comment is going to be displayed on.
...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	 ...
	 * @param BD_Post $post    The {@see BD_Post} object.
	 */
	$args = apply_filters( 'comment_reply_link_args', $args, $comment, $post );

	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf( '<a rel="nofollow" class="comment-reply-login" href="%s">%s</a>',
			esc_url( bd_login_url( get_permalink() ) ),
			$args['login_text']
		);
	} else {
 ...
 * }
 * @param int|BD_Post $post    Optional. Post ID or BD_Post object the comment is going to be displayed on.
 *                             Default current post.
...
function get_post_reply_link($args = array(), $post = null) {
	 ...
	 * @param int|BD_Post $post      The post ID or BD_Post object.
	 */
 ...
 */
function bd_list_comments( $args = array(), $comments = null ) {
	global $bd_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	 ...
	 */
	$r = apply_filters( 'bd_list_comments_args', $r );
	...
	if ( null !== $comments ) {
		...
		if ( empty($bd_query->comments) )
			return;
		if ( 'all' != $r['type'] ) {
			if ( empty($bd_query->comments_by_type) )
				$bd_query->comments_by_type = separate_comments($bd_query->comments);
			if ( empty($bd_query->comments_by_type[$r['type']]) )
				return;
			$_comments = $bd_query->comments_by_type[$r['type']];
		} else {
			$_comments = $bd_query->comments;
		}
	}
	...
	$bd_query->max_num_comment_pages = $walker->max_pages;
 ...
 * }
 * @param int|BD_Post $post_id Post ID or BD_Post object to generate the form for. Default current post.
 */
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.' ), bd_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
		/** This filter is documented in bd-includes/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>' ), get_edit_user_link(), $user_identity, bd_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
	 ...
	 */
	$args = bd_parse_args( $args, apply_filters( 'comment_form_defaults', $defaults ) );
		...
		if ( comments_open( $post_id ) ) : ?>
			...
			<div id="respond" class="comment-respond">
				...
				<?php if ( get_option( 'comment_registration' ) && !is_user_logged_in() ) : ?>
					...
					?>
				<?php else : ?>
					<form action="<?php echo site_url( '/bd-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
						<?php
						/**
