 ...
 *
 * @param string|BD_Screen $screen The screen you want the headers for
 ...
 * @since 1.0.0
 *
 * @param string|BD_Screen $screen The screen you want the hidden columns for
 * @return array
 */
...
function meta_box_prefs( $screen ) {
	...
	if ( empty($bd_meta_boxes[$screen->id]) )
		return;

	$hidden = get_hidden_meta_boxes($screen);

	foreach ( array_keys( $bd_meta_boxes[ $screen->id ] ) as $context ) {
		foreach ( array( 'high', 'core', 'default', 'low' ) as $priority ) {
			if ( ! isset( $bd_meta_boxes[ $screen->id ][ $context ][ $priority ] ) ) {
				continue;
			}
			foreach ( $bd_meta_boxes[ $screen->id ][ $context ][ $priority ] as $box ) {
...
function get_hidden_meta_boxes( $screen ) {
	...
	if ( $use_defaults ) {
		 ...
		 * @param BD_Screen $screen BD_Screen object of the current screen.
		 */
 ...
 *
 * @return BD_Screen Current screen object
 */
...
function set_current_screen( $hook_name = '' ) {
	BD_Screen::get( $hook_name )->set_current_screen();
}
...
final class BD_Screen {
	 ...
	 *
	 * @param string|BD_Screen $hook_name Optional. The hook name (also known as the hook suffix) used to determine the screen.
	 * 	                                  Defaults to the current $hook_suffix global.
	 * @return BD_Screen Screen object.
	 */
	public static function get( $hook_name = '' ) {
		if ( $hook_name instanceof BD_Screen ) {
			return $hook_name;
		}
		...
		if ( ! $post_type && $hook_name ) {
			...
			if ( defined( 'BD_NETWORK_ADMIN' ) && BD_NETWORK_ADMIN )
				$in_admin = 'network';
			elseif ( defined( 'BD_USER_ADMIN' ) && BD_USER_ADMIN )
				$in_admin = 'user';
			else
				$in_admin = 'site';
		}
	...
	public function set_current_screen() {
		 ...
		 * @since 1.0.0
		 *
		 * @param BD_Screen $current_screen Current BD_Screen object.
		 */
	 ...
	 * @static
	 *
	 * @param BD_Screen $screen A screen object.
	...
	public function render_screen_meta() {
		 ...
		 * @param BD_Screen $this            Current BD_Screen instance.
		 */
	...
	public function show_screen_options() {
		...
		$show_screen = ! empty( $bd_meta_boxes[ $this->id ] ) || $columns || $this->get_option( 'per_page' );
		 ...
		 * @param BD_Screen $this            BD_Screen object.
		 */
	...
	public function render_screen_options() {
		...
		<?php if ( isset( $bd_meta_boxes[ $this->id ] ) || $this->get_option( 'per_page' ) || ( $columns && empty( $columns['_title'] ) ) ) : ?>
			<h5><?php _e( 'Show on screen' ); ?></h5>
		<?php
		endif;

		if ( isset( $bd_meta_boxes[ $this->id ] ) ) : ?>
				...
				<?php
					...
					if ( 'dashboard' === $this->id && has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) {
						if ( isset( $_GET['welcome'] ) ) {
							...
							if ( 2 == $welcome_checked && bd_get_current_user()->user_email != get_option( 'admin_email' ) )
								$welcome_checked = false;
						}
						echo '<label for="bd_welcome_panel-hide">';
						echo '<input type="checkbox" id="bd_welcome_panel-hide"' . checked( (bool) $welcome_checked, true, false ) . ' />';
		...
		?>
		<div><?php bd_nonce_field( 'screen-options-nonce', 'screenoptionnonce', false ); ?></div>
		</form>
		</div>
		<?php
	}
	...
	public function render_per_page_options() {
		...
		if ( 'edit_comments_per_page' == $option ) {
			...
			/** This filter is documented in bd-admin/includes/class-bd-comments-list-table.php */
			$per_page = apply_filters( 'comments_per_page', $per_page, $comment_status );
		} elseif ( 'categories_per_page' == $option ) {
			/** This filter is documented in bd-admin/includes/class-bd-terms-list-table.php */
			$per_page = apply_filters( 'edit_categories_per_page', $per_page );
		} else {
			/** This filter is documented in bd-admin/includes/class-bd-list-table.php */
		...
		<div class="screen-options">
			<?php if ( $per_page_label ) : ?>
				...
				<input type="number" step="1" min="1" max="999" class="screen-per-page" name="bd_screen_options[value]"
					id="<?php echo esc_attr( $option ); ?>" maxlength="3"
					value="<?php echo esc_attr( $per_page ); ?>" />
			<?php endif;

			echo get_submit_button( __( 'Apply' ), 'button', 'screen-options-apply', false ); ?>
			<input type="hidden" name="bd_screen_options[option]" value="<?php echo esc_attr( $option ); ?>" />
		</div>
		<?php
	}
}
