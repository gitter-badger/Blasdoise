...
bd_reset_vars(array('action', 'option_page'));
...
if ( is_multisite() ) {
	if ( ! empty($_GET[ 'adminhash' ] ) ) {
		...
		bd_redirect( admin_url( $redirect ) );
		exit;
	} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' == $_GET['dismiss'] ) {
		delete_option( 'adminhash' );
		delete_option( 'new_admin_email' );
		bd_redirect( admin_url( 'options-general.php?updated=true' ) );
		exit;
	}
}
...
$whitelist_options = array(
	'general' => array( 'blogname', 'blogdescription', 'gmt_offset', 'date_format', 'time_format', 'start_of_week', 'timezone_string', 'BDLANG' ),
...
if ( !is_multisite() ) {
	...
	if ( get_option( 'upload_url_path' ) || ( get_option('upload_path') != 'bd-content/uploads' && get_option('upload_path') ) ) {
...
if ( 'update' == $action ) {
	...
	if ( !isset( $whitelist_options[ $option_page ] ) )
		bd_die( __( '<strong>ERROR</strong>: options page not found.' ) );

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! is_super_admin() )
			bd_die( __( 'You do not have sufficient permissions to modify unregistered settings for this site.' ) );
		$options = explode( ',', bd_unslash( $_POST[ 'page_options' ] ) );
	} else {
	...
	if ( 'general' == $option_page ) {
		...
		if ( !empty($_POST['date_format']) && isset($_POST['date_format_custom']) && '\c\u\s\t\o\m' == bd_unslash( $_POST['date_format'] ) )
			$_POST['date_format'] = $_POST['date_format_custom'];
		if ( !empty($_POST['time_format']) && isset($_POST['time_format_custom']) && '\c\u\s\t\o\m' == bd_unslash( $_POST['time_format'] ) )
		...
		if ( ! empty( $_POST['BDLANG'] ) && ( ! is_multisite() || is_super_admin() ) ) { // @todo: Skip if already installed
			require_once( ABSPATH . 'bd-admin/includes/translation-install.php' );

			if ( bd_can_install_language_pack() ) {
				$language = bd_download_language_pack( $_POST['BDLANG'] );
				if ( $language ) {
					$_POST['BDLANG'] = $language;
				}
			}
		}
	}
	...
	if ( $options ) {
		foreach ( $options as $option ) {
			if ( $unregistered )
				_deprecated_argument( 'options.php', '1.0', sprintf( __( 'The <code>%1$s</code> setting is unregistered. Unregistered settings are deprecated.' ), $option, $option_page ) );
		...
		// Switch translation in case BDLANG was changed.
		$language = get_option( 'BDLANG' );
		if ( $language ) {
	 ...
	 */
	$goback = add_query_arg( 'settings-updated', 'true',  bd_get_referer() );
	bd_redirect( $goback );
	exit;
}
...
<div class="wrap">
  ...
  <?php bd_nonce_field('options-options') ?>
  <input type="hidden" name="action" value="update" />
  <input type="hidden" name="option_page" value="options" />
  <table class="form-table">
<?php
$options = $bddb->get_results( "SELECT * FROM $bddb->options ORDER BY option_name" );
