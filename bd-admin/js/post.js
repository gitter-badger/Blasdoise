/* global postL10n, ajaxurl, bdAjax, setPostThumbnailL10n, postboxes, pagenow, tinymce, alert, deleteUserSetting */
/* global theList:true, theExtraList:true, getUserSetting, setUserSetting, commentReply */

var commentsBox, BDSetThumbnailHTML, BDSetThumbnailID, BDRemoveThumbnail, bdtitlehint, makeSlugeditClickable, editPermalink;
	...
	get : function(total, num) {
		...
		$.post(ajaxurl, data,
			function(r) {
				...
				$('#the-comment-list').append('<tr><td colspan="2">'+bdAjax.broken+'</td></tr>');
			}
		);
...
BDSetThumbnailHTML = function(html){
	$('.inside', '#postimagediv').html(html);
};

BDSetThumbnailID = function(id){
...
BDRemoveThumbnail = function(nonce){
	$.post(ajaxurl, {
...
(function($) {
	...
	$(document).on( 'heartbeat-send.bd-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#bd-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_bdnonce').val() ) {
				data['bd-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.bd-refresh-nonces', function( e, data ) {
		var nonces = data['bd-refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('bd-tab-first') && e.shiftKey ) {
			$(this).find('.bd-tab-last').focus();
			e.preventDefault();
		} else if ( target.hasClass('bd-tab-last') && ! e.shiftKey ) {
			$(this).find('.bd-tab-first').focus();
			e.preventDefault();
		}
	}).filter(':visible').find('.bd-tab-first').focus();

	// Set the heartbeat interval to 15 sec. if post lock dialogs are enabled
	if ( bd.heartbeat && $('#post-lock-dialog').length ) {
		bd.heartbeat.interval( 15 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#bd-preview'),
			target = $this.attr('target') || 'bd-preview',
		...
		if ( bd.autosave ) {
			bd.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! bd.heartbeat || ! bd.heartbeat.hasConnectionError() ) {
	...
	$(window).on( 'beforeunload.edit-post', function() {
		...
		if ( ( editor && ! editor.isHidden() && editor.isDirty() ) ||
			( bd.autosave && bd.autosave.server.postChanged() ) ) {
		...
		$.ajax({
			...
			async: false,
			data: {
				action: 'bd-remove-post-lock',
				_bdnonce: $('#_bdnonce').val(),
				post_ID: $('#post_ID').val(),
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see bd-lists.js
		...
		$('#' + taxonomy + '-add-toggle').click( function() {
			$('#' + taxonomy + '-adder').toggleClass( 'bd-hidden-children' );
	...
	if ( $('#postcustom').length ) {
		$( '#the-list' ).bdList( { addAfter: function() {
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'bd-editor-expand' ) ) {
				return;
			}
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#bd-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.bd-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.bd-editor-resize', dragging )
				.on( 'mouseup.bd-editor-resize mouseleave.bd-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.bd-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.bd-autosave', function( event ) {
		if ( event.which === 83 ) {
			if ( event.shiftKey || event.altKey || ( isMac && ( ! event.metaKey || event.ctrlKey ) ) || ( ! isMac && ! event.ctrlKey ) ) {
				return;
			}

			bd.autosave && bd.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
});
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#bd-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new bd.utils.WordCounter() );
