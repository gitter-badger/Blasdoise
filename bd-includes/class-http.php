 ...
 *
 * Standardizes the HTTP requests for Blasdoise. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * Blasdoise HTTP Class for managing HTTP Transports and making HTTP requests.
 *
...
class BD_Http {
	 ...
	 *                                             Default Blasdoise/' . $bd_version . '; ' . get_bloginfo( 'url' ).
	 *     @type bool         $reject_unsafe_urls  Whether to pass URLs through {@see bd_http_validate_url()}.
	 *                                             Default false.
	 ...
	 *                                             Default ABSPATH . BDINC . '/certificates/ca-bundle.crt'.
	 *     @type bool         $stream              Whether to stream to a file. If set to true and no filename was
	 *                                             given, it will be droped it in the BD temp dir and its name will
	 ...
	 *
	 * }
	 * @return array|BD_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A BD_Error instance upon error.
	 */
	public function request( $url, $args = array() ) {
		...
		$defaults = array(
			 ...
			 * @since 1.0.0
			 *
			 * @param string $user_agent Blasdoise user agent string.
			 */
			'user-agent' => apply_filters( 'http_headers_useragent', 'Blasdoise/' . $bd_version . '; ' . get_bloginfo( 'url' ) ),
			/**
			 * Filter whether to pass URLs through bd_http_validate_url() in an HTTP request.
			 *
			 * @since 1.0.0
			 *
			 * @param bool $pass_url Whether to pass URLs through bd_http_validate_url().
			 *                       Default false.
			 */
			...
			'decompress' => true,
			'sslverify' => true,
			'sslcertificates' => ABSPATH . BDINC . '/certificates/ca-bundle.crt',
			'stream' => false,
			'filename' => null,
		...
		if ( function_exists( 'bd_kses_bad_protocol' ) ) {
			if ( $r['reject_unsafe_urls'] )
				$url = bd_http_validate_url( $url );
			if ( $url ) {
				$url = bd_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( empty( $url ) || empty( $arrURL['scheme'] ) )
			return new BD_Error('http_request_failed', __('A valid URL was not provided.'));

		if ( $this->block_request( $url ) )
			return new BD_Error( 'http_request_failed', __( 'User has blocked requests through HTTP.' ) );
		...
		// Determine if this request is to OUR install of Blasdoise.
		$homeURL = parse_url( get_bloginfo( 'url' ) );
		$r['local'] = 'localhost' == $arrURL['host'] || ( isset( $homeURL['host'] ) && $homeURL['host'] == $arrURL['host'] );
		unset( $homeURL );

		/*
		 * If we are streaming to a file but no filename was given drop it in the BD temp dir
		 * and pick its name using the basename of the $url.
		 */
		if ( $r['stream']  && empty( $r['filename'] ) ) {
			$r['filename'] = get_temp_dir() . bd_unique_filename( get_temp_dir(), basename( $url ) );
		}
		...
		if ( $r['stream'] ) {
			$r['blocking'] = true;
			if ( ! bd_is_writable( dirname( $r['filename'] ) ) )
				return new BD_Error( 'http_request_failed', __( 'Destination directory for file streaming does not exist or is not writable.' ) );
		}
		...
		if ( ! isset( $r['headers']['Accept-Encoding'] ) ) {
			if ( $encoding = BD_Http_Encoding::accept_encoding( $url, $r ) )
		...
		if ( ! empty( $r['cookies'] ) ) {
			$cookies_set = bd_list_pluck( $response['cookies'], 'name' );
	 ...
	 * @param string $url URL to Request
	 * @param array $args Request arguments
	 * @return array|BD_Error Array containing 'headers', 'body', 'response', 'cookies', 'filename'. A BD_Error instance upon error
	 */
	private function _dispatch_request( $url, $args ) {
		...
		if ( !$class )
			return new BD_Error( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		 ...
		 * @since 1.0.0
		 *
		 * @param array|BD_Error $response HTTP response or BD_Error object.
	...
	public static function processHeaders( $headers, $url = '' ) {
		...
		foreach ( (array) $headers as $tempheader ) {
			...
			if ( 'set-cookie' == $key )
				$cookies[] = new BD_Http_Cookie( $value, $url );
		}
	 ...
	 *
	 * If it's found, then it upgrades any basic name => value pairs to BD_Http_Cookie instances,
	...
	public static function buildCookieHeader( &$r ) {
		if ( ! empty($r['cookies']) ) {
			// Upgrade any name => value cookie pairs to BD_HTTP_Cookie instances.
			foreach ( $r['cookies'] as $name => $value ) {
				if ( ! is_object( $value ) )
					$r['cookies'][ $name ] = new BD_HTTP_Cookie( array( 'name' => $name, 'value' => $value ) );
			}
	 ...
	 *
	 * You block external URL requests by defining BD_HTTP_BLOCK_EXTERNAL as true in your bd-config.php
	 * file and this will only allow localhost and your blog to make requests. The constant
	 * BD_ACCESSIBLE_HOSTS will allow additional hosts to go through for requests. The format of the
	 * BD_ACCESSIBLE_HOSTS constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg *.blasdoise.com will allow for all subdomains of blasdoise.com to be contacted.
	 *
	 * @since 1.0.0
	 *
	...
	public function block_request($uri) {
		...
		if ( ! defined( 'BD_HTTP_BLOCK_EXTERNAL' ) || ! BD_HTTP_BLOCK_EXTERNAL )
			return false;
		...
		if ( !defined('BD_ACCESSIBLE_HOSTS') )
			return true;

		static $accessible_hosts = null;
		static $wildcard_regex = array();
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split('|,\s*|', BD_ACCESSIBLE_HOSTS);

			if ( false !== strpos(BD_ACCESSIBLE_HOSTS, '*') ) {
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		if ( ! $url_parts = BD_HTTP::parse_url( $url ) ) {
			return $maybe_relative_path;
		}

		if ( ! $relative_url_parts = BD_HTTP::parse_url( $maybe_relative_path ) ) {
			return $maybe_relative_path;
		}
	 ...
	 * @return false|object False if no redirect is present, a BD_HTTP or BD_Error result otherwise.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 )
			return new BD_Error( 'http_request_failed', __('Too many redirects.') );
		...
		$redirect_location = BD_HTTP::make_absolute_url( $redirect_location, $url );
		...
		return bd_remote_request( $redirect_location, $args );
	}
 ...
 * @since 1.0.0
 */
class BD_Http_Streams {
	/**
	 * Send a HTTP request to a URI using PHP Streams.
	 *
	 * @see BD_Http::request For default options descriptions.
	 *
	 * @since 1.0.0
	 *
	 * @access public
	...
	public function request($url, $args = array()) {
		...
		if ( false === $handle ) {
			...
			if ( $secure_transport && 0 === $connection_error && '' === $connection_error_str )
				return new BD_Error( 'http_request_failed', __( 'The SSL certificate for the host could not be verified.' ) );

			return new BD_Error('http_request_failed', $connection_error . ': ' . $connection_error_str );
		}
		...
		if ( $r['stream'] ) {
			if ( ! BD_DEBUG )
				...
				$stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $stream_handle )
				return new BD_Error( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
			...
			while ( ! feof($handle) && $keep_reading ) {
				...
				if ( ! $bodyStarted ) {
					...
					if ( strpos( $strResponse, "\r\n\r\n" ) ) {
						$process = BD_Http::processResponse( $strResponse );
						$bodyStarted = true;
						$block = $process['body'];
						unset( $strResponse );
						$process['body'] = '';
				...
				if ( $bytes_written_to_file != $this_block_size ) {
					...
					return new BD_Error( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
				}
		...
		$arrHeaders = BD_Http::processHeaders( $process['headers'], $url );
		...
		// Handle redirects.
		if ( false !== ( $redirect_response = BD_HTTP::handle_redirects( $url, $r, $response ) ) )
			return $redirect_response;

		// If the body was chunk encoded, then decode it.
		if ( ! empty( $process['body'] ) && isset( $arrHeaders['headers']['transfer-encoding'] ) && 'chunked' == $arrHeaders['headers']['transfer-encoding'] )
			$process['body'] = BD_Http::chunkTransferDecode($process['body']);

		if ( true === $r['decompress'] && true === BD_Http_Encoding::should_decode($arrHeaders['headers']) )
			$process['body'] = BD_Http_Encoding::decompress( $process['body'] );
	...
	public static function verify_ssl_certificate( $stream, $host ) {
		 ...
		 */
		$host_type = ( BD_HTTP::is_ip_address( $host ) ? 'ip' : 'dns' );
 ...
 * All code should make use of BD_HTTP directly through it's API.
 *
 * @see BD_HTTP::request
 *
 * @since 1.0.0
 * @deprecated 1.0.0 Please use BD_HTTP::request() directly
 */
class BD_HTTP_Fsockopen extends BD_HTTP_Streams {
	...
	public function request($url, $args = array()) {
		...
		if ( ! $r['blocking'] ) {
			...
			if ( $curl_error = curl_error( $handle ) ) {
				curl_close( $handle );
				return new BD_Error( 'http_request_failed', $curl_error );
			}
		...
		curl_exec( $handle );
		$theHeaders = BD_Http::processHeaders( $this->headers, $url );
		$theBody = $this->body;
			...
			if ( CURLE_WRITE_ERROR /* 23 */ == $curl_error ) {
				if ( ! $this->max_body_length || $this->max_body_length != $bytes_written_total ) {
					if ( $r['stream'] ) {
						...
						curl_close( $handle );
						return new BD_Error( 'http_request_failed', curl_error( $handle ) );
					}
				}
			} else {
				if ( $curl_error = curl_error( $handle ) ) {
		...
		if ( true === $r['decompress'] && true === BD_Http_Encoding::should_decode($theHeaders['headers']) )
			$theBody = BD_Http_Encoding::decompress( $theBody );
...
/**
 * Adds Proxy support to the Blasdoise HTTP API.
 *
 * There are caveats to proxy support. It requires that defines be made in the bd-config.php file to
 ...
 * <ol>
 * <li>BD_PROXY_HOST - Enable proxy support and host for connecting.</li>
 * <li>BD_PROXY_PORT - Proxy port for connection. No default, must be defined.</li>
 * <li>BD_PROXY_USERNAME - Proxy username, if it requires authentication.</li>
 * <li>BD_PROXY_PASSWORD - Proxy password, if it requires authentication.</li>
 * <li>BD_PROXY_BYPASS_HOSTS - Will prevent the hosts in this list from going through the proxy.
 * You do not need to have localhost and the blog host in this list, because they will not be passed
 * through the proxy. The list should be presented in a comma separated list, wildcards using * are supported, eg. *.blasdoise.com</li>
 * </ol>
 *
 * An example can be as seen below.
 *
 *     define('BD_PROXY_HOST', '192.168.84.101');
 *     define('BD_PROXY_PORT', '8080');
 *     define('BD_PROXY_BYPASS_HOSTS', 'localhost, www.example.com, *.blasdoise.com');
 *
 * @since 1.0.0
 */
class BD_HTTP_Proxy {
	...
	public function is_enabled() {
		return defined('BD_PROXY_HOST') && defined('BD_PROXY_PORT');
	}
	 ...
	 * @since 1.0.0
	 *
	 * @use BD_PROXY_USERNAME
	 * @use BD_PROXY_PASSWORD
	 *
	 * @return bool
	 */
	public function use_authentication() {
		return defined('BD_PROXY_USERNAME') && defined('BD_PROXY_PASSWORD');
	}
	...
	public function host() {
		if ( defined('BD_PROXY_HOST') )
			return BD_PROXY_HOST;
	...
	public function port() {
		if ( defined('BD_PROXY_PORT') )
			return BD_PROXY_PORT;
	...
	public function username() {
		if ( defined('BD_PROXY_USERNAME') )
			return BD_PROXY_USERNAME;
	...
	public function password() {
		if ( defined('BD_PROXY_PASSWORD') )
			return BD_PROXY_PASSWORD;
	...
	public function send_through_proxy( $uri ) {
		...
		if ( !defined('BD_PROXY_BYPASS_HOSTS') )
			return true;

		static $bypass_hosts = null;
		static $wildcard_regex = array();
		if ( null === $bypass_hosts ) {
			$bypass_hosts = preg_split('|,\s*|', BD_PROXY_BYPASS_HOSTS);

			if ( false !== strpos(BD_PROXY_BYPASS_HOSTS, '*') ) {
				$wildcard_regex = array();
 ...
 * already a BD_Http_Cookie() object, then they are turned into one.
 *
 * @todo The Blasdoise convention is to use underscores instead of camelCase for function and method
...
class BD_Http_Cookie {
	...
	public function getHeaderValue() {
		 ...
		 */
		return $this->name . '=' . apply_filters( 'bd_http_cookie_value', $this->value, $this->name );
	}

	/**
	 * Retrieve cookie header for usage in the rest of the Blasdoise HTTP API.
	 *
	 * @access public
	 * @since 1.0.0
	 *
...
class BD_Http_Encoding {
	...
	public static function accept_encoding( $url, $args ) {
		 ...
		 * @param array  $args HTTP request arguments.
		 */
		$type = apply_filters( 'bd_http_accept_encoding', $type, $url, $args );
