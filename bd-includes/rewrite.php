...
function add_rewrite_rule($regex, $redirect, $after = 'bottom') {
	global $bd_rewrite;
	$bd_rewrite->add_rule($regex, $redirect, $after);
}
...
function add_rewrite_tag( $tag, $regex, $query = '' ) {
	...
	$bd_rewrite->add_rewrite_tag( $tag, $regex, $query );
}
 ...
 *                       see {@link BD_Rewrite::add_permastruct()} for full details.
 */
function add_permastruct( $name, $struct, $args = array() ) {
	...
	$bd_rewrite->add_permastruct( $name, $struct, $args );
}
...
function add_feed($feedname, $function) {
	global $bd_rewrite;
	if ( ! in_array($feedname, $bd_rewrite->feeds) ) //override the file if it is
		$bd_rewrite->feeds[] = $feedname;
...
function add_rewrite_endpoint( $name, $places, $query_var = true ) {
	global $bd_rewrite;
	$bd_rewrite->add_endpoint( $name, $places, $query_var );
}
 ...
 * @return string
 */
function _bd_filter_taxonomy_base( $base ) {
	if ( !empty( $base ) ) {
 ...
 *
 * Permalinks of posts with numeric slugs can sometimes look to BD_Query::parse_query()
 ...
 * post permalink.
 *
 * Note that, since 1.0.0, bd_unique_post_slug() prevents the creation of post slugs
 ...
 *                          BD::parse_request(). Default empty array.
 * @return array Returns the original array of query vars, with date/post conflicts resolved.
 */
function bd_resolve_numeric_slug_conflicts( $query_vars = array() ) {
...
function url_to_postid( $url ) {
	...
	$rewrite = $bd_rewrite->bd_rewrite_rules();
	...
	if ( !$bd_rewrite->using_index_permalinks() )
		$url = str_replace( $bd_rewrite->index . '/', '', $url );
	...
	foreach ( (array)$rewrite as $match => $query) {
		...
		if ( preg_match("#^$match#", $request_match, $matches) ) {
			...
			if ( $bd_rewrite->use_verbose_page_rules && preg_match( '/pagename=\$matches\[([0-9]+)\]/', $query, $varmatch ) ) {
			...
			$query = addslashes(BD_MatchesMapRegex::apply($query, $matches));
			...
			foreach ( (array) $query_vars as $key => $value ) {
				if ( in_array( $key, $bd->public_query_vars ) ){
					$query[$key] = $value;
			...
			$query = bd_resolve_numeric_slug_conflicts( $query );
...
/**
 * Blasdoise Rewrite Component.
 *
 * The Blasdoise Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * Blasdoise Query class.
 *
 * The Rewrite along with BD class function as a front controller for Blasdoise.
...
class BD_Rewrite {
	 ...
	 * {@link BD_Rewrite::$index} with a trailing slash appended. Otherwise
	 * the root will be empty.
	 *
	 * @see BD_Rewrite::init()
	 * @see BD_Rewrite::using_index_permalinks()
	 * @since 1.0.0
	 * @var string
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to Blasdoise' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for Blasdoise into the .htaccess file.
	 *
	 * This is off by default, turning it on might print a lot of rewrite rules
	 * to the .htaccess file.
	 *
	 * @see BD_Rewrite::mod_rewrite_rules()
	 * @since 1.0.0
	 * @access public
	 * @var bool
	 */
	 ...
	 * set to true. Prior to Blasdoise 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells {@link BD::parse_request()} to check if a URL matching the
	 ...
	 * {@link BD_Rewrite::$rewritereplace} and are rewritten to the
	 * query variables listed in {@link BD_Rewrite::$queryreplace}.
	 *
	 ...
	 * of rewrite tags, see {@link BD_Rewrite::$rewritecode}.
	 *
	 * @since 1.0.0
	 * @access private
	 * @var array
	...
	/**
	 * Query variables that rewrite tags map to, see {@link BD_Rewrite::$rewritecode}.
	 *
	 * @since 1.0.0
	 * @access private
	 * @var array
	...
	public function page_uri_index() {
		...
		$pages = $bddb->get_results("SELECT ID, post_name, post_parent FROM $bddb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'");
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $bddb->get_results( $bddb->prepare( "SELECT ID, post_name, post_parent FROM $bddb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 *
	 * @see BD_Rewrite::$rewritecode
	 * @see BD_Rewrite::$rewritereplace
	 * @see BD_Rewrite::$queryreplace
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * The main BD_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of {@link BD_Rewrite::generate_rewrite_rules()} that
	 * allows for shorter list of parameters. See the method for longer
	 * description of what generating rewrite rules does.
	 *
	 * @uses BD_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * reference to the current BD_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*bd-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*bd-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*bd-signup.php$'] = $this->index . '?signup=true';
			$registration_pages['.*bd-activate.php$'] = $this->index . '?activate=true';
		}
		$registration_pages['.*bd-register.php$'] = $this->index . '?register=true'; // Deprecated
		...
		foreach ( $this->extra_permastructs as $permastructname => $struct ) {
			...
			if ( 'post_tag' == $permastructname ) {
				 ...
				 *
				 * @since 1.0.0
				 * @deprecated 1.0.0 Use 'post_tag_rewrite_rules' instead
				 *
		 ...
		 * @since 1.0.0
		 *
		 * @param BD_Rewrite $this Current BD_Rewrite instance, passed by reference.
		 */
	 ...
	 * BD_Rewrite::rewrite_rules()} is that this method stores the rewrite rules
	 ...
	 */
	public function bd_rewrite_rules() {
	 ...
	 *
	 * Will add the non_bd_rules property rules to the .htaccess file before
	 * the Blasdoise rewrite rules one.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function mod_rewrite_rules() {
		...
		//add in the rules that don't redirect to BD's index.php (and thus shouldn't be handled by BD at all)
		foreach ( (array) $this->non_bd_rules as $match => $query) {
		 ...
		 *
		 * @since 1.0.0
		 * @deprecated 1.0.0 Use the mod_rewrite_rules filter instead.
		 *
		 * @param string $rules mod_rewrite Rewrite rules formatted for .htaccess.
		 */
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="blasdoise" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	...
	public function add_rule($regex, $redirect, $after = 'bottom') {
		...
		if ( $front != $this->index ) { //it doesn't redirect to BD's index.php
	...
	public function add_external_rule($regex, $redirect) {
		$this->non_bd_rules[$regex] = $redirect;
	}
	 ...
	 * The new permastruct is added to the {@link BD_Rewrite::$extra_permastructs} array. When the
	 * rewrite rules are built by {@link BD_Rewrite::rewrite_rules()} all of these extra permastructs
	 * are passed to {@link BD_Rewrite::generate_rewrite_rules()} which transforms them into the
	 * regular expressions that many love to hate.
	 *
	 * The $args parameter gives you control over how {@link BD_Rewrite::generate_rewrite_rules()}
	 ...
	 * @param array  $args   Optional configuration for building the rules from the permalink structure:
	 *     - with_front (bool) - Should the structure be prepended with BD_Rewrite::$front? Default is true.
	 ...
	 * Remove rewrite rules and then recreate rewrite rules.
	 *
	 * Calls {@link BD_Rewrite::bd_rewrite_rules()} after removing the
	...
	public function flush_rules( $hard = true ) {
		...
		if ( ! did_action( 'bd_loaded' ) ) {
			add_action( 'bd_loaded', array( $this, 'flush_rules' ) );
	...
	public function init() {
		$this->extra_rules = $this->non_bd_rules = $this->endpoints = array();
	 ...
	 * {@link BD_Rewrite::init()} after the option is updated.
	 *
