...
if ( ! current_user_can('manage_sites') )
	bd_die(__('You do not have sufficient permissions to edit this site.'));
...
$referer = remove_query_arg( 'update', bd_get_referer() );
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'newuser':
			check_admin_referer( 'add-user', '_bdnonce_add-new-user' );
			$user = $_POST['user'];
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				$update = 'err_new';
			} else {
				$password = bd_generate_password( 12, false);
				$user_id = bdmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
	...
	bd_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit();
}
 ...
 */
if ( ! bd_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) )
...
/** This filter is documented in bd-admin/network/site-users.php */
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table">
		<tr>
			...
			<td><input type="text" class="regular-text bd-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
...
<form action="<?php echo network_admin_url('site-users.php?action=newuser'); ?>" id="newuser" method="post">
	...
	<?php bd_nonce_field( 'add-user', '_bdnonce_add-new-user' ) ?>
