...
if ( ! current_user_can( 'manage_sites' ) )
	bd_die( __( 'You do not have sufficient permissions to add sites to this network.' ) );
...
if ( isset($_REQUEST['action']) && 'add-site' == $_REQUEST['action'] ) {
	check_admin_referer( 'add-blog', '_bdnonce_add-blog' );

	if ( ! is_array( $_POST['blog'] ) )
		bd_die( __( 'Can&#8217;t create an empty site.' ) );
	...
	if ( ! is_subdomain_install() ) {
		...
		if ( in_array( $domain, $subdirectory_reserved_names ) )
			bd_die( sprintf( __('The following words are reserved for use by Blasdoise functions and cannot be used as blog names: <code>%s</code>' ), implode( '</code>, <code>', $subdirectory_reserved_names ) ) );
	}

	$title = $blog['title'];

	if ( empty( $domain ) )
		bd_die( __( 'Missing or invalid site address.' ) );

	if ( isset( $blog['email'] ) && '' === trim( $blog['email'] ) ) {
		bd_die( __( 'Missing email address.' ) );
	}

	$email = sanitize_email( $blog['email'] );
	if ( ! is_email( $email ) ) {
		bd_die( __( 'Invalid email address.' ) );
	}
	...
	if ( !$user_id ) { // Create a new user with a random password
		...
		$user_id = bdmu_create_user( $domain, $password, $email );
		if ( false === $user_id )
			bd_die( __( 'There was an error creating the user.' ) );
		else
			bd_new_user_notification( $user_id, null, 'both' );
	}

	$bddb->hide_errors();
	$id = bdmu_create_blog( $newdomain, $path, $title, $user_id , array( 'public' => 1 ), $current_site->id );
	$bddb->show_errors();
	if ( ! is_bd_error( $id ) ) {
...
Name: %3$s' ),
		...
		bd_mail( get_site_option('admin_email'), sprintf( __( '[%s] New Site Created' ), $current_site->site_name ), $content_mail, 'From: "Site Admin" <' . get_site_option( 'admin_email' ) . '>' );
		bdmu_welcome_notification( $id, $user_id, $password, $title, array( 'public' => 1 ) );
		bd_redirect( add_query_arg( array( 'update' => 'added', 'id' => $id ), 'site-new.php' ) );
		exit;
	} else {
		bd_die( $id->get_error_message() );
	}
}
...
<?php bd_nonce_field( 'add-blog', '_bdnonce_add-blog' ) ?>
	<table class="form-table">
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text bd-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" /></td>
		</tr>
		<tr class="form-field">
