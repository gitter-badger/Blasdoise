/* global tinymce, bdCookies, autosaveL10n, switchEditors */
// Back-compat
	...
	function autosave() {
		...
		function autosaveLocal() {
			...
			function checkStorage() {
				...
				try {
					window.sessionStorage.setItem( 'bd-test', test );
					result = window.sessionStorage.getItem( 'bd-test' ) === test;
					window.sessionStorage.removeItem( 'bd-test' );
				} catch(e) {}
			...
			function getStorage() {
				...
				if ( hasStorage && blog_id ) {
					stored_obj = sessionStorage.getItem( 'bd-autosave-' + blog_id );
			...
			function run() {
				...
				if ( $( '#bd-content-wrap' ).hasClass( 'tmce-active' ) ) {
				...
				$( 'form#post' ).on( 'submit.autosave-local', function() {
					...
					bdCookies.set( 'bd-saving-post', post_id + '-check', 24 * 60 * 60 );
				});
			}
			...
			function checkPost() {
				var content, post_title, excerpt, $notice,
					postData = getSavedPostData(),
					cookie = bdCookies.get( 'bd-saving-post' );

				if ( cookie === post_id + '-saved' ) {
					bdCookies.remove( 'bd-saving-post' );
			...
			function restorePost( postData ) {
				...
				if ( postData ) {
					...
					if ( editor && ! editor.isHidden() && typeof switchEditors !== 'undefined' ) {
						...
						editor.setContent( postData.content ? switchEditors.bdautop( postData.content ) : '' );
					} else {
		...
		function autosaveServer() {
			...
			function save() {
				...
				$document.trigger( 'bdcountwords', [ postData.content ] )
					.trigger( 'before-autosave', [ postData ] );

				postData._bdnonce = $( '#_bdnonce' ).val() || '';
			...
			$document.on( 'heartbeat-send.autosave', function( event, data ) {
				...
				if ( autosaveData ) {
					data.bd_autosave = autosaveData;
				}
			}).on( 'heartbeat-tick.autosave', function( event, data ) {
				if ( data.bd_autosave ) {
					response( data.bd_autosave );
				}
				...
				if ( 'timeout' === error || 603 === status ) {
					...
					if ( ! bd.autosave.local.hasStorage ) {
