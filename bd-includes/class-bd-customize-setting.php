<?php
/**
 * Blasdoise Customize Setting classes
 *
 * @package Blasdoise
...
class BD_Customize_Setting {
	 ...
	 * @since 1.0.0
	 *
	 * @param BD_Customize_Manager $manager
	 ...
	 *
	 * @see BD_Customize_Setting::preview()
	 * @since 1.0.0
	 * @var mixed
	 */
	...
	public function preview() {
		...
		switch( $this->type ) {
			...
			default :
				...
				/**
				 * Fires when the {@see BD_Customize_Setting::preview()} method is called for settings
				 ...
				 * @since 1.0.0
				 *
				 * @param BD_Customize_Setting $this {@see BD_Customize_Setting} instance.
				 */
	 ...
	 *
	 * @since 1.0.0
	 * @uses BD_Customize_Setting::multidimensional_replace()
	 *
	...
	final public function save() {
		...
		/**
		 * Fires when the BD_Customize_Setting::save() method is called.
		 *
	...
	public function sanitize( $value ) {
		 ...
		 * @param BD_Customize_Setting $this  BD_Customize_Setting instance.
		 */
	...
	protected function update( $value ) {
		switch( $this->type ) {
			...
			default :
				...
				/**
				 * Fires when the {@see BD_Customize_Setting::update()} method is called for settings
				 * not handled as theme_mods or options.
				 *
 ...
 * @since 1.0.0
 *
 * @see BD_Customize_Setting
 */
class BD_Customize_Filter_Setting extends BD_Customize_Setting {
 ...
 * @see BD_Customize_Setting
 */
final class BD_Customize_Header_Image_Setting extends BD_Customize_Setting {
 ...
 * @see BD_Customize_Setting
 */
final class BD_Customize_Background_Image_Setting extends BD_Customize_Setting {
 ...
 *
 * Subclass of BD_Customize_Setting to represent a nav_menu taxonomy term, and
 ...
 * @see BD_Customize_Setting
 */
class BD_Customize_Nav_Menu_Item_Setting extends BD_Customize_Setting {
	 ...
	 * @var array
	 *
	 * @see bd_setup_nav_menu_item()
	 */
	public $default = array(
		// The $menu_item_data for bd_update_nav_menu_item().
		'object_id'        => 0,
		...
		'status'           => 'publish',
		'original_title'   => '',
		'nav_menu_term_id' => 0, // This will be supplied as the $menu_id arg for bd_update_nav_menu_item().
	...
	/**
	 * Storage of pre-setup menu item to prevent wasted calls to bd_setup_nav_menu_item().
	 *
	 * @since 1.0.0
	 * @access protected
	 * @var array
	 ...
	 * @var int
	 *
	 * @see BD_Customize_Nav_Menu_Item_Setting::update()
	 * @see BD_Customize_Nav_Menu_Item_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by bd_update_nav_menu_item() when setting is updated.
	 *
	 * @since 1.0.0
	 * @access public
	 * @var BD_Error
	 ...
	 * @access public
	 *
	 * @param BD_Customize_Manager $manager Bootstrap Customizer instance.
	 * @param string               $id      An specific ID of the setting. Can be a
	 *                                      theme mod or option name.
	 ...
	 */
	public function __construct( BD_Customize_Manager $manager, $id, array $args = array() ) {
		if ( empty( $manager->nav_menus ) ) {
			throw new Exception( 'Expected BD_Customize_Manager::$nav_menus to be set.' );
		}
		...
		add_action( 'bd_update_nav_menu_item', array( $this, 'flush_cached_value' ), 10, 2 );
		...
		if ( $this->is_previewed && $this->_previewed_blog_id === get_current_blog_id() ) {
			...
			if ( $this->post_id > 0 ) {
				...
				if ( $post && self::POST_TYPE === $post->post_type ) {
					$value = (array) bd_setup_nav_menu_item( $post );
				}
			}
			...
			// Cache the value for future calls to avoid having to re-call bd_setup_nav_menu_item().
			$this->value = $value;
			$this->populate_value();
			$value = $this->value;
		}
	 ...
	 *
	 * Translates some properties added by bd_setup_nav_menu_item() and removes others.
	 *
	 * @since 1.0.0
	 * @access protected
	 *
	 * @see BD_Customize_Nav_Menu_Item_Setting::value()
	 */
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = bd_get_post_terms( $this->post_id, BD_Customize_Nav_Menu_Setting::TAXONOMY, array(
				'fields' => 'ids',
			) );
			if ( ! empty( $menus ) ) {
	 ...
	 * @access public
	 *
	 * @see BD_Customize_Manager::post_value()
	 */
	public function preview() {
		...
		add_filter( 'bd_get_nav_menu_items', array( $this, 'filter_bd_get_nav_menu_items' ), 10, 3 );

		$sort_callback = array( __CLASS__, 'sort_bd_get_nav_menu_items' );
		if ( ! has_filter( 'bd_get_nav_menu_items', $sort_callback ) ) {
			add_filter( 'bd_get_nav_menu_items', array( __CLASS__, 'sort_bd_get_nav_menu_items' ), 1000, 3 );
		}
	...
	/**
	 * Filter the bd_get_nav_menu_items() result to supply the previewed menu items.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @see bd_get_nav_menu_items()
	 *
	 ...
	 */
	public function filter_bd_get_nav_menu_items( $items, $menu, $args ) {
		...
		if ( $should_update ) {
			foreach ( $items as $item ) {
				if ( $item->db_id === $this->post_id ) {
					foreach ( get_object_vars( $this->value_as_bd_post_nav_menu_item() ) as $key => $value ) {
						$item->$key = $value;
					}
					$mutated = true;
				}
			...
			if ( ! $mutated ) {
				$items[] = $this->value_as_bd_post_nav_menu_item();
			}
		}
	...
	/**
	 * Re-apply the tail logic also applied on $items by bd_get_nav_menu_items().
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	 ...
	 * @return array Array of menu items,
	 */
	public static function sort_bd_get_nav_menu_items( $items, $menu, $args ) {
	...
	/**
	 * Get the value emulated into a BD_Post and set up as a nav_menu_item.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @return BD_Post With bd_setup_nav_menu_item() applied.
	 */
	public function value_as_bd_post_nav_menu_item() {
	 ...
	 * Sanitize an input.
	 *
	 * Note that parent::sanitize() erroneously does bd_unslash() on $value, but
	...
	public function sanitize( $menu_item_value ) {
		...
		$menu_item_value = bd_array_slice_assoc( $menu_item_value, array_keys( $default ) );
		...
		/** This filter is documented in bd-includes/class-bd-customize-setting.php */
	 ...
	 * @access protected
	 *
	 * @see bd_update_nav_menu_item()
	 *
	 * @param array|false $value The menu item array to update. If false, then the menu item will be deleted
	 *                           entirely. See BD_Customize_Nav_Menu_Item_Setting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = bd_delete_post( $this->post_id, true );

				if ( false === $r ) {
					$this->update_error  = new BD_Error( 'delete_failure' );
			...
			if ( $value['nav_menu_term_id'] < 0 ) {
				...
				if ( ! $nav_menu_setting || ! ( $nav_menu_setting instanceof BD_Customize_Nav_Menu_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new BD_Error( 'unexpected_nav_menu_setting' );
					return;
				}

				if ( false === $nav_menu_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new BD_Error( 'nav_menu_setting_failure' );
					return;
				}

				if ( $nav_menu_setting->previous_term_id !== intval( $value['nav_menu_term_id'] ) ) {
					$this->update_status = 'error';
					$this->update_error  = new BD_Error( 'unexpected_previous_term_id' );
					return;
				}
			...
			if ( $value['menu_item_parent'] < 0 ) {
				...
				if ( ! $parent_nav_menu_item_setting || ! ( $parent_nav_menu_item_setting instanceof BD_Customize_Nav_Menu_Item_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new BD_Error( 'unexpected_nav_menu_item_setting' );
					return;
				}

				if ( false === $parent_nav_menu_item_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new BD_Error( 'nav_menu_item_setting_failure' );
					return;
				}

				if ( $parent_nav_menu_item_setting->previous_post_id !== intval( $value['menu_item_parent'] ) ) {
					$this->update_status = 'error';
					$this->update_error  = new BD_Error( 'unexpected_previous_post_id' );
					return;
				}
	 ...
	 * @see BD_Customize_Nav_Menu_Item_Setting::update()
	 *
	 * @param array $data Additional information passed back to the 'saved' event on `bd.customize`.
 ...
 * @since 1.0.0
 *
 * @see bd_get_nav_menu_object()
 * @see BD_Customize_Setting
 */
class BD_Customize_Nav_Menu_Setting extends BD_Customize_Setting {
	 ...
	 * @var int
	 *
	 * @see BD_Customize_Nav_Menu_Setting::update()
	 * @see BD_Customize_Nav_Menu_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by bd_update_nav_menu_object() when setting is updated.
	 *
	 * @since 1.0.0
	 * @access public
	 * @var BD_Error
	...
	public function value() {
		if ( $this->is_previewed && $this->_previewed_blog_id === get_current_blog_id() ) {
			...
			if ( $this->term_id > 0 ) {
				$term = bd_get_nav_menu_object( $this->term_id );

				if ( $term ) {
					$value = bd_array_slice_assoc( (array) $term, array_keys( $this->default ) );
	...
	public function preview() {
		...
		add_filter( 'bd_get_nav_menus', array( $this, 'filter_bd_get_nav_menus' ), 10, 2 );
		add_filter( 'bd_get_nav_menu_object', array( $this, 'filter_bd_get_nav_menu_object' ), 10, 2 );
	...
	/**
	 * Filter the bd_get_nav_menus() result to ensure the inserted menu object is included, and the deleted one is removed.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @see bd_get_nav_menus()
	 *
	 * @param array $menus An array of menu objects.
	 * @param array $args  An array of arguments used to retrieve menu objects.
	 * @return array
	 */
	public function filter_bd_get_nav_menus( $menus, $args ) {
	 ...
	 * @var string
	 *
	 * @see BD_Customize_Nav_Menu_Setting::filter_bd_get_nav_menus()
	 * @see BD_Customize_Nav_Menu_Setting::_sort_menus_by_orderby()
	 */
	...
	/**
	 * Filter the bd_get_nav_menu_object() result to supply the previewed menu object.
	 *
	 ...
	 * @see bd_get_nav_menu_object()
	 *
	 * @param object|null $menu_obj Object returned by bd_get_nav_menu_object().
	 * @param string      $menu_id  ID of the nav_menu term. Requests by slug or name will be ignored.
	 * @return object|null
	 */
	public function filter_bd_get_nav_menu_object( $menu_obj, $menu_id ) {
		$ok = (
	...
	public function sanitize( $value ) {
		...
		$value = bd_array_slice_assoc( $value, array_keys( $default ) );

		$value['name']        = trim( esc_html( $value['name'] ) ); // This sanitization code is used in bd-admin/nav-menus.php.
	 ...
	 * @access protected
	 *
	 * @see bd_update_nav_menu_object()
	 *
	 * @param array|false $value {
	 *     The value to update. Note that slug cannot be updated via bd_update_nav_menu_object().
	...
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = bd_delete_nav_menu( $this->term_id );
			...
			// Insert or update menu.
			$menu_data = bd_array_slice_assoc( $value, array( 'description', 'parent' ) );
			$menu_data['menu-name'] = $value['name'];

			$menu_id = $is_placeholder ? 0 : $this->term_id;
			$r = bd_update_nav_menu_object( $menu_id, $menu_data );
			$original_name = $menu_data['menu-name'];
			$name_conflict_suffix = 1;
			while ( is_bd_error( $r ) && 'menu_exists' === $r->get_error_code() ) {
				$name_conflict_suffix += 1;
		...
		if ( 'inserted' === $this->update_status ) {
			...
			foreach ( array_keys( $this->manager->unsanitized_post_values() ) as $setting_id ) {
				...
				$widget_instance = $nav_menu_widget_setting->post_value(); // Note that this calls BD_Customize_Widgets::sanitize_widget_instance().
	 ...
	 * @access protected
	 *
	 * @see BD_Customize_Nav_Menu_Setting::filter_nav_menu_options()
	 * @see BD_Customize_Nav_Menu_Setting::update()
	 *
