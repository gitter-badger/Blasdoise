...
if ( isset($_REQUEST['action']) && 'update-site' == $_REQUEST['action'] && is_array( $_POST['option'] ) ) {
	...
	foreach ( (array) $_POST['option'] as $key => $val ) {
		...
		if ( $key === 0 || is_array( $val ) || in_array($key, $skip_options) )
			continue; // Avoids "0 is a protected BD option and may not be modified" error when edit blog options
 ...
 * @since 1.0.0
 */
	do_action( 'bdmu_update_blog_options' );
	restore_current_blog();
	bd_redirect( add_query_arg( array( 'update' => 'updated', 'id' => $id ), 'site-settings.php') );
	exit;
}
...
<form method="post" action="site-settings.php?action=update-site">
	...
	<table class="form-table">
		...
		$query = $bddb->prepare( $sql,
			$bddb->esc_like( '_' ) . '%',
			'%' . $bddb->esc_like( 'user_roles' )
		);
		$options = $bddb->get_results( $query );
		 ...
		 */
		do_action( 'bdmueditblogaction', $id );
		?>
	</table>
	<?php submit_button(); ?>
</form>
