/* global adminCommentsL10n, thousandsSeparator, list_args, QTags, ajaxurl, bdAjax */
...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			bdListsData = $( settings.target ).attr( 'data-bd-lists' );
		...
		if ( bdListsData.indexOf(':trash=1') != -1 )
			action = 'trash';
		else if ( bdListsData.indexOf(':spam=1') != -1 )
			action = 'spam';

		if ( action ) {
			id = bdListsData.replace(/.*?comment-([0-9]+).*/, '$1');
			el = $('#comment-' + id);
			...
			a = $('.undo a', '#undo-' + id);
			a.attr('href', 'comment.php?action=un' + action + 'comment&c=' + id + '&_bdnonce=' + settings.data._ajax_nonce);
			a.attr('data-bd-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
			a.attr('class', 'vim-z vim-destructive');
			$('.avatar', el).first().clone().prependTo('#undo-' + id + ' .' + action + '-undo-inside');

			a.click(function(){
				list.bdList.del(this);
	...
	delAfter = function( r, settings ) {
		...
		theList.get(0).bdList.add( theExtraList.children(':eq(0)').remove().clone() );
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see BD_Comments_List_Table::prepare_items() @ class-bd-comments-list-table.php
		} else {
			args.number = 1;
		...
		$.ajax({
			...
			success: function(response) {
				theExtraList.get(0).bdList.add( response.rows );
			}
		});
	};

	theExtraList = $('#the-extra-comment-list').bdList( { alt: '', delColor: 'none', addColor: 'none' } );
	theList = $('#the-comment-list').bdList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('bdListDelEnd', function(e, s){
			var bdListsData = $(s.target).attr('data-bd-lists'), id = s.element.replace(/[^0-9]+/g, '');

			if ( bdListsData.indexOf(':trash=1') != -1 || bdListsData.indexOf(':spam=1') != -1 )
...
commentReply = {
	...
	show : function(xml) {
		...
		r = bdAjax.parseAjaxResponse(xml);
		if ( r.errors ) {
			t.error({'responseText': bdAjax.broken});
			return false;
		}
...
$(document).ready(function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		...
		toggle_all = function() {
			$('#cb-select-all-1').data( 'bd-toggle', 1 ).trigger( 'click' ).removeData( 'bd-toggle' );
		};
