...
bd_reset_vars( array( 'url', 'return' ) );
$url = bd_unslash( $url );
$url = bd_validate_redirect( $url, home_url( '/' ) );
if ( $return ) {
	$return = bd_unslash( $return );
	$return = bd_validate_redirect( $return );
}
if ( ! $return ) {
	if ( $url ) {
		$return = $url;
 ...
 */
global $bd_scripts, $bd_customize;

$registered = $bd_scripts->registered;
$bd_scripts = new BD_Scripts;
$bd_scripts->registered = $registered;

add_action( 'customize_controls_print_scripts',        'print_head_scripts', 20 );
add_action( 'customize_controls_print_footer_scripts', '_bd_footer_scripts'     );
...
bd_enqueue_script( 'customize-controls' );
bd_enqueue_style( 'customize-controls' );
...
$body_class = 'bd-core-ui bd-customizer js';
...
$is_ios = bd_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] );
...
if ( $bd_customize->is_theme_active() ) {
...
<div class="bd-full-overlay expanded">
	<form id="customize-controls" class="wrap bd-full-overlay-sidebar">
		<div id="customize-header-actions" class="bd-full-overlay-header">
			<?php
			$save_text = $bd_customize->is_theme_active() ? __( 'Save &amp; Publish' ) : __( 'Save &amp; Activate' );
		...
		<div class="bd-full-overlay-sidebar-content" tabindex="-1">
			<div id="customize-info" class="accordion-section customize-info">
				<div class="accordion-section-title" aria-label="<?php esc_attr_e( 'Customizer Options' ); ?>">
					...
					?></span>
					<button class="customize-help-toggle basicons basicons-editor-help" aria-expanded="false"><span class="screen-reader-text"><?php _e( 'Help' ); ?></span></button>
				</div>
		...
		<div id="customize-footer-actions" class="bd-full-overlay-footer">
	...
	<div id="customize-preview" class="bd-full-overlay-main"></div>
	<?php

	// Render Panel, Section, and Control templates.
	$bd_customize->render_panel_templates();
	$bd_customize->render_section_templates();
	$bd_customize->render_control_templates();
	...
	$settings = array(
		'theme'    => array(
			'stylesheet' => $bd_customize->get_stylesheet(),
			'active'     => $bd_customize->is_theme_active(),
		),
		'url'      => array(
		...
		'sections' => array(),
		'nonce'    => array(
			'save'    => bd_create_nonce( 'save-customize_' . $bd_customize->get_stylesheet() ),
			'preview' => bd_create_nonce( 'preview-customize_' . $bd_customize->get_stylesheet() )
		),
		'autofocus' => array(),
	...
	foreach ( $bd_customize->settings() as $id => $setting ) {
	...
	foreach ( $bd_customize->controls() as $id => $control ) {
	...
	foreach ( $bd_customize->sections() as $id => $section ) {
	...
	foreach ( $bd_customize->panels() as $panel_id => $panel ) {
	...
	if ( isset( $_GET['autofocus'] ) ) {
		$autofocus = bd_unslash( $_GET['autofocus'] );
