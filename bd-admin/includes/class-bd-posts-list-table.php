 ...
 * @access private
 */
class BD_Posts_List_Table extends BD_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		if ( !current_user_can( $post_type_object->cap->edit_others_posts ) ) {
			...
			$this->user_posts_count = $bddb->get_var( $bddb->prepare( "
				SELECT COUNT( 1 ) FROM $bddb->posts
		...
		if ( 'post' == $post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
			...
			$this->sticky_posts_count = $bddb->get_var( $bddb->prepare( "SELECT COUNT( 1 ) FROM $bddb->posts WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND ID IN ($sticky_posts)", $post_type ) );
		}
	}
	...
	public function prepare_items() {
		global $avail_post_stati, $bd_query, $per_page, $mode;

		$avail_post_stati = bd_edit_posts_query();

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->screen->post_type ) && 'menu_order title' == $bd_query->query['orderby'] );

		$total_items = $this->hierarchical_display ? $bd_query->post_count : $bd_query->found_posts;

		$post_type = $this->screen->post_type;
		$per_page = $this->get_items_per_page( 'edit_' . $post_type . '_per_page' );

		/** This filter is documented in bd-admin/includes/post.php */
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts = bd_count_posts( $post_type, 'readable' );
		$class = '';
		$allposts = '';
...
<?php
		if ( 'top' == $which && !is_singular() ) {
			...
			if ( is_object_in_taxonomy( $this->screen->post_type, 'category' ) ) {
				...
				bd_dropdown_categories( $dropdown_options );
			}
...
<?php
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $bd_query, $per_page;

		if ( empty( $posts ) )
			$posts = $bd_query->posts;
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$bddb->update( $bddb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	...
	public function column_title( $post ) {
		...
		if ( $can_edit_post && $post->post_status != 'trash' ) {
			$lock_holder = bd_check_post_lock( $post->ID );
	...
	protected function handle_row_actions( $post, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( 'trash' == $post->post_status )
				$actions['untrash'] = "<a title='" . esc_attr__( 'Restore this item from the Trash' ) . "' href='" . bd_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . "'>" . __( 'Restore' ) . "</a>";
	...
	<?php endif; // $bulk
		...
		if ( post_type_supports( $screen->post_type, 'author' ) ) :
			...
			if ( is_super_admin() || current_user_can( $post_type_object->cap->edit_others_posts ) ) :
				...
				if ( $authors = bd_dropdown_users( $users_opt ) ) :
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name )?>-checklist">
				<?php bd_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ) ?>
			</ul>
	...
	<?php
		...
		bd_dropdown_pages( $dropdown_args );
	?>
			</label>
	...
	<?php
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( $bulk ) {
				 ...
				 * @param BD_Post $post_type   The post type slug.
				 */
	...
	?>
		<p class="submit inline-edit-save">
			...
			<?php if ( ! $bulk ) {
				bd_nonce_field( 'inlineeditnonce', '_inline_edit', false );
				?>
