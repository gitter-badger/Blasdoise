...
final class BD_Theme implements ArrayAccess {
	...
	/**
	 * Absolute path to the theme root, usually bd-content/themes
	 *
	 * @access private
	 * @var string
	 */
	...
	/**
	 * URL to the theme root, usually an absolute URL to bd-content/themes
	 *
	 * @access private
	 * var string
	 */
	 ...
	 *
	 * Default is false. Can be set with the bd_cache_themes_persistently filter.
	 *
	 * @static
	 * @access private
	 * @var bool
	...
	/**
	 * Constructor for BD_Theme.
	 *
	 * @global array $bd_theme_directories
	 *
	 * @param string $theme_dir Directory of the theme within the theme_root.
	 * @param string $theme_root Theme root.
	 * @param BD_Error|void $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			...
			self::$persistently_cache = apply_filters( 'bd_cache_themes_persistently', false, 'BD_Theme' );
			if ( self::$persistently_cache ) {
				bd_cache_add_global_groups( 'themes' );
				if ( is_int( self::$persistently_cache ) )
					self::$cache_expiration = self::$persistently_cache;
			} else {
				bd_cache_add_non_persistent_groups( 'themes' );
			}
		}
		...
		if ( ! in_array( $theme_root, (array) $bd_theme_directories ) && in_array( dirname( $theme_root ), (array) $bd_theme_directories ) ) {
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) )
				$this->errors = new BD_Error( 'theme_not_found', sprintf( __( 'The theme directory "%s" does not exist.' ), $this->stylesheet ) );
			else
				$this->errors = new BD_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing.' ) );
			...
			$this->headers['Name'] = $this->stylesheet;
			$this->errors = new BD_Error( 'theme_stylesheet_not_readable', __( 'Stylesheet is not readable.' ) );
			$this->template = $this->stylesheet;
			$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
			return;
		} else {
			...
			// Properly identify default themes that are inside a directory within bd-content/themes.
		...
		if ( ! $this->template && ! ( $this->template = $this->headers['Template'] ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet . '/index.php' ) ) {
				$this->errors = new BD_Error( 'theme_no_index', __( 'Template is missing.' ) );
				$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				return;
			}
		}
		...
		if ( ! is_array( $cache ) && $this->template != $this->stylesheet && ! file_exists( $this->theme_root . '/' . $this->template . '/index.php' ) ) {
			...
			// bd-content/themes/directory-of-themes/*
			...
			if ( '.' != $parent_dir && file_exists( $this->theme_root . '/' . $parent_dir . '/' . $this->template . '/index.php' ) ) {
				...
				$this->errors = new BD_Error( 'theme_no_parent', sprintf( __( 'The parent theme is missing. Please install the "%s" parent theme.' ), $this->template ) );
				$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				$this->parent = new BD_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template != $this->stylesheet ) {
			...
			if ( $_child instanceof BD_Theme && $_child->template == $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new BD_Error( 'theme_parent_invalid', sprintf( __( 'The "%s" theme is not a valid parent theme.' ), $_child->template ) );
				$_child->cache_add( 'theme', array( 'headers' => $_child->headers, 'errors' => $_child->errors, 'stylesheet' => $_child->stylesheet, 'template' => $_child->template ) );
				// The two themes actually reference each other with the Template header.
				if ( $_child->stylesheet == $this->template ) {
					$this->errors = new BD_Error( 'theme_parent_invalid', sprintf( __( 'The "%s" theme is not a valid parent theme.' ), $this->template ) );
					$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				}
				return;
			}
			// Set the parent. Pass the current instance so we can do the crazy checks above and assess errors.
			$this->parent = new BD_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}
	 ...
	 * @access public
	 *
	 * @return BD_Error|false BD_Error if there are errors, or false.
	 */
	public function errors() {
		return is_bd_error( $this->errors ) ? $this->errors : false;
	}
	 ...
	 * @access public
	 *
	 * @return BD_Theme|false Parent theme, or false if the current theme is not a child theme.
	 */
	 ...
	 * @return bool Return value from bd_cache_add()
	 */
	private function cache_add( $key, $data ) {
		return bd_cache_add( $key . '-' . $this->cache_hash, $data, 'themes', self::$cache_expiration );
	}
	...
	private function cache_get( $key ) {
		return bd_cache_get( $key . '-' . $this->cache_hash, 'themes' );
	}
	...
	public function cache_delete() {
		foreach ( array( 'theme', 'screenshot', 'headers', 'page_templates' ) as $key )
			bd_cache_delete( $key . '-' . $this->cache_hash, 'themes' );
	 ...
	 * get_template() takes into account where Blasdoise actually located the theme and
	...
	private function sanitize_header( $header, $value ) {
		switch ( $header ) {
			...
			case 'Name' :
				...
				$value = bd_kses( $value, $header_tags );
				break;
			case 'Author' :
			...
			case 'Description' :
				...
				$value = bd_kses( $value, $header_tags_with_a );
				break;
			case 'ThemeURI' :
			case 'AuthorURI' :
	 ...
	 *
	 * This is typically the absolute path to bd-content/themes.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * This is typically the absolute URL to bd-content/themes. This forms the basis
	 * for all other URLs returned by BD_Theme, so we pass it to the public function
	...
	public function get_page_templates( $post = null ) {
		 ...
		 * @param BD_Theme     $this           The theme object.
		 * @param BD_Post|null $post           The post being edited, provided for context, or null.
		 */
	...
	public static function sort_by_name( &$themes ) {
		if ( 0 === strpos( get_locale(), 'en_' ) ) {
			uasort( $themes, array( 'BD_Theme', '_name_sort' ) );
		} else {
			uasort( $themes, array( 'BD_Theme', '_name_sort_i18n' ) );
		}
	}
