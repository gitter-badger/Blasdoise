...
( function( window, bd, shortcode, $ ) {
	...
	bd.mce.views = {
		 ...
		 * @param {Object} extend An object to extend bd.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = bd.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		...
		setMarkers: function( content ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: '<p data-bdview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-bdview-marker=/g, '<p data-bdview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		 ...
		 *
		 * @return {bd.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {bd.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-bdview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-bdview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	bd.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	bd.mce.View.extend = Backbone.View.extend;

	_.extend( bd.mce.View.prototype, {
		...
		unbind: function() {
			this.getNodes( function( editor, node, contentNode ) {
				...
				$( node ).trigger( 'bd-mce-view-unbind' );
			}, true );
		},
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-bdview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
					...
					.each( function() {
						callback.call( self, editor, this, $( this ).find( '.bdview-content' ).get( 0 ) );
					} );
			} );
		},
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-bdview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== this.text ) {
					editor.dom.setAttrib( node, 'data-bdview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="bdview-wrap" data-bdview-text="' + this.encodedText + '" data-bdview-type="' + this.type + '">' +
						'<p class="bdview-selection-before">\u00a0</p>' +
						'<div class="bdview-body" contenteditable="false">' +
							'<div class="bdview-content bdview-type-' + this.type + '"></div>' +
						'</div>' +
						'<p class="bdview-selection-after">\u00a0</p>' +
					'</div>'
				);
				...
				if ( selected ) {
					editor.bd.setViewCursor( false, $viewNode[0] );
				}
			} );
		},
			...
			if ( _.isObject( content ) && content.body.indexOf( '<script' ) !== -1 ) {
				...
				this.getNodes( function( editor, node, contentNode ) {
					...
					if ( content.indexOf( '<iframe' ) !== -1 ) {
						content += '<div class="bdview-overlay"></div>';
					}
			...
			this.getNodes( function( editor, node, contentNode ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/blasdoise/bd-content.css' ) === -1 ) {
				...
				setTimeout( function() {
					...
					dom.add( contentNode, 'div', { 'class': 'bdview-overlay' } );
							...
							'<head>' +
								...
								'<style>' +
									'html {' +
										...
										'margin: 0;' +
									'}' +
									'body#bdview-iframe-sandbox {' +
										'background: transparent;' +
										'padding: 1px 0 !important;' +
										'margin: -1px 0 0 !important;' +
									'}' +
									'body#bdview-iframe-sandbox:before,' +
									'body#bdview-iframe-sandbox:after {' +
										'display: none;' +
										'content: "";' +
									'}' +
								'</style>' +
							'</head>' +
							'<body id="bdview-iframe-sandbox" class="' + bodyClasses + '">' +
								body +
							'</body>' +
						'</html>'
					);
					...
					if ( MutationObserver ) {
						...
						$( node ).one( 'bd-mce-view-unbind', function() {
							observer.disconnect();
						} );
					} else {
						for ( i = 1; i < 6; i++ ) {
					...
					editor.on( 'bd-body-class-change', classChange );

					$( node ).one( 'bd-mce-view-unbind', function() {
						editor.off( 'bd-body-class-change', classChange );
					} );
		...
		setLoader: function() {
			this.setContent(
				'<div class="loading-placeholder">' +
					'<div class="basicons basicons-admin-media"></div>' +
					'<div class="bdview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		 ...
		 * @param {String} basicon A basicon ID. Optional.
		 */
		setError: function( message, basicon ) {
			this.setContent(
				'<div class="bdview-error">' +
					'<div class="basicons basicons-' + ( basicon || 'no' ) + '"></div>' +
					'<p>' + message + '</p>' +
				'</div>'
			);
		},
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-bdview-text', encodeURIComponent( text ) );
					bd.mce.views.createInstance( type, text, match.options, force ).render();
					editor.focus();
		 ...
		 */
		remove: function( editor, node ) {
			this.unbindNode.call( this, editor, node, $( node ).find( '.bdview-content' ).get( 0 ) );
			$( node ).trigger( 'bd-mce-view-unbind' );
			editor.dom.remove( node );
			editor.focus();
		}
	} );
} )( window, window.bd, window.bd.shortcode, window.jQuery );

/*
 * The Blasdoise core TinyMCE views.
...
( function( window, views, media, $ ) {
	...
	av = _.extend( {}, base, {
		...
		initialize: function() {
			...
			this.getEditors( function( editor ) {
				editor.on( 'bdview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.bdview-sandbox', content ).get( 0 );
...
} )( window, window.bd.mce.views, window.bd.media, window.jQuery );
