 ...
 *
 * Requires bd-admin/includes/revision.php.
 *
 * @package Blasdoise
...
bd_reset_vars( array( 'revision', 'action', 'from', 'to' ) );
...
case 'restore' :
	...
	// Revisions disabled (previously checked autosaves && ! bd_is_post_autosave( $revision ))
	...
	bd_restore_post_revision( $revision->ID );
...
default :
	...
	if ( ! bd_revisions_enabled( $post ) && ! bd_is_post_autosave( $revision ) ) {
		$redirect = 'edit.php?post_type=' . $post->post_type;
		break;
	}
...
bd_enqueue_script( 'revisions' );
bd_localize_script( 'revisions', '_bdRevisionsSettings', bd_prepare_revisions_for_js( $post, $revision_id, $from ) );
