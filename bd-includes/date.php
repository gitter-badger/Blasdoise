 ...
 *
 * `BD_Date_Query` is a helper that allows primary query classes, such as {@see BD_Query},
 ...
 * See {@link BD_Date_Query::validate_date_values()}.
 *
 * @since 1.0.0
 */
class BD_Date_Query {
	 ...
	 *
	 * See {@see BD_Date_Query::__construct()} for information on date query arguments.
	 *
	 * @since 1.0.0
	 * @access public
	 * @var array
	...
	public function validate_date_values( $date_query = array() ) {
		...
		foreach ( $min_max_checks as $key => $check ) {
			...
			foreach ( (array) $date_query[ $key ] as $_value ) {
				...
				if ( ! is_numeric( $_value ) || ! $is_between ) {
					...
					_doing_it_wrong( __CLASS__, $error, '1.0.0' );
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! bd_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
			 ...
			 */
			if ( ! bd_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
		...
		if ( ! empty( $day_month_year_error_msg ) ) {
			_doing_it_wrong( __CLASS__, $day_month_year_error_msg, '1.0.0' );
		}
	 ...
	 * known tables, and then, if found, have a table prefix (such as 'bd_posts.') prepended.
	 * Prefixed column names (such as 'bd_posts.post_date') bypass this whitelist check,
	...
	public function get_sql() {
		 ...
		 * @param BD_Date_Query $this  The BD_Date_Query instance.
		 */
	 ...
	 *
	 * Called by the public {@see BD_Date_Query::get_sql()}, this method
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( ! empty( $query['after'] ) )
			$where_parts[] = $bddb->prepare( "$column $gt %s", $this->build_mysql_datetime( $query['after'], ! $inclusive ) );

		if ( ! empty( $query['before'] ) )
			$where_parts[] = $bddb->prepare( "$column $lt %s", $this->build_mysql_datetime( $query['before'], $inclusive ) );
		...
		if ( isset( $query['week'] ) && false !== ( $value = $this->build_value( $compare, $query['week'] ) ) ) {
			$where_parts[] = _bd_mysql_week( $column ) . " $compare $value";
	...
	public function build_time_query( $column, $compare, $hour = null, $minute = null, $second = null ) {
		...
		return $bddb->prepare( "DATE_FORMAT( $column, %s ) $compare %f", $format, $time );
	}
}
