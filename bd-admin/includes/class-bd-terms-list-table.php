 ...
 * @access private
 */
class BD_Terms_List_Table extends BD_List_Table {
	...
	public function prepare_items() {
		...
		$search = !empty( $_REQUEST['s'] ) ? trim( bd_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( !empty( $_REQUEST['orderby'] ) )
			$args['orderby'] = trim( bd_unslash( $_REQUEST['orderby'] ) );

		if ( !empty( $_REQUEST['order'] ) )
			$args['order'] = trim( bd_unslash( $_REQUEST['order'] ) );

		$this->callback_args = $args;

		$this->set_pagination_args( array(
			'total_items' => bd_count_terms( $this->screen->taxonomy, compact( 'search' ) ),
	...
	public function display_rows_or_placeholder() {
		...
		$args = bd_parse_args( $this->callback_args, array(
			'page' => 1,
			'number' => 20,
			'search' => '',
			'hide_empty' => 0
	...
	public function column_name( $tag ) {
		 ...
		 * @since 1.0.0
		 *
		 * @see BD_Terms_List_Table::column_name()
		 *
	...
	protected function handle_row_actions( $tag, $column_name, $primary ) {
		...
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term )
			$actions['delete'] = "<a class='delete-tag' href='" . bd_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ) . "'>" . __( 'Delete' ) . "</a>";
		if ( $tax->public )
		 ...
		 *
		 * @since 1.0.0
		 * @deprecated 1.0.0 Use {$taxonomy}_row_actions instead.
		 *
...
?>
	...
	?>
		...
		<p class="inline-edit-save submit">
			...
			<?php bd_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
