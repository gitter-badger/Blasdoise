<?php
/**
 * Blasdoise Customize Widgets classes
 *
 * @package Blasdoise
 ...
 */
final class BD_Customize_Widgets {
	 ...
	 * @access public
	 *
	 * @param BD_Customize_Manager $manager Customize manager bootstrap instance.
	 */
	public function __construct( $manager ) {
		...
		add_action( 'bd_loaded',                               array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( bd_unslash( $_REQUEST['widget-id'] ) );
		}
		 ...
		 * But don't do this if a customize_save because this will cause BD to think there is nothing
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = bd_get_sidebars_widgets();
		...
		// reset global cache var used by bd_get_sidebars_widgets()
		unset( $GLOBALS['_bd_sidebars_widgets'] );
	}
	 ...
	 * theme_mod.
	 *
	 * @see BD_Customize_Widgets::handle_theme_switch()
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * Note: these actions are also fired in bd_ajax_update_widget().
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function customize_controls_init() {
		/** This action is documented in bd-admin/includes/ajax-actions.php */
	 ...
	 * after Blasdoise is loaded so that all filters have been
	...
	public function schedule_customize_register() {
		if ( is_admin() ) {
			...
			add_action( 'bd', array( $this, 'customize_register' ) );
		}
	}
	...
	public function customize_register() {
		global $bd_registered_widgets, $bd_registered_widget_controls, $bd_registered_sidebars;

		$sidebars_widgets = array_merge(
			array( 'bd_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $bd_registered_sidebars ), array() ),
			bd_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $bd_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_registered_sidebar = isset( $bd_registered_sidebars[ $sidebar_id ] );
			$is_inactive_widgets   = ( 'bd_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title' => $bd_registered_sidebars[ $sidebar_id ]['name'],
						'description' => $bd_registered_sidebars[ $sidebar_id ]['description'],
						'priority' => array_search( $sidebar_id, array_keys( $bd_registered_sidebars ) ),
						'panel' => 'widgets',
					...
					$section = new BD_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					$control = new BD_Widget_Area_Customize_Control( $this->manager, $setting_id, array(
						'section'    => $section_id,
						'sidebar_id' => $sidebar_id,
			...
			foreach ( $sidebar_widget_ids as $i => $widget_id ) {
				...
				if ( ! $is_active_sidebar || ! isset( $bd_registered_widgets[$widget_id] ) ) {
					continue;
				}

				$registered_widget = $bd_registered_widgets[$widget_id];
				$setting_id        = $this->get_setting_id( $widget_id );
				$id_base           = $bd_registered_widget_controls[$widget_id]['id_base'];

				$control = new BD_Widget_Form_Customize_Control( $this->manager, $setting_id, array(
					...
					'priority'       => $i,
					'width'          => $bd_registered_widget_controls[$widget_id]['width'],
					'height'         => $bd_registered_widget_controls[$widget_id]['height'],
	 ...
	 * @return BD_Error|array Array containing a widget's id_base and number components,
	 *                        or a BD_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new BD_Error( 'widget_setting_invalid_id' );
		}
	...
	public function enqueue_scripts() {
		global $bd_scripts, $bd_registered_sidebars, $bd_registered_widgets;

		bd_enqueue_style( 'customize-widgets' );
		bd_enqueue_script( 'customize-widgets' );
		...
		$settings = array(
			'nonce'                => bd_create_nonce( 'update-widget' ),
			'registeredSidebars'   => array_values( $bd_registered_sidebars ),
			'registeredWidgets'    => $bd_registered_widgets,
		...
		$bd_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _bdCustomizeWidgetsSettings = %s;', bd_json_encode( $settings ) )
		);
	}
	...
	public function get_available_widgets() {
		...
		global $bd_registered_widgets, $bd_registered_widget_controls;
		require_once ABSPATH . '/bd-admin/includes/widgets.php'; // for next_widget_id_number()

		$sort = $bd_registered_widgets;
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $bd_registered_widget_controls[$widget['id']]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $bd_registered_widget_controls[$widget['id']]['id_base'];
				...
				if ( $sidebar && 'bd_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = bd_list_widget_controls_dynamic_sidebar( array( 0 => $args, 1 => $widget['params'][0] ) );
			...
			$available_widget = array_merge( $available_widget, array(
				...
				'id_base'      => $id_base,
				'transport'    => 'refresh',
				'width'        => $bd_registered_widget_controls[$widget['id']]['width'],
				'height'       => $bd_registered_widget_controls[$widget['id']]['height'],
	...
	public function get_widget_control( $args ) {
		...
		call_user_func_array( 'bd_widget_control', $args );
		$replacements = array(
	...
	public function customize_preview_init() {
		add_action( 'bd_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'bd_print_styles',    array( $this, 'print_preview_css' ), 1 );
		add_action( 'bd_footer',          array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = bd_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because bd_get_sidebars_widgets() gets called early at init
	 * (via bd_convert_widget_settings()) and can set global variable
	 * $_bd_sidebars_widgets to the value of get_option( 'sidebars_widgets' )
	...
	public function customize_preview_enqueue() {
		bd_enqueue_script( 'customize-preview-widgets' );
	}
	...
	public function print_preview_css() {
		...
		.widget-customizer-highlighted-widget {
			outline: none;
			-webkit-shadow: 0 0 2px rgba(30,140,190,0.8);
			shadow: 0 0 2px rgba(30,140,190,0.8);
			position: relative;
			z-index: 1;
		}
		</style>
	...
	/**
	 * At the very end of the page, at the very end of the bd_footer,
	...
	public function export_preview_data() {
		...
		<script type="text/javascript">
			var _bdWidgetCustomizerPreviewSettings = <?php echo bd_json_encode( $settings ); ?>;
		</script>
		<?php
	}
	...
	public function tally_sidebars_via_is_active_sidebar_calls( $is_active, $sidebar_id ) {
		if ( isset( $GLOBALS['bd_registered_sidebars'][$sidebar_id] ) ) {
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return bd_hash( $serialized_instance );
	}
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $bd_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param  string $widget_id Widget ID.
	 * @return BD_Error|array Array containing the updated widget information.
	 *                        A BD_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $bd_registered_widget_updates, $bd_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new BD_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new BD_Error( 'widget_setting_unsanitized' );
			}
			...
			if ( ! is_null( $parsed_id['number'] ) ) {
				...
				$_REQUEST[$key] = $_POST[$key] = bd_slash( $value );
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new BD_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new BD_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from BD_Customize_Setting::preview() will use this value
		...
		ob_start();
		$form = $bd_registered_widget_controls[ $widget_id ];
		if ( $form ) {
	 ...
	 *
	 * Most code here copied from bd_ajax_save_widget()
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @see bd_ajax_save_widget()
	 *
	 */
	public function bd_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			bd_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			bd_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_bd_error( $updated_widget ) ) {
			bd_send_json_error( $updated_widget->get_error_code() );
		}

		$form = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'] );

		bd_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function capture_filter_pre_get_option( $value ) {
		...
		if ( isset( $this->_captured_options[ $option_name ] ) ) {
			...
			/** This filter is documented in bd-includes/option.php */
	...
	/**
	 * @since 1.0.0
	 * @deprecated 1.0.0 Deprecated in favor of customize_dynamic_setting_args filter.
	 */
