...
/**
 * Blasdoise View plugin.
 */
tinymce.PluginManager.add( 'bdview', function( editor ) {
	var $ = editor.$,
		selected,
		Env = tinymce.Env,
		VK = tinymce.util.VK,
	...
	function getView( node ) {
		return getParent( node, 'bdview-wrap' );
	}
	...
	function setViewCursor( before, view ) {
		var location = before ? 'before' : 'after',
			offset = before ? 0 : 1;
		deselect();
		editor.selection.setCursorLocation( editor.dom.select( '.bdview-selection-' + location, view )[0], offset );
		editor.nodeChanged();
	}
	...
	function removeView( view ) {
		editor.undoManager.transact( function() {
			handleEnter( view );
			bd.mce.views.remove( editor, view );
		});
	}
	...
	function select( viewNode ) {
		...
		if ( viewNode !== selected ) {
			...
			clipboard = dom.create( 'div', {
				'class': 'bdview-clipboard',
				'contenteditable': 'true'
			}, bd.mce.views.getText( viewNode ) );

			editor.dom.select( '.bdview-body', viewNode )[0].appendChild( clipboard );
		...
		editor.nodeChanged();
		editor.fire( 'bdview-selected', viewNode );
	}
	...
	function deselect() {
		...
		if ( selected ) {
			clipboard = editor.dom.select( '.bdview-clipboard', selected )[0];
	...
	// Check if the `bd.mce` API exists.
	if ( typeof bd === 'undefined' || ! bd.mce ) {
		return {
			getView: _noop
		};
	}
	...
	function resetViews( content ) {
		return content.replace( /<div[^>]+data-bdview-text="([^"]+)"[^>]*>(?:[\s\S]+?bdview-selection-after[^>]+>[^<>]*<\/p>\s*|\.)<\/div>/g, resetViewsCallback )
			.replace( /<p [^>]*?data-bdview-marker="([^"]+)"[^>]*>[\s\S]*?<\/p>/g, resetViewsCallback );
	}
	...
	editor.on( 'BeforeSetContent', function( event ) {
		...
		event.content = bd.mce.views.setMarkers( event.content );
	});
	...
	editor.on( 'click', function( event ) {
		...
		if ( y < firstRect.top && ( view = getView( first ) ) ) {
			...
			tinymce.each( editor.dom.select( '.bdview-wrap' ), function( view ) {
	...
	editor.on( 'init', function() {
		...
		if ( MutationObserver ) {
			new MutationObserver( function() {
				editor.fire( 'bd-body-class-change' );
			} )
	...
	// Empty the bdview wrap and marker nodes
	function emptyViewNodes( rootNode ) {
		$( 'div[data-bdview-text], p[data-bdview-marker]', rootNode ).each( function( i, node ) {
			node.innerHTML = '.';
		});
	}
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.content ) {
			event.content = event.content.replace( /<div [^>]*?data-bdview-text="([^"]+)"[^>]*>[\s\S]*?<\/div>/g, resetViewsCallback )
				.replace( /<p [^>]*?data-bdview-marker="([^"]+)"[^>]*>[\s\S]*?<\/p>/g, resetViewsCallback );
		}
	});
	...
	editor.on( 'keydown', function( event ) {
		...
		if ( selected ) {
			...
			if ( ! ( ( cursorBefore = dom.hasClass( view, 'bdview-selection-before' ) ) ||
					( cursorAfter = dom.hasClass( view, 'bdview-selection-after' ) ) ) ) {
				return;
			}
	...
	editor.on( 'NodeChange', function( event ) {
		var dom = editor.dom,
			views = editor.dom.select( '.bdview-wrap' ),
		...
		tinymce.each( views, function ( view ) {
			if ( view.className ) {
				view.className = view.className.replace( / ?\bbdview-(?:selection-before|selection-after|cursor-hide)\b/g, '' );
			}
		});

		if ( focus && view ) {
			if ( ( className === 'bdview-selection-before' || className === 'bdview-selection-after' ) &&
				...
				cursorInterval = setInterval( function() {
					if ( dom.hasClass( view, 'bdview-cursor-hide' ) ) {
						dom.removeClass( view, 'bdview-cursor-hide' );
					} else {
						dom.addClass( view, 'bdview-cursor-hide' );
					}
				}, 500 );
			...
			} else if ( ! getParent( event.element, 'bdview-clipboard' ) && ! setViewCursorTries ) {
				deselect();
				setViewCursorTries++;
				setViewCursor( true, view );
			}
	...
	editor.on( 'BeforeExecCommand', function() {
		...
		if ( node && ( ( execCommandBefore = node.className === 'bdview-selection-before' ) || node.className === 'bdview-selection-after' ) && ( view = getView( node ) ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		if ( editor.dom.hasClass( event.target, 'bdview-wrap' ) ) {
			event.name = editor.dom.getAttrib( event.target, 'data-bdview-type' ) || 'bdview';
			event.stopPropagation();
	...
	editor.addButton( 'bd_view_edit', {
		tooltip: 'Edit ', // trailing space is needed, used for context
		icon: 'basicon basicons-edit',
		onclick: function() {
			selected && bd.mce.views.edit( editor, selected );
		}
	} );

	editor.addButton( 'bd_view_remove', {
		tooltip: 'Remove',
	...
	// Add to editor.bd
	editor.bd = editor.bd || {};
	editor.bd.getView = getView;
	editor.bd.setViewCursor = setViewCursor;
