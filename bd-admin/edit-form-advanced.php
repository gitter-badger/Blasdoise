...
bd_enqueue_script('post');
$_bd_editor_expand = $_content_editor_dfw = false;
 ...
 */
if ( post_type_supports( $post_type, 'editor' ) && ! bd_is_mobile() &&
	 ! ( $is_IE && preg_match( '/MSIE [5678]/', $_SERVER['HTTP_USER_AGENT'] ) ) &&
	 apply_filters( 'bd_editor_expand', true, $post_type ) ) {

	bd_enqueue_script('editor-expand');
	$_content_editor_dfw = true;
	$_bd_editor_expand = ( get_user_setting( 'editor_expand', 'on' ) === 'on' );
}
...
if ( $post_ID == get_option( 'page_for_posts' ) && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_bd_posts_page_notice' );
...
$messages['post'] = array(
	...
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __('Post restored to revision from %s'), bd_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page'] = array(
	 ...
	 3 => __('Custom field deleted.'),
	 4 => __('Page updated.'),
	 5 => isset($_GET['revision']) ? sprintf( __('Page restored to revision from %s'), bd_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' == $post->post_status ) {
	...
	$autosave = bd_get_post_autosave( $post_ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _bd_post_revision_fields() as $autosave_field => $_autosave_field ) {
	...
	if ( ! $notice )
		bd_delete_post_revision( $autosave->ID );
...
require_once( ABSPATH . 'bd-admin/includes/meta-boxes.php' );
 ...
 *
 * @since 1.0.0
 * @deprecated 1.0.0 Use 'add_meta_boxes' instead.
 *
...
if ( 'post' == $post_type ) {
	...
	if ( current_theme_supports( 'post-formats' ) && post_type_supports( 'post', 'post-formats' ) ) {
		$publish_box .= '<li>' . __( '<strong>Format</strong> - Post Formats designate how your theme will display a specific post. For example, you could have a <em>standard</em> blog post with a title and paragraphs, or a short <em>aside</em> that omits the title and contains a short text blurb. Your theme could enable all or some of 10 possible formats.' ) . '</li>';
	}
	...
	$discussion_settings  = '<p>' . __('<strong>Send Trackbacks</strong> - Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other Blasdoise sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.') . '</p>';
...
if ( 'draft' != get_post_status( $post ) )
	bd_original_referer_field(true, 'previous');
...
$shortlink = bd_get_shortlink($post->ID, 'post');
...
<?php
bd_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
?>
</div><!-- /titlediv -->
<?php
}
...
?>
<div id="postdivrich" class="postarea<?php if ( $_bd_editor_expand ) { echo ' bd-editor-expand'; } ?>">

<?php bd_editor( $post->post_content, 'content', array(
	...
	'tinymce' => array(
		'resize' => false,
		'bd_autoresize_on' => $_bd_editor_expand,
		'add_unload_trigger' => false,
	),
) ); ?>
<table id="post-status-info"><tbody><tr>
	<td id="bd-word-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></td>
...
<?php if ( ! bd_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
