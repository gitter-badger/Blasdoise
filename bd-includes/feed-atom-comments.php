...
/** This action is documented in bd-includes/feed-rss2.php */
...
<feed
	...
	<?php
		/** This action is documented in bd-includes/feed-atom.php */
		do_action( 'atom_ns' );
...
>
	<title type="text"><?php
		...
		elseif ( is_search() )
			...
			printf( ent2ncr( __( 'Comments for %s' ) ), get_bloginfo_rss( 'name' ) . get_bd_title_rss() );
	?></title>
	...
	<entry>
		<title><?php
			if ( !is_singular() ) {
				...
				/** This filter is documented in bd-includes/feed.php */
...
<?php else : // This comment is in reply to another comment
	...
	// The rel attribute below and the id tag above should be GUIDs, but BD doesn't create them for comments (unlike posts). Either way, it's more important that they both use the same system
?>
