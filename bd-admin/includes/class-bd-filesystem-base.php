...
/**
 * Base Blasdoise Filesystem class for which Filesystem implementations extend
 *
 * @since 1.0.0
 */
class BD_Filesystem_Base {
	/**
	...
	public function abspath() {
		...
		// Perhaps the FTP folder is rooted at the Blasdoise install, Check for bd-includes folder in root, Could have some false positives, but rare.
		if ( ! $folder && $this->is_dir( '/' . BDINC ) )
			$folder = '/';
		return $folder;
	}

	/**
	 * Return the path on the remote filesystem of BD_CONTENT_DIR.
	 *
	 * @access public
	 * @since 1.0.0
	 *
	 * @return string The location of the remote path.
	 */
	public function bd_content_dir() {
		return $this->find_folder(BD_CONTENT_DIR);
	}

	/**
	 * Return the path on the remote filesystem of BD_PLUGIN_DIR.
	 *
	 * @access public
	 * @since 1.0.0
	 *
	 * @return string The location of the remote path.
	 */
	public function bd_plugins_dir() {
		return $this->find_folder(BD_PLUGIN_DIR);
	}
	 ...
	 * @return string The location of the remote path.
	 */
	public function bd_themes_dir( $theme = false ) {
	...
	/**
	 * Return the path on the remote filesystem of BD_LANG_DIR.
	 *
	 * @access public
	 * @since 1.0.0
	 *
	 * @return string The location of the remote path.
	 */
	public function bd_lang_dir() {
		return $this->find_folder(BD_LANG_DIR);
	}
	 ...
	 * @access public
	 * @since 1.0.0
	 * @see BD_Filesystem::abspath()
	 * @see BD_Filesystem::bd_content_dir()
	 * @see BD_Filesystem::bd_plugins_dir()
	 * @see BD_Filesystem::bd_themes_dir()
	 * @see BD_Filesystem::bd_lang_dir()
	 *
	...
	public function find_base_dir( $base = '.', $echo = false ) {
		_deprecated_function(__FUNCTION__, '1.0', 'BD_Filesystem::abspath() or BD_Filesystem::bd_*_dir()' );
		$this->verbose = $echo;
		return $this->abspath();
	}
	...
	public function find_folder( $folder ) {
		...
		if ( stripos($this->method, 'ftp') !== false ) {
			$constant_overrides = array(
				'FTP_BASE' => ABSPATH,
				'FTP_CONTENT_DIR' => BD_CONTENT_DIR,
				'FTP_PLUGIN_DIR' => BD_PLUGIN_DIR,
				'FTP_LANG_DIR' => BD_LANG_DIR
			);
	...
	public function search_for_folder( $folder, $base = '.', $loop = false ) {
		...
		foreach ( $folder_parts as $index => $key ) {
			...
			/*
			 * Working from /home/ to /user/ to /blasdoise/ see if that file exists within
			 * the current folder, If it's found, change into it and follow through looking
			 * for it. If it cant find Blasdoise down that route, it'll continue onto the next
		...
		// This comes into effect when the CWD is /home/user/ but BD is at /var/www/....
	 ...
	 * @since 1.0.0
	 * @abstract
	 * @return bool True on success or false on failure (always true for BD_Filesystem_Direct).
	 */
