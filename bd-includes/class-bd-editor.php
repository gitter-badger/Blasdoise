<?php
/**
 * Facilitates adding of the Blasdoise editor as used on the Write and Edit screens.
 *
 * @package Blasdoise
 * @since 1.0.0
 *
 * Private, not included by default. See bd_editor() in bd-includes/general-template.php.
 */

final class _BD_Editors {
	 ...
	 *
	 *     @type bool       $bdautop           Whether to use bdautop(). Default true.
	 ...
	 *     @type bool       $dfw               Deprecated in 1.0. Since 1.0 used only to enqueue bd-fullscreen-stub.js for backwards compatibility.
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filter the bd_editor() settings.
		 *
		 * @since 1.0.0
		 *
		 * @see _BD_Editors()::parse_settings()
		 *
		 * @param array  $settings  Array of editor arguments.
		 * @param string $editor_id ID for the current editor instance.
		 */
		$settings = apply_filters( 'bd_editor_settings', $settings, $editor_id );

		$set = bd_parse_args( $settings, array(
			'bdautop'             => true,
			'media_buttons'       => true,
			'default_editor'      => '',
		...
		if ( self::$this_tinymce ) {
			if ( false !== strpos( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'bd_editor()', '1.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['bd_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' bd-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : bd_default_editor();
				// 'html' is used for the "Text" editor tab.
				if ( 'html' !== $default_editor ) {
					$default_editor = 'tinymce';
				}

				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce" class="bd-switch-editor switch-tmce"' .
					' data-bd-editor-id="' . $editor_id_attr . '">' . __('Visual') . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html" class="bd-switch-editor switch-html"' .
					' data-bd-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class = 'bd-core-ui bd-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="bd-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			bd_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="bd-' . $editor_id_attr . '-editor-tools" class="bd-editor-tools hide-if-no-js">';

			if ( $set['media_buttons'] ) {
				self::$has_medialib = true;

				if ( ! function_exists( 'media_buttons' ) )
					include( ABSPATH . 'bd-admin/includes/media.php' );

				echo '<div id="bd-' . $editor_id_attr . '-media-buttons" class="bd-media-buttons">';
			...
			echo '<div class="bd-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		 ...
		 */
		$the_editor = apply_filters( 'the_editor', '<div id="bd-' . $editor_id_attr . '-editor-container" class="bd-editor-container">' .
			$quicktags_toolbar .
		...
		if ( 'html' === $default_editor && has_filter( 'htmledit_pre' ) ) {
			...
			_deprecated_function( 'add_filter( htmledit_pre )', '1.0.0', 'add_filter( format_for_editor )' );
			$content = apply_filters( 'htmledit_pre', $content );
		} elseif ( 'tinymce' === $default_editor && has_filter( 'richedit_pre' ) ) {
			_deprecated_function( 'add_filter( richedit_pre )', '1.0.0', 'add_filter( format_for_editor )' );
	...
	public static function editor_settings($editor_id, $set) {
		global $bd_version, $tinymce_version;
		...
		if ( empty(self::$first_init) ) {
			if ( is_admin() ) {
				...
				add_action( 'bd_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'bd_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					self::$plugins = $plugins = apply_filters( 'teeny_mce_plugins', array( 'colorpicker', 'lists', 'fullscreen', 'image', 'blasdoise', 'bdeditimage', 'bdlink' ), $editor_id );
				} else {
					 ...
					 * 'myplugin' => 'http://mysite.com/bd-content/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in Blasdoise should be added to the TinyMCE instance.
					 *
					 * @since 1.0.0
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as bd_mce_translation(),
						...
						foreach ( $mce_external_plugins as $name => $url ) {
							...
							if ( ! in_array( $name, $loaded_langs, true ) ) {
								...
								$path = BD_CONTENT_DIR . $path . '/langs/';
				...
				self::$first_init = array(
					...
					'formats' => '{' .
						'alignleft: [' .
							...
							'{selector: "img,table,dl.bd-caption", classes: "alignleft"}' .
						'],' .
						'aligncenter: [' .
							'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
							'{selector: "img,table,dl.bd-caption", classes: "aligncenter"}' .
						'],' .
						'alignright: [' .
							'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
							'{selector: "img,table,dl.bd-caption", classes: "alignright"}' .
						'],' .
					...
					'entity_encoding' => 'raw',
					'keep_styles' => false,
					'cache_suffix' => 'bd-mce-' . $tinymce_version,

					// Limit the preview styles in the menu/toolbar
					'preview_styles' => 'font-family font-size font-weight font-style text-decoration text-transform',

					'end_container_on_empty_block' => true,
					'bdeditimage_disable_captions' => $no_captions,
					'bdeditimage_html5_captions' => current_theme_supports( 'html5', 'caption' ),
					'plugins' => implode( ',', $plugins ),
				);

				if ( ! empty( $mce_external_plugins ) ) {
					self::$first_init['external_plugins'] = bd_json_encode( $mce_external_plugins );
				}

				$suffix = SCRIPT_DEBUG ? '' : '.min';
				$version = 'ver=' . $bd_version;
				$basicons = includes_url( "css/basicons$suffix.css?$version" );

				// Blasdoise default stylesheet and basicons
				$mce_css = array(
					$basicons,
					self::$baseurl . '/skins/blasdoise/bd-content.css?' . $version
				);
			...
			if ( $set['teeny'] ) {
				...
				$mce_buttons = array( 'bold', 'italic', 'strikethrough', 'bullist', 'numlist', 'blockquote', 'hr', 'alignleft', 'aligncenter', 'alignright', 'link', 'unlink', 'bd_more', 'spellchecker' );
			...
			if ( $post = get_post() ) {
				...
				if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
					...
					if ( $post_format && ! is_bd_error( $post_format ) )
	...
	public static function enqueue_scripts() {
		if ( self::$has_tinymce )
			bd_enqueue_script('editor');

		if ( self::$has_quicktags ) {
			bd_enqueue_script( 'quicktags' );
			bd_enqueue_style( 'buttons' );
		}

		if ( in_array('bdlink', self::$plugins, true) || in_array('link', self::$qt_buttons, true) ) {
			bd_enqueue_script('bdlink');
		}

		if ( self::$old_dfw_compat ) {
			bd_enqueue_script('bd-fullscreen-stub');
		}
		 ...
		 */
		do_action( 'bd_enqueue_editor', array(
	 ...
	 * Can be used directly (_BD_Editors::bd_mce_translation()) by passing the same locale as set in the TinyMCE init object.
	 *
	 * @static
	 ...
	 */
	public static function bd_mce_translation( $mce_locale = '', $json_only = false ) {
		 ...
		 */
		$mce_translation = apply_filters( 'bd_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return bd_json_encode( $mce_translation );
		}

		$baseurl = self::$baseurl ? self::$baseurl : includes_url( 'js/tinymce' );

		return "tinymce.addI18n( '$mce_locale', " . bd_json_encode( $mce_translation ) . ");\n" .
	...
	public static function editor_js() {
		global $bd_version, $tinymce_version, $concatenate_scripts, $compress_scripts;
		 ...
		 */
		do_action( 'before_bd_tiny_mce', self::$mce_settings );
		?>
		...
		// Load tinymce.js when running from /src, else load bd-tinymce.js.gz (production) or tinymce.min.js (SCRIPT_DEBUG)
		$mce_suffix = false !== strpos( $bd_version, '-src' ) ? '' : '.min';

		if ( $tmce_on ) {
			if ( $compressed ) {
				echo "<script type='text/javascript' src='{$baseurl}/bd-tinymce.php?c=1&amp;$version'></script>\n";
			} else {
			...
			echo "<script type='text/javascript'>\n" . self::bd_mce_translation() . "</script>\n";

			if ( self::$ext_plugins ) {
				// Load the old-format English strings to prevent unsightly labels in old style popups
				echo "<script type='text/javascript' src='{$baseurl}/langs/bd-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'bd_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				for ( id in tinyMCEPreInit.mceInit ) {
					...
					$wrap = tinymce.$( '#bd-' + id + '-wrap' );

					if ( ( $wrap.hasClass( 'tmce-active' ) || ! tinyMCEPreInit.qtInit.hasOwnProperty( id ) ) && ! init.bd_skip_init ) {
						tinymce.init( init );

						if ( ! window.bdActiveEditor ) {
							window.bdActiveEditor = id;
						}
					}
				}
			}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_bd_tiny_mce', self::$mce_settings );
	}
	 ...
	 */
	public static function bd_fullscreen_html() {
	 ...
	 */
	public static function bd_link_query( $args = array() ) {
		 ...
		 *
		 * @see BD_Query for a full list of arguments
		 *
		 * @since 1.0.0
		 *
		 * @param array $query An array of BD_Query arguments.
		 */
		$query = apply_filters( 'bd_link_query_args', $query );
		 ...
		 * @param array $query  An array of BD_Query arguments.
		 */
		return apply_filters( 'bd_link_query', $results, $query );
	}
	 ...
	 * @static
	 */
	public static function bd_link_dialog() {
		$search_panel_visible = '1' == get_user_setting( 'bdlink', '0' ) ? ' search-panel-visible' : '';

		// display: none is required here, see #BD27605
		?>
		<div id="bd-link-backdrop" style="display: none"></div>
		<div id="bd-link-wrap" class="bd-core-ui<?php echo $search_panel_visible; ?>" style="display: none">
		<form id="bd-link" tabindex="-1">
		<?php bd_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<div id="link-modal-title">
			<?php _e( 'Insert/edit link' ) ?>
			<button type="button" id="bd-link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
	 	</div>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto"><?php _e( 'Enter the destination URL' ); ?></p>
				<div>
					<label><span><?php _e( 'URL' ); ?></span><input id="bd-link-url" type="text" /></label>
				</div>
				<div class="bd-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span><input id="bd-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span>&nbsp;</span><input type="checkbox" id="bd-link-target" /> <?php _e( 'Open link in a new window/tab' ); ?></label>
				</div>
			</div>
			<p class="howto"><a href="#" id="bd-link-search-toggle"><?php _e( 'Or link to existing content' ); ?></a></p>
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						<span class="search-label"><?php _e( 'Search' ); ?></span>
						<input type="search" id="bd-link-search" class="link-search-field" autocomplete="off" />
		...
		<div class="submitbox">
			<div id="bd-link-cancel">
				<a class="submitdelete deletion" href="#"><?php _e( 'Cancel' ); ?></a>
			</div>
			<div id="bd-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="bd-link-submit" name="bd-link-submit">
			</div>
		</div>
		</form>
		</div>
