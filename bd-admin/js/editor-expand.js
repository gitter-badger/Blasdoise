( function( window, $, undefined ) {
	...
	$( function() {
		var $wrap = $( '#postdivrich' ),
			$contentWrap = $( '#bd-content-wrap' ),
			$tools = $( '#bd-content-editor-tools' ),
			$visualTop = $(),
			$visualEditor = $(),
			$textTop = $( '#ed_toolbar' ),
			$textEditor = $( '#content' ),
			$textEditorClone = $( '<div id="content-textarea-clone" class="bd-exclude-emoji"></div>' ),
			...
			$postBody = $('#post-body'),
			fullscreen = window.bd.editor && window.bd.editor.fullscreen,
			mceEditor,
			mceBind = function(){},
			mceUnbind = function(){},
			fixedTop = false,
		...
		$document.on( 'tinymce-editor-init.editor-expand', function( event, editor ) {
			...
			function mceGetCursorOffset() {
				...
				if ( editor.bd && editor.bd.getView && ( view = editor.bd.getView( node ) ) ) {
			...
			// but also to scroll the window when the cursor moves out of the viewport to a bdview.
			...
			function mceShow() {
				...
				setTimeout( function() {
					editor.execCommand( 'bdAutoResize' );
					adjust();
				}, 300 );
			}
			...
			mceBind = function() {
				...
				editor.on( 'bd-toolbar-toggle', toggleAdvanced );
				// Adjust when the editor resizes.
				editor.on( 'setcontent bd-autoresize bd-toolbar-toggle', adjust );
			...
			mceUnbind = function() {
				...
				editor.off( 'bd-toolbar-toggle', toggleAdvanced );
				editor.off( 'setcontent bd-autoresize bd-toolbar-toggle', adjust );
			...
			if ( $wrap.hasClass( 'bd-editor-expand' ) ) {
				// Adjust "immediately"
				mceBind();
		...
		function adjust( event ) {
			...
			if ( ! canPin ) {
				...
				if ( ( ! fixedBottom || ( resize && advanced ) ) &&
						// +[n] for the border around the .bd-editor-container.
		...
		function on() {
			...
			$wrap.addClass( 'bd-editor-expand' );
			...
			$document.on( 'bd-collapse-menu.editor-expand postboxes-columnchange.editor-expand editor-classchange.editor-expand', adjust )
				.on( 'postbox-toggled.editor-expand', function() {
					...
					adjust();
				}).on( 'bd-window-resized.editor-expand', function() {
					if ( mceEditor && ! mceEditor.isHidden() ) {
						mceEditor.execCommand( 'bdAutoResize' );
					} else {
						textEditorResize();
					}
				});
			...
			if ( mceEditor ) {
				mceEditor.settings.bd_autoresize_on = true;
				mceEditor.execCommand( 'bdAutoResizeOn' );
		...
		function off() {
			...
			$wrap.removeClass( 'bd-editor-expand' );
			...
			if ( mceEditor ) {
				mceEditor.settings.bd_autoresize_on = false;
				mceEditor.execCommand( 'bdAutoResizeOff' );
	...
	$( function() {
		...
		$document.on( 'tinymce-editor-setup.focus', function( event, editor ) {
			...
			editor.addCommand( 'bdToggleDFW', toggle );
			editor.addShortcut( 'alt+shift+w', '', 'bdToggleDFW' );
		} );
		...
		$document.on( 'tinymce-editor-init.focus', function( event, editor ) {
			...
			if ( editor.id === 'content' ) {
				...
				mceBind = function() {
					...
					editor.on( 'blur', blur );
					editor.on( 'bd-autoresize', recalcEditorRect );
				};
				...
				mceUnbind = function() {
					...
					editor.off( 'bd-autoresize', recalcEditorRect );
				};
