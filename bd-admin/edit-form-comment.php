...
<?php bd_nonce_field('update-comment_' . $comment->comment_ID) ?>
<div class="wrap">
...
<?php
	...
	bd_editor( $comment->comment_content, 'content', array( 'media_buttons' => false, 'tinymce' => false, 'quicktags' => $quicktags_settings ) );
...
<?php echo "<a class='submitdelete deletion' href='" . bd_nonce_url("comment.php?action=" . ( !EMPTY_TRASH_DAYS ? 'deletecomment' : 'trashcomment' ) . "&amp;c=$comment->comment_ID&amp;_bd_original_http_referer=" . urlencode(bd_get_referer()), 'delete-comment_' . $comment->comment_ID) . "'>" . ( !EMPTY_TRASH_DAYS ? __('Delete Permanently') : __('Move to Trash') ) . "</a>\n"; ?>
</div>
