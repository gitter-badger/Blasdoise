/* global tinymce */
tinymce.PluginManager.add( 'bdeditimage', function( editor ) {
	...
	editor.addButton( 'bd_img_remove', {
		tooltip: 'Remove',
	...
	each( {
		...
		editor.addButton( 'bd_img_' + name, {
			tooltip: tooltip,
			icon: 'basicon basicons-align-' + direction,
			cmd: 'alignnone' === name ? 'bdAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.bd-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:bd_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:bd_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			if ( ! editor.getParam( 'bdeditimage_html5_captions' ) ) {
				width += 10;
			}

			return '<div class="mceTemp"><dl id="' + id + '" class="bd-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="bd-caption-dt">'+ img +'</dt><dd class="bd-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}
	...
	function getShortcode( content ) {
		return content.replace( /<div (?:id="attachment_|class="mceTemp)[^>]*>([\s\S]+?)<\/div>/g, function( a, b ) {
			...
			out = b.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /bd-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^bd-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'bd-image-', '' ), 10 );
		...
		// Extract caption
		captionBlock = dom.getParents( imageNode, '.bd-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.bd-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'bd-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.bd-caption', captionNode );
				...
				dd = dom.select( '.bd-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="bd-caption-dt"></dt><dd class="bd-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.bd-caption-dt' ).append( node );
		...
		if ( bd.media.events ) {
			bd.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		bd.media.events.trigger( 'editor:image-edit', {
			editor: editor,
			metadata: metadata,
			image: img
		} );
		...
		bd.media.events.trigger( 'editor:frame-create', { frame: frame } );
	...
	editor.on( 'init', function() {
		var dom = editor.dom,
			captionClass = editor.getParam( 'bdeditimage_html5_captions' ) ? 'html5-captions' : 'html4-captions';

		dom.addClass( editor.getBody(), captionClass );

		// Add caption field to the default image dialog
		editor.on( 'bdLoadImageForm', function( event ) {
			if ( editor.getParam( 'bdeditimage_disable_captions' ) ) {
				return;
			}
		...
		editor.on( 'bdNewImageRefresh', function( event ) {
			var parent, captionWidth;

			if ( parent = dom.getParent( event.node, 'dl.bd-caption' ) ) {
		...
		editor.on( 'bdImageFormSubmit', function( event ) {
			...
			if ( ! imgNode ) {
				...
				if ( caption ) {
					...
					html = '<dl class="bd-caption alignnone"' + captionWidth + '>' +
						'<dt class="bd-caption-dt">'+ html +'</dt><dd class="bd-caption-dd">'+ caption +'</dd></dl>';
				...
				wrap = dom.getParent( imgNode, 'dl.bd-caption' );

				if ( caption ) {
					if ( wrap ) {
						if ( parent = dom.select( 'dd.bd-caption-dd', wrap )[0] ) {
							parent.innerHTML = caption;
						}
					} else {
						if ( imgNode.className ) {
							captionId = imgNode.className.match( /bd-image-([0-9]+)/ );
						...
						captionAlign = ' class="bd-caption ' + captionAlign + '"';
						...
						html = '<dl ' + captionId + captionAlign + captionWidth + '>' +
							'<dt class="bd-caption-dt"></dt><dd class="bd-caption-dd">'+ caption +'</dd></dl>';
			...
			imgNode = dom.get('__bd-temp-img-id');
			dom.setAttrib( imgNode, 'id', imgId );
			event.imgData.node = imgNode;
		});

		editor.on( 'bdLoadImageData', function( event ) {
			var parent,
				data = event.imgData.data,
				imgNode = event.imgData.node;

			if ( parent = dom.getParent( imgNode, 'dl.bd-caption' ) ) {
				parent = dom.select( 'dd.bd-caption-dd', parent )[0];
		...
		dom.bind( editor.getDoc(), 'dragstart', function( event ) {
			...
			if ( node.nodeName === 'IMG' && dom.getParent( node, '.bd-caption' ) ) {
				event.preventDefault();
			}
		});

		// Prevent IE11 from making dl.bd-caption resizable
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.bd-caption' ) ) {
					// Hide the thick border with resize handles around dl.bd-caption
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'bd-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.bd-caption' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'bdAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.bd-caption' );
	...
	editor.bdSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.bdGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.bdSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.bdGetImgCaption( event.content );
		}
	});

	// Add to editor.bd
	editor.bd = editor.bd || {};
	editor.bd.isPlaceholder = isPlaceholder;
