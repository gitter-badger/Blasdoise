<?php
/**
 * Creates common globals for the rest of Blasdoise
 *
 * Sets $pagenow global which is the current page. Checks
 * for the browser to set which one is currently being used.
 *
 * Detects which user environment Blasdoise is being used on.
 * Only attempts to check for Apache, Nginx and IIS -- three web
 * servers with known pretty permalink capability.
 *
 * Note: Though Nginx is detected, Blasdoise does not currently
...
if ( is_admin() ) {
	// bd-admin pages are checked more carefully
	if ( is_network_admin() )
		preg_match('#/bd-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	elseif ( is_user_admin() )
		preg_match('#/bd-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	else
		preg_match('#/bd-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( '.php' !== substr($pagenow, -4, 4) )
			$pagenow .= '.php'; // for Options +Multiviews: /bd-admin/themes/index.php (themes.php is queried)
	}
} else {
