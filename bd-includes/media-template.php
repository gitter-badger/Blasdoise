 ...
 * @since 1.0.0
 */
function bd_underscore_audio_template() {
	$audio_types = bd_get_audio_extensions();
?>
<audio style="visibility: hidden"
	controls
	class="bd-audio-shortcode"
...
>
	...
	<source src="{{ data.model.src }}" type="{{ bd.media.view.settings.embedMimes[ data.model.src.split('.').pop() ] }}" />
	<# } #>

	<?php foreach ( $audio_types as $type ):
	?><# if ( ! _.isEmpty( data.model.<?php echo $type ?> ) ) { #>
	<source src="{{ data.model.<?php echo $type ?> }}" type="{{ bd.media.view.settings.embedMimes[ '<?php echo $type ?>' ] }}" />
	<# } #>
	<?php endforeach;
?></audio>
<?php
 ...
 * @since 1.0.0
 */
function bd_underscore_video_template() {
	$video_types = bd_get_video_extensions();
?>
<#  var w_rule = h_rule = '', classes = [],
		w, h, settings = bd.media.view.settings,
...
#>
<div style="{{ w_rule }}{{ h_rule }}" class="bd-video">
<video controls
	class="bd-video-shortcode {{ classes.join( ' ' ) }}"
	<# if ( w ) { #>width="{{ w }}"<# } #>
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
 ...
 * @global bool $is_IE
 */
function bd_print_media_templates() {
	global $is_IE;
	$class = 'media-modal bd-core-ui';
	...
	<script type="text/html" id="tmpl-uploader-inline">
		...
		<# if ( data.canClose ) { #>
		<button class="close basicons basicons-no"><span class="screen-reader-text"><?php _e( 'Close uploader' ); ?></span></button>
		<# } #>
		...
		<?php if ( ! _device_can_upload() ) : ?>
			<h3 class="upload-instructions"><?php printf( __('The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.'), 'http://blasdoise.com/apps/' ); ?></h3>
		<?php elseif ( is_multisite() && ! is_upload_space_available() ) : ?>
			<h3 class="upload-instructions"><?php _e( 'Upload Limit Exceeded' ); ?></h3>
			<?php
			/** This action is documented in bd-admin/includes/media.php */
	...
	<script type="text/html" id="tmpl-edit-attachment-frame">
		<div class="edit-media-header">
			<button class="left basicons <# if ( ! data.hasPrevious ) { #> disabled <# } #>"><span class="screen-reader-text"><?php _e( 'Edit previous media item' ); ?></span></button>
			<button class="right basicons <# if ( ! data.hasNext ) { #> disabled <# } #>"><span class="screen-reader-text"><?php _e( 'Edit next media item' ); ?></span></button>
		</div>
		<div class="media-frame-title"></div>
		<div class="media-frame-content"></div>
	</script>
		...
		<div class="attachment-media-view {{ data.orientation }}">
			<div class="thumbnail thumbnail-{{ data.type }}">
				...
				<# } else if ( 'video' === data.type ) {
					...
					} else if ( bd.media.view.settings.contentWidth ) {
						w_rule = 'width: ' + bd.media.view.settings.contentWidth + 'px;';
					}
					if ( data.height ) {
						h_rule = 'height: ' + data.height + 'px;';
					}
				#>
				<div style="{{ w_rule }}{{ h_rule }}" class="bd-media-wrapper bd-video">
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<label class="setting">
			...
			<select class="link-to"
				...
				<option value="post" <# if ( ! bd.media.galleryDefaults.link || 'post' == bd.media.galleryDefaults.link ) {
					#>selected="selected"<# }
				#>>
					<?php esc_attr_e('Attachment Page'); ?>
				</option>
				<option value="file" <# if ( 'file' == bd.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_attr_e('Media File'); ?>
				</option>
				<option value="none" <# if ( 'none' == bd.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_attr_e('None'); ?>
				</option>
			</select>
		</label>
			...
			<select class="columns" name="columns"
				...
				<?php for ( $i = 1; $i <= 9; $i++ ) : ?>
					<option value="<?php echo esc_attr( $i ); ?>" <#
						if ( <?php echo $i ?> == bd.media.galleryDefaults.columns ) { #>selected="selected"<# }
					#>>
						<?php echo esc_html( $i ); ?>
					</option>
				<?php endfor; ?>
	...
	<script type="text/html" id="tmpl-audio-details">
		<# var ext, html5types = {
			mp3: bd.media.view.settings.embedMimes.mp3,
			ogg: bd.media.view.settings.embedMimes.ogg
		}; #>

		<?php $audio_types = bd_get_audio_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-audio-settings">
				<?php bd_underscore_audio_template() ?>
	...
	<script type="text/html" id="tmpl-video-details">
		<# var ext, html5types = {
			mp4: bd.media.view.settings.embedMimes.mp4,
			ogv: bd.media.view.settings.embedMimes.ogv,
			webm: bd.media.view.settings.embedMimes.webm
		}; #>

		<?php $video_types = bd_get_video_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-video-settings">
				<div class="bd-video-holder">
				<#
				...
				<?php bd_underscore_video_template() ?>
	...
	<script type="text/html" id="tmpl-editor-gallery">
				...
				<# _.each( data.attachments, function( attachment, index ) { #>
					<dl class="gallery-item">
						...
						<# if ( attachment.caption ) { #>
							<dd class="bd-caption-text gallery-caption">
								{{ attachment.caption }}
							</dd>
						<# } #>
					</dl>
		...
		<# } else { #>
			<div class="bdview-error">
				<div class="basicons basicons-format-gallery"></div><p><?php _e( 'No items found.' ); ?></p>
			</div>
		<# } #>
	</script>
