<?php
/**
 * Blasdoise Customize Nav Menus classes
 *
 * @package Blasdoise
 ...
 */
final class BD_Customize_Nav_Menus {
	 ...
	 * @access public
	 *
	 * @param object $manager An instance of the BD_Customize_Manager class.
	 */
	public function __construct( $manager ) {
		$this->previewed_menus = array();
		$this->manager         = $manager;

		add_action( 'bd_ajax_load-available-menu-items-customizer', array( $this, 'ajax_load_available_items' ) );
		add_action( 'bd_ajax_search-available-menu-items-customizer', array( $this, 'ajax_search_available_items' ) );
	...
	public function ajax_load_available_items() {
		...
		if ( empty( $_POST['type'] ) || empty( $_POST['object'] ) ) {
			bd_send_json_error( 'nav_menus_missing_type_or_object_parameter' );
		}
		...
		if ( is_bd_error( $items ) ) {
			bd_send_json_error( $items->get_error_code() );
		} else {
			bd_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 * @return BD_Error|array Returns either a BD_Error object or an array of menu items.
	 */
	public function load_available_items_query( $type = 'post_type', $object = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $type ) {
			if ( ! get_post_type_object( $object ) ) {
				return new BD_Error( 'nav_menus_invalid_post_type' );
			}
	...
	public function ajax_search_available_items() {
		...
		if ( empty( $_POST['search'] ) ) {
			bd_send_json_error( 'nav_menus_missing_search_parameter' );
		}
		...
		$s = sanitize_text_field( bd_unslash( $_POST['search'] ) );
		$items = $this->search_available_items_query( array( 'pagenum' => $p, 's' => $s ) );

		if ( empty( $items ) ) {
			bd_send_json_error( array( 'message' => __( 'No results found.' ) ) );
		} else {
			bd_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 *
	 * Based on BD_Editor::bd_link_query().
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function enqueue_scripts() {
		bd_enqueue_style( 'customize-nav-menus' );
		bd_enqueue_script( 'customize-nav-menus' );

		$temp_nav_menu_setting      = new BD_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new BD_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );

		// Pass data to JS.
		$settings = array(
			'nonce'                => bd_create_nonce( 'customize-menus' ),
			'allMenus'             => bd_get_nav_menus(),
		...
		$data = sprintf( 'var _bdCustomizeNavMenusSettings = %s;', bd_json_encode( $settings ) );
		bd_scripts()->add_data( 'customize-nav-menus', 'data', $data );
		...
		bd_localize_script( 'nav-menu', 'menus', $nav_menus_l10n );
	}
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( BD_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type' => BD_Customize_Nav_Menu_Setting::TYPE,
			);
		} elseif ( preg_match( BD_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type' => BD_Customize_Nav_Menu_Item_Setting::TYPE,
			);
		}
		return $setting_args;
	}
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && BD_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'BD_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && BD_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'BD_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'BD_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'BD_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'BD_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'BD_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'BD_Customize_Nav_Menu_Item_Control' );
		...
		if ( current_theme_supports( 'widgets' ) ) {
			$description .= '<p>' . sprintf( __( 'Menus can be displayed in locations defined by your theme or in <a href="%s">widget areas</a> by adding a &#8220;Custom Menu&#8221; widget.' ), "javascript:bd.customize.panel( 'widgets' ).focus();" ) . '</p>';
		} else {
			$description .= '<p>' . __( 'Menus can be displayed in locations defined by your theme.' ) . '</p>';
		}
		$this->manager->add_panel( new BD_Customize_Nav_Menus_Panel( $this->manager, 'nav_menus', array(
			'title'       => __( 'Menus' ),
			'description' => $description,
			'priority'    => 100,
			// 'theme_supports' => 'menus|widgets', @todo allow multiple theme supports
		) ) );
		$menus = bd_get_nav_menus();
		...
		foreach ( $menus as $menu ) {
			$choices[ $menu->term_id ] = bd_html_excerpt( $menu->name, 40, '&hellip;' );
		}
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control( new BD_Customize_Nav_Menu_Location_Control( $this->manager, $setting_id, array(
				'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section( new BD_Customize_Nav_Menu_Section( $this->manager, $section_id, array(
			...
			$this->manager->add_setting( new BD_Customize_Nav_Menu_Setting( $this->manager, $nav_menu_setting_id ) );

			// Add the menu contents.
			$menu_items = (array) bd_get_nav_menu_items( $menu_id );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting( new BD_Customize_Nav_Menu_Item_Setting( $this->manager, $menu_item_setting_id, array(
					'value' => $value,
				) ) );

				// Create a control for each menu item.
				$this->manager->add_control( new BD_Customize_Nav_Menu_Item_Control( $this->manager, $menu_item_setting_id, array(
					'label'    => $item->title,
					'section'  => $section_id,
					'priority' => 10 + $i,
				) ) );
		...
		$this->manager->add_section( new BD_Customize_New_Menu_Section( $this->manager, 'add_menu', array(
			'title'    => __( 'Add a Menu' ),
			'panel'    => 'nav_menus',
			'priority' => 999,
		) ) );
		...
		$this->manager->add_control( new BD_Customize_New_Menu_Control( $this->manager, 'create_new_menu', array(
			'section' => 'add_menu',
		) ) );
	}
	 ...
	 *
	 * Templates are imported into the JS use bd.template.
	 *
	 * @since 1.0.0
	 * @access public
	 */
				...
				<div class="menu-item-bar">
					<div class="menu-item-handle">
						...
						<span class="item-title" aria-hidden="true">
							<span class="menu-item-title<# if ( ! data.title ) { #> no-title<# } #>">{{ data.title || bd.customize.Menus.data.l10n.untitled }}</span>
						</span>
						<button type="button" class="not-a-button item-add">
							<span class="screen-reader-text"><?php
								/* translators: 1: Title of a menu item, 2: Type of a menu item */
								printf( __( 'Add to menu: %1$s (%2$s)' ), '{{ data.title || bd.customize.Menus.data.l10n.untitled }}', '{{ data.type_label }}' );
							?></span>
						</button>
					</div>
				</div>
	...
	const RENDER_QUERY_VAR = 'bd_customize_menu_render';

	/**
	 * The number of bd_nav_menu() calls which have happened in the preview.
	 *
	 * @since 1.0.0
	 * @access public
	 * @var int
	...
	public function customize_preview_init() {
		...
		add_action( 'bd_enqueue_scripts', array( $this, 'customize_preview_enqueue_deps' ) );

		if ( ! isset( $_REQUEST[ self::RENDER_QUERY_VAR ] ) ) {
			add_filter( 'bd_nav_menu_args', array( $this, 'filter_bd_nav_menu_args' ), 1000 );
			add_filter( 'bd_nav_menu', array( $this, 'filter_bd_nav_menu' ), 10, 2 );
		}
	}

	/**
	 * Keep track of the arguments that are being passed to bd_nav_menu().
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @see bd_nav_menu()
	 *
	 * @param array $args An array containing bd_nav_menu() arguments.
	 * @return array Arguments.
	 */
	public function filter_bd_nav_menu_args( $args ) {
	...
	/**
	 * Prepare bd_nav_menu() calls for partial refresh. Wraps output in container for refreshing.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 * @param object $args             An object containing bd_nav_menu() arguments.
	 * @return null
	 */
	public function filter_bd_nav_menu( $nav_menu_content, $args ) {
	...
	public function hash_nav_menu_args( $args ) {
		return bd_hash( bd_create_nonce( self::RENDER_AJAX_ACTION ) . serialize( $args ) );
	}
	...
	public function customize_preview_enqueue_deps() {
		bd_enqueue_script( 'customize-preview-nav-menus' );
		bd_enqueue_style( 'customize-preview' );

		add_action( 'bd_print_footer_scripts', array( $this, 'export_preview_data' ) );
	}
	...
	public function export_preview_data() {
		...
		// Why not bd_localize_script? Because we're not localizing, and it forces values into strings.
		$exports = array(
			'renderQueryVar'        => self::RENDER_QUERY_VAR,
			'renderNonceValue'      => bd_create_nonce( self::RENDER_AJAX_ACTION ),
			...
			'previewCustomizeNonce' => bd_create_nonce( 'preview-customize_' . $this->manager->get_stylesheet() ),
			'navMenuInstanceArgs'   => $this->preview_nav_menu_instance_args,
		);

		if ( ! empty( $_SERVER['REQUEST_URI'] ) ) {
			$exports['requestUri'] = esc_url_raw( home_url( bd_unslash( $_SERVER['REQUEST_URI'] ) ) );
		}

		printf( '<script>var _bdCustomizePreviewNavMenusExports = %s;</script>', bd_json_encode( $exports ) );
	}

	/**
	 * Render a specific menu via bd_nav_menu() using the supplied arguments.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function render_menu() {
		...
		if ( empty( $_POST[ self::RENDER_NONCE_POST_KEY ] ) ) {
			bd_send_json_error( 'missing_nonce_param' );
		}

		if ( ! is_customize_preview() ) {
			bd_send_json_error( 'expected_customize_preview' );
		}

		if ( ! check_ajax_referer( self::RENDER_AJAX_ACTION, self::RENDER_NONCE_POST_KEY, false ) ) {
			bd_send_json_error( 'nonce_check_fail' );
		}

		if ( ! current_user_can( 'edit_theme_options' ) ) {
			bd_send_json_error( 'unauthorized' );
		}

		if ( ! isset( $_POST['bd_nav_menu_args'] ) ) {
			bd_send_json_error( 'missing_param' );
		}

		if ( ! isset( $_POST['bd_nav_menu_args_hash'] ) ) {
			bd_send_json_error( 'missing_param' );
		}

		$bd_nav_menu_args = json_decode( bd_unslash( $_POST['bd_nav_menu_args'] ), true );
		if ( ! is_array( $bd_nav_menu_args ) ) {
			bd_send_json_error( 'bd_nav_menu_args_not_array' );
		}

		$bd_nav_menu_args_hash = sanitize_text_field( bd_unslash( $_POST['bd_nav_menu_args_hash'] ) );
		if ( ! hash_equals( $this->hash_nav_menu_args( $bd_nav_menu_args ), $bd_nav_menu_args_hash ) ) {
			bd_send_json_error( 'bd_nav_menu_args_hash_mismatch' );
		}

		$bd_nav_menu_args['echo'] = false;
		bd_send_json_success( bd_nav_menu( $bd_nav_menu_args ) );
	}
}
