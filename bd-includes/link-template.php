<?php
/**
 * Blasdoise Link Template Functions
 *
 * @package Blasdoise
 * @subpackage Template
 */
...
function user_trailingslashit($string, $type_of_url = '') {
	global $bd_rewrite;
	if ( $bd_rewrite->use_trailing_slashes )
 ...
 * @see get_permalink()
 *
 * @param int|BD_Post $id        Optional. Post ID or post object. Default is the current post.
 ...
 * @since 1.0.0
 *
 * @param int|BD_Post $id        Optional. Post ID or post object. Default current post.
 * @param bool        $leavename Optional. Whether to keep post name or page name. Default false.
 * @return string|false The permalink URL or false if post does not exist.
 */
function get_permalink( $id = 0, $leavename = false ) {
	 ...
	 * @param BD_Post $post      The post in question.
	...
	if ( '' != $permalink && !in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft', 'future' ) ) ) {
		...
		if ( strpos($permalink, '%category%') !== false ) {
			...
			if ( empty($category) ) {
				...
				$category = is_bd_error( $default_category ) ? '' : $default_category->slug;
			}
		}
 ...
 * @return string|BD_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $bd_rewrite->get_extra_permastruct($post->post_type);
 ...
 * @since 1.0.0
 *
 * @param int|BD_Post $post_id    Optional. Post ID or BD_Post object. Default is global $post.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $bd_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the Blasdoise Loop or outside of it.
 *
 * @since 1.0.0
 *
 * @global BD_Rewrite $bd_rewrite
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $bd_rewrite->using_permalinks() && $parent ) {
...
function get_year_link($year) {
	...
	$yearlink = $bd_rewrite->get_year_permastruct();
	if ( !empty($yearlink) ) {
...
function get_month_link($year, $month) {
	...
	$monthlink = $bd_rewrite->get_month_permastruct();
	if ( !empty($monthlink) ) {
...
function get_day_link($year, $month, $day) {
	...
	$daylink = $bd_rewrite->get_day_permastruct();
	if ( !empty($daylink) ) {
...
function get_feed_link($feed = '') {
	...
	$permalink = $bd_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos($feed, 'comments_') ) {
			$feed = str_replace('comments_', '', $feed);
			$permalink = $bd_rewrite->get_comment_feed_permastruct();
		}
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $bd_rewrite->get_search_permastruct();
...
function get_search_comments_feed_link($search_query = '', $feed = '') {
	...
	/** This filter is documented in bd-includes/link-template.php */
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] )
			$struct = $bd_rewrite->front . $struct;
		else
			$struct = $bd_rewrite->root . $struct;
 ...
 *
 * Can be used within the Blasdoise loop or outside of it. Can be used with
 ...
 *
 * Can be used within the Blasdoise loop or outside of it, with any post type.
 *
 * @since 1.0.0
 *
 * @param int    $id           Optional. Post ID.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', bd_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 * @return null|string|BD_Post Post object if successful. Null if global $post is not set. Empty string if no corresponding post exists.
 */
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		$join = " INNER JOIN $bddb->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $bddb->term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";
		$where = $bddb->prepare( "AND tt.taxonomy = %s", $taxonomy );

		if ( ! empty( $excluded_terms ) && ! is_array( $excluded_terms ) ) {
			// back-compat, $excluded_terms used to be $excluded_terms with IDs separated by " and "
			if ( false !== strpos( $excluded_terms, ' and ' ) ) {
				_deprecated_argument( __FUNCTION__, '1.0', sprintf( __( 'Use commas instead of %s to separate excluded terms.' ), "'and'" ) );
		...
		if ( $in_same_term ) {
			if ( ! is_object_in_taxonomy( $post->post_type, $taxonomy ) )
				return '';
			$term_array = bd_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is_bd_error( $term_array ) )
				return '';
		...
		if ( ! empty( $excluded_terms ) ) {
			$where .= " AND p.ID NOT IN ( SELECT tr.object_id FROM $bddb->term_relationships tr LEFT JOIN $bddb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" . implode( $excluded_terms, ',' ) . ') )';
		}
	}
	...
	if ( is_user_logged_in() ) {
		...
		foreach ( (array) $private_states as $state ) {
			if ( current_user_can( $read_private_cap ) ) {
				$where .= $bddb->prepare( " OR p.post_status = %s", $state );
			} else {
				$where .= $bddb->prepare( " OR (p.post_author = %d AND p.post_status = %s)", $user_id, $state );
			}
		}
		$where .= " )";
	} else {
	 ...
	 * @param array  $excluded_terms Array of excluded term IDs.
	 */
	$where = apply_filters( "get_{$adjacent}_post_where", $bddb->prepare( "WHERE p.post_date $op %s AND p.post_type = %s $where", $current_post_date, $post->post_type ), $in_same_term, $excluded_terms );
	...
	$query = "SELECT p.ID FROM $bddb->posts AS p $join $where $sort";
	$query_key = 'adjacent_post_' . md5( $query );
	$result = bd_cache_get( $query_key, 'counts' );
	if ( false !== $result ) {
		if ( $result )
	...
	$result = $bddb->get_var( $query );
	if ( null === $result )
		$result = '';

	bd_cache_set( $query_key, $result, 'counts' );
 ...
 *
 * This is meant to be attached to actions like 'bd_header'. Do not call this directly in plugins or theme templates.
 * @since 1.0.0
 *
 */
function adjacent_posts_rel_link_bd_header() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param BD_Post $post     The adjacent post.
...
function get_pagenum_link($pagenum = 1, $escape = true ) {
	...
	if ( !$bd_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$bd_rewrite->pagination_base/\d+/?$|", '', $request);
		$request = preg_replace( '|^' . preg_quote( $bd_rewrite->index, '|' ) . '|i', '', $request);
		$request = ltrim($request, '/');

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $bd_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) )
			$base .= $bd_rewrite->index . '/';

		if ( $pagenum > 1 ) {
			$request = ( ( !empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $bd_rewrite->pagination_base . "/" . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $bd_query;

	if ( !$max_page )
		$max_page = $bd_query->max_num_pages;
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		...
		$max_num_pages = $bd_query->max_num_pages;
 ...
 * @since 1.0.0
 *
 * @global BD_Query $bd_query Blasdoise Query object.
 *
 * @param array $args {
...
function get_the_posts_navigation( $args = array() ) {
	...
	if ( $GLOBALS['bd_query']->max_num_pages > 1 ) {
		$args = bd_parse_args( $args, array(
...
function get_comments_pagenum_link( $pagenum = 1, $max_page = 0 ) {
	...
	if ( 'newest' == get_option('default_comments_page') ) {
		if ( $pagenum != $max_page ) {
			if ( $bd_rewrite->using_permalinks() )
				$result = user_trailingslashit( trailingslashit($result) . $bd_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged');
			else
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($max_page) )
		$max_page = $bd_query->max_num_comment_pages;
...
function paginate_comments_links($args = array()) {
	...
	if ( $bd_rewrite->using_permalinks() )
		$defaults['base'] = user_trailingslashit(trailingslashit(get_permalink()) . $bd_rewrite->comments_pagination_base . '-%#%', 'commentpaged');
...
function get_shortcut_link() {
	global $is_IE, $bd_version;

	include_once( ABSPATH . 'bd-admin/includes/class-bd-press-this.php' );
	$bookmarklet_version = $GLOBALS['bd_press_this']->version;
	$link = '';
	...
	if ( $is_IE ) {
		...
		if ( ! empty( $ua ) && preg_match( '/\bMSIE (\d)/', $ua, $matches ) && (int) $matches[1] <= 8 ) {
			$url = bd_json_encode( admin_url( 'press-this.php' ) );
	...
	if ( empty( $link ) ) {
		$src = @file_get_contents( ABSPATH . 'bd-admin/js/bookmarklet.min.js' );

		if ( $src ) {
			$url = bd_json_encode( admin_url( 'press-this.php' ) . '?v=' . $bookmarklet_version );
...
function get_admin_url( $blog_id = null, $path = '', $scheme = 'admin' ) {
	$url = get_site_url($blog_id, 'bd-admin/', $scheme);
...
function includes_url( $path = '', $scheme = null ) {
	 ...
	 * @param string $path Path relative to the URL to the bd-includes directory. Blank string
...
function content_url($path = '') {
	$url = set_url_scheme( BD_CONTENT_URL );
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$path = bd_normalize_path( $path );
	$plugin = bd_normalize_path( $plugin );
	$mu_plugin_dir = bd_normalize_path( BDMU_PLUGIN_DIR );
...
function network_admin_url( $path = '', $scheme = 'admin' ) {
	...
	$url = network_site_url('bd-admin/network/', $scheme);
...
function user_admin_url( $path = '', $scheme = 'admin' ) {
	$url = network_site_url('bd-admin/user/', $scheme);
...
function rel_canonical() {
	...
	global $bd_the_query;
	if ( !$id = $bd_the_query->get_queried_object_id() )
		return;
 ...
 */
function bd_get_shortlink($id = 0, $context = 'post', $allow_slugs = true) {
	/**
	...
	if ( 'query' == $context && is_singular() ) {
		$post_id = $bd_query->get_queried_object_id();
 ...
 *
 *  Attached to the bd_header action.
 *
 * @since 1.0.0
 */
function bd_shortlink_bd_header() {
	$shortlink = bd_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the bd action.
 *
 * @since 1.0.0
 */
function bd_shortlink_header() {
	if ( headers_sent() )
		return;
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = bd_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof BD_User ) {
		// User Object
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof BD_Post ) {
		// Post Object
		...
		if ( ( ! $user || is_bd_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
