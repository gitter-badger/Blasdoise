...
if ( empty($_REQUEST) ) {
	$referer = '<input type="hidden" name="bd_http_referer" value="'. esc_attr( bd_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset($_REQUEST['bd_http_referer']) ) {
	$redirect = remove_query_arg(array('bd_http_referer', 'updated', 'delete_count'), bd_unslash( $_REQUEST['bd_http_referer'] ) );
	$referer = '<input type="hidden" name="bd_http_referer" value="' . esc_attr($redirect) . '" />';
} else {
	$redirect = 'users.php';
	$referer = '';
}
...
switch ( $bd_list_table->current_action() ) {
...
case 'promote':
	...
	if ( ! current_user_can( 'promote_users' ) )
		bd_die( __( 'You can&#8217;t edit that user.' ) );
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $current_user->ID && ! $bd_roles->role_objects[ $_REQUEST['new_role'] ]->has_cap('promote_users')
	...
	bd_redirect(add_query_arg('update', $update, $redirect));
	exit();

case 'dodelete':
	if ( is_multisite() )
		bd_die( __('User deletion is not allowed from this screen.') );
	...
	if ( empty( $_REQUEST['delete_option'] ) ) {
		...
		$url = str_replace( '&amp;', '&', bd_nonce_url( $url, 'bulk-users' ) );
		bd_redirect( $url );
		exit;
	}

	if ( ! current_user_can( 'delete_users' ) )
		bd_die(__('You can&#8217;t delete users.'));
	...
	foreach ( $userids as $id ) {
		if ( ! current_user_can( 'delete_user', $id ) )
			bd_die(__( 'You can&#8217;t delete that user.' ) );
		...
		case 'delete':
			...
			break;
		case 'reassign':
			bd_delete_user( $id, $_REQUEST['reassign_user'] );
			break;
		}
		++$delete_count;
	}
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new BD_Error( 'edit_users', __( 'You can&#8217;t delete users.' ) );
...
<?php bd_nonce_field('delete-users') ?>
<?php echo $referer; ?>
...
<?php if ( $go_delete ) : ?>
	...
	<ul style="list-style:none;">
		...
		bd_dropdown_users( array( 'name' => 'reassign_user', 'exclude' => array_diff( $userids, array($current_user->ID) ) ) ); ?></li>
	</ul></fieldset>
	<?php
	/**
	 ...
	 * @since 1.0.0
	 *
	 * @param BD_User $current_user BD_User object for the user being deleted.
	 */
...
case 'doremove':
	...
	if ( ! is_multisite() )
		bd_die( __( 'You can&#8217;t remove users.' ) );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new BD_Error('edit_users', __('You can&#8217;t remove users.'));
...
?>
<form method="post" name="updateusers" id="updateusers">
<?php bd_nonce_field('remove-users') ?>
<?php echo $referer; ?>
...
default:
	...
	if ( !empty($_GET['_bd_http_referer']) ) {
		bd_redirect( remove_query_arg( array( '_bd_http_referer', '_bdnonce'), bd_unslash( $_SERVER['REQUEST_URI'] ) ) );
		exit;
	}

	$bd_list_table->prepare_items();
	$total_pages = $bd_list_table->get_pagination_arg( 'total_pages' );
	if ( $pagenum > $total_pages && $total_pages > 0 ) {
		bd_redirect( add_query_arg( 'paged', $total_pages ) );
		exit;
	}
		...
		case 'add':
			if ( isset( $_GET['id'] ) && ( $user_id = $_GET['id'] ) && current_user_can( 'edit_user', $user_id ) ) {
				$messages[] = '<div id="message" class="updated notice is-dismissible"><p>' . sprintf( __( 'New user created. <a href="%s">Edit user</a>' ),
					esc_url( add_query_arg( 'bd_http_referer', urlencode( bd_unslash( $_SERVER['REQUEST_URI'] ) ),
...
<?php $bd_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
