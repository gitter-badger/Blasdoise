 ...
 * @access private
 */
class BD_MS_Users_List_Table extends BD_List_Table {
	/**
	 *
	 * @return bool
	 */
	...
	public function prepare_items() {
		global $usersearch, $role, $bddb, $mode;
		...
		if ( bd_is_large_network( 'users' ) )
			$args['search'] = ltrim( $args['search'], '*' );

		if ( $role == 'super' ) {
			$logins = implode( "', '", get_super_admins() );
			$args['include'] = $bddb->get_col( "SELECT ID FROM $bddb->users WHERE user_login IN ('$logins')" );
		}
		 ...
		 */
		if ( !$usersearch && bd_is_large_network( 'users' ) ) {
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'bdmu_users_columns', $users_columns );
	}
	 ...
	 * @access public
	 *
	 * @param BD_User $user The current BD_User object.
	 */
	...
	public function column_username( $user ) {
		...
		$edit_link = esc_url( add_query_arg( 'bd_http_referer', urlencode( bd_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	...
	public function column_default( $user, $column_name ) {
		/** This filter is documented in bd-admin/includes/class-bd-users-list-table.php */
	...
	protected function handle_row_actions( $user, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins ) ) {
			$actions['delete'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_bd_http_referer', urlencode( bd_unslash( $_SERVER['REQUEST_URI'] ) ), bd_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
