<?php
/**
 * Blasdoise API for creating bbcode like tags or what Blasdoise calls
 ...
 *
 * {@internal
 * Please be aware that the above note was made during the beta of Blasdoise 1.0
...
function do_shortcodes_in_html_tags( $content, $ignore_html ) {
	...
	foreach ( $textarr as &$element ) {
		...
		$attributes = bd_kses_attr_parse( $element );
		if ( false === $attributes ) {
		...
		foreach ( $attributes as &$attr ) {
			...
			if ( ( false === $single || $open < $single ) && ( false === $double || $open < $double ) ) {
				...
				if ( $count > 0 ) {
					...
					$new_attr = bd_kses_one_attr( $new_attr, $elname );
					if ( '' !== trim( $new_attr ) ) {
