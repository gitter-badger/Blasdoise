...
// Forked for Blasdoise so it can be turned on/off after loading.
 ...
 * it's initialized.
 */
tinymce.PluginManager.add( 'bdautoresize', function( editor ) {
	...
	function resize( e ) {
		...
		if (resizeHeight !== oldSize) {
			...
			editor.fire( 'bd-autoresize', { height: resizeHeight, deltaHeight: e.type === 'nodechange' ? deltaSize : null } );
		}
	}
	...
	function on() {
		if ( ! editor.dom.hasClass( editor.getBody(), 'bd-autoresize' ) ) {
			isActive = true;
			editor.dom.addClass( editor.getBody(), 'bd-autoresize' );
	...
	function off() {
		...
		if ( ! settings.bd_autoresize_on ) {
			isActive = false;
			doc = editor.getDoc();
			editor.dom.removeClass( editor.getBody(), 'bd-autoresize' );
	...
	editor.addCommand( 'bdAutoResize', resize );

	// On/off
	editor.addCommand( 'bdAutoResizeOn', on );
	editor.addCommand( 'bdAutoResizeOff', off );
});
