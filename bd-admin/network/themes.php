...
if ( !current_user_can('manage_network_themes') )
	bd_die( __( 'You do not have sufficient permissions to manage network themes.' ) );
	...
	switch ( $action ) {
		case 'enable':
			...
			if ( false === strpos( $referer, '/network/themes.php' ) )
				bd_redirect( network_admin_url( 'themes.php?enabled=1' ) );
			else
				bd_safe_redirect( add_query_arg( 'enabled', 1, $referer ) );
			exit;
		case 'disable':
			check_admin_referer('disable-theme_' . $_GET['theme']);
			unset( $allowed_themes[ $_GET['theme'] ] );
			update_site_option( 'allowedthemes', $allowed_themes );
			bd_safe_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
		case 'enable-selected':
			check_admin_referer('bulk-themes');
			$themes = isset( $_POST['checked'] ) ? (array) $_POST['checked'] : array();
			if ( empty($themes) ) {
				bd_safe_redirect( add_query_arg( 'error', 'none', $referer ) );
				exit;
			}
			foreach( (array) $themes as $theme )
				$allowed_themes[ $theme ] = true;
			update_site_option( 'allowedthemes', $allowed_themes );
			bd_safe_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
		case 'disable-selected':
			...
			update_site_option( 'allowedthemes', $allowed_themes );
			bd_safe_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
		...
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) ) {
				bd_die( __('You do not have sufficient permissions to delete themes for this site.') );
			}
			...
			if ( empty( $themes ) ) {
				bd_safe_redirect( add_query_arg( 'error', 'main', $referer ) );
				exit;
			}
			...
			foreach ( $themes as $key => $theme ) {
				$theme_info[ $theme ] = bd_get_theme( $theme );
				...
				<div id="files-list" style="display:none;">
					...
					<?php
						foreach ( (array) $files_to_delete as $file ) {
							echo '<li>' . esc_html( str_replace( BD_CONTENT_DIR . '/themes', '', $file ) ) . '</li>';
						}
					?>
					</ul>
				</div>
			...
			foreach ( $themes as $theme ) {
				$delete_result = delete_theme( $theme, esc_url( add_query_arg( array(
					...
					'_bdnonce' => $_REQUEST['_bdnonce']
				), network_admin_url( 'themes.php' ) ) ) );
			}

			$paged = ( $_REQUEST['paged'] ) ? $_REQUEST['paged'] : 1;
			bd_redirect( add_query_arg( array(
...
bd_enqueue_script( 'theme-preview' );
